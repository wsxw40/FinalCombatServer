...
//					lastRunTime.get(Calendar.DAY_OF_WEEK)==now.get(Calendar.DAY_OF_WEEK))){
			if(!isManual &&(lastRunTime!=null&&lastRunTime.get(Calendar.YEAR)==now.get(Calendar.YEAR) && lastRunTime.get(Calendar.WEEK_OF_YEAR)==now.get(Calendar.WEEK_OF_YEAR))){	
			...
			}else if(now.after(ServiceLocator.getService.getCompeteBuyTime(0))&&now.before(ServiceLocator.getService.getCompeteBuyTime(1))){
				...
				ServiceLocator.soClient.updateBillBoardList(CommonUtil.messageFormatI18N(CommonMsg.CMPT_OVER_NTS_BFR_MSG,validNums));
