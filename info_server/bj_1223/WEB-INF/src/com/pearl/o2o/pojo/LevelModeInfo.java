...
public class LevelModeInfo {
	...
	public LevelModeInfo(LevelInfo pojo){
		this.id = pojo.getId();
		this.type = pojo.getType();
		this.name = pojo.getName();
		this.displayName=pojo.getDisplayName();
		this.displayNameCN=pojo.getDisplayNameCN();
		...
		if (!StringUtil.isEmptyString(pojo.getVehicleInfo())) {
			this.vehicleInfo=VehicleInfo.createFromText(pojo.getVehicleInfo());
		}
		//start points
		...
		//vehicle line
		if (!StringUtil.isEmptyString(pojo.getLineInfo())&&(type==Constants.GAMETYPE.DELIVER.getValue()||type==Constants.GAMETYPE.NEWTRAIN.getValue())) {
			...
			for (String s : sArray) {
				if(s.indexOf(",")!=-1){
					...
					if("1".equals(tArray[0])){
						vehicleLine1Points.add(VehicleLinePoint.createFromText(s));
					}else if("2".equals(tArray[0])){
						vehicleLine2Points.add(VehicleLinePoint.createFromText(s));
					}
				}
			}
			Collections.sort(vehicleLine1Points,new Comparator<VehicleLinePoint>(){

				@Override
				public int compare(VehicleLinePoint o1, VehicleLinePoint o2) {
					
					return ((Integer)o1.getIndex()).compareTo((Integer)o2.getIndex());
				}});
			Collections.sort(vehicleLine2Points,new Comparator<VehicleLinePoint>(){
		...
		if(!StringUtil.isEmptyString(pojo.getLineInfo())&&type==Constants.GAMETYPE.BOSS2.getValue()){
			sArray = pojo.getLineInfo().split(";");
			for (String s : sArray) {
...
//TODO no use	
	public LevelInfo toPojo(){
		LevelInfo pojo = new LevelInfo();
		pojo.setId(this.id);
		...
		for (AABBPoints v : this.blastPoints){
			sb.append(v.toPlainString()).append(";");
		}
		pojo.setBlastPoints(sb.toString());
		
		sb = new StringBuilder();
		for (BannerPoints v : this.bannerPoint){
			sb.append(v.toPlainString()).append(";");
		}
		pojo.setFlagPoints(sb.toString());
		
		sb  = new StringBuilder();
		for (BossItem v : this.bossItems){
			sb.append(v.toPlainString()).append(";");
		}
		pojo.setBossItems(sb.toString());
	...
	public static class TeamPoints{
		...
		public static TeamPoints createFromText(String s){
			...
			return new TeamPoints(Integer.valueOf(id), Transform.createFromText(pointStr));
		}
	}
	public static class BannerPoints{
		...
		public static BannerPoints createFromText(String s){
			...
			String id = s.substring(0, index);
			String pointStr = s.substring(index + 1); 
			return new BannerPoints(Integer.valueOf(id), AABBPoints.createFromText(pointStr));
		}
	}
	public static class WeaponPoint{
		...
		public String toPlainString(){
			StringBuilder sb = new StringBuilder();
			sb.append(id).append(",").append(transform.toPlainString()).append(",").append(type);
			return sb.toString();
		}
		//id,x,y,z,rotate,type
		public static WeaponPoint createFromText(String s){
			...
			String id = index[0];
			String transformStr =index[1]+","+index[2]+","+index[3]+","+index[4];
			String type=index[5];
			return new WeaponPoint(Integer.valueOf(id), Transform.createFromText(transformStr),Integer.valueOf(type));
		}
	}
	...
	public static class AABBPoints{
		...
		public String toPlainString(){
			StringBuilder sb = new StringBuilder();
			sb.append(transform.toPlainString()).append(",").append(point.toPlainString());
			return sb.toString();
		}
		//x,y,z,rotate,x1,y1,z1
		public static AABBPoints createFromText(String s){
			...
			String transStr=sArray[0]+","+sArray[1]+","+sArray[2]+","+sArray[3];
			String pointStr=sArray[4]+","+sArray[5]+","+sArray[6];
			return new AABBPoints(Transform.createFromText(transStr),Vector3.createFromText(pointStr));
		}
	}
	...
	public static class BossItem {
		...
		public static BossItem createFromText(String s){
			...
			return new BossItem(Integer.valueOf(sArray[0]), Float.valueOf(sArray[1]), Float.valueOf(sArray[2]),Integer.valueOf(sArray[3]));
		}
	}
	public static class Supply {
		...
		public Supply(Vector3 vector, int type, String name,  int value, float appearRation, float skillTime) {
			this.vector=vector;
			this.type = type;
			this.name = name;
			this.value = value;
		...
		public String toPlainString(){
			StringBuilder sb = new StringBuilder();
			sb.append(vector.toPlainString()).append(type).append(",").append(name).append(",")
		...
		public static Supply createFromText(String s){
			...
			Vector3 v=new Vector3(Float.parseFloat(sArray[0]), Float.parseFloat(sArray[1]),Float.parseFloat(sArray[2]));
			return new Supply(v, Integer.valueOf(sArray[3]),sArray[4],Integer.valueOf(sArray[5]), Float.valueOf(sArray[6]), Float.valueOf(sArray[7]));
		}
	}
	...
	public static class VehicleLinePoint{
		...
		public VehicleLinePoint(int lineId,int index,Vector3 vector,Vector3 vector2,int isSlope) {
			this.lineId=lineId;
			this.index=index;
			this.vector=vector;
		...
		public String toPlainString(){
			StringBuilder sb = new StringBuilder();
			sb.append(lineId).append(",").append(index).append(",").append(vector.toPlainString()).append(",")
			.append(vector2.toPlainString()).append(",").append(isSlope);
			return sb.toString();
		}
		...
		public static VehicleLinePoint createFromText(String s){
			...
			Vector3 v=new Vector3(Float.valueOf(sArray[2]),Float.valueOf(sArray[3]),Float.valueOf(sArray[4]));
			Vector3 v2=new Vector3(Float.valueOf(sArray[5]),Float.valueOf(sArray[6]),Float.valueOf(sArray[7]));
			return new VehicleLinePoint(Integer.valueOf(sArray[0]),Integer.valueOf(sArray[1]),v,v2,Integer.valueOf(sArray[8]));
		}
	}
	public static class BossLinePoint{
		...
		public BossLinePoint(int lineId,int index,Vector3 vector,Vector3 vector2,int isSlope) {
			this.lineId=lineId;
			this.index=index;
			this.vector=vector;
			this.vector2=vector2;
		...
		public static BossLinePoint createFromText(String s){
			...
			Vector3 v=new Vector3(Float.valueOf(sArray[2]),Float.valueOf(sArray[3]),Float.valueOf(sArray[4]));
			Vector3 v2=new Vector3(Float.valueOf(sArray[5]),Float.valueOf(sArray[6]),Float.valueOf(sArray[7]));
			return new BossLinePoint(Integer.valueOf(sArray[0]),Integer.valueOf(sArray[1]),v,v2,Integer.valueOf(sArray[8]));
		}
	}
	public static class VehicleInfo{
		...
		public static VehicleInfo createFromText(String s){
			...
			Vector3 v=new Vector3(Float.valueOf(sArray[1]),Float.valueOf(sArray[2]),Float.valueOf(sArray[3]));
