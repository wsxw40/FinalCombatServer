...
import com.pearl.o2o.utils.Constants.BOUNDS;
...
//		sendSystemMail(player, CommonMsg.GIFT_EMAIL_SYS, content.toString(),ids.toString());
	...
	public void createTeamByItem(Integer playerId, String name,
		...
		team.setNumber(1);
		//TODO : mock data
		team.setUnusableResource(1000000);
		team.setUsableResource(100000);
...
//			}
		...
		}else if(Constants.RES_PAY == payTpye){//使用个人黑曜石进行付费
			...
			int stones= team==null ? 0 : playerRes.get(Player.RES);//可用黑曜石
			if (stones < cost) {
			...
			if (disStones < cost) {
				...
				//add log for have paid
				if(check!=null){
					check.append("paid "+cost+" DISSTONE|");
				}
				// 发布购买事件
				if (!Constants.BOOLEAN_YES.equals(isGift)&&!(sysItem.getIBuffId()>=61&&sysItem.getIBuffId()<=74)) {//战队buff:ibuffId:61-74,不会显示通知
					nosqlService.publishEvent(new PurchaseEvent(DateUtil.getCurrentTimeStr(), player.getId(), player.getName(), sysItem));
		...
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			...
			if (!isPresentPackage) {
				PlayerItem playerItem = getService.getPlayerItemById(playerId,
						playerItemId);
				expireDate = playerItem.getExpireDate();
			}
...
//				mcc.delete(CacheUtil.oPlayerInfo(playerId));
		...
		} else if(Constants.RES_PAY_TEAM == payTpye){//使用团队黑曜石进行付费
			if (teamStones < cost) {
...
//							.format(expireDate) : ""));
	...
	/**
	 * 用于个人空间，个人第一次进入时没有坦克，会创建一个个数为0的playerItem
	 * @param teamId
	 * @param sysItem
	 * @param unit
	 * @param unitType
	...
	public int sendItem(SysItem sysItem, Player player, Payment payment,String isGift,String isBind,String isDefault) throws Exception {
		...
		if(Constants.BOOLEAN_NO.equals(isGift)&&!(sysItem.getIBuffId()>=61&&sysItem.getIBuffId()<=74)&&sysItem.getType()!=9){//战队buff:ibuffId:61-74,不会显示通知
			soClient.puchCMDtoClient(player.getName(), CommonUtil.messageFormat(CommonMsg.HIGHLIGHT_STORAGE,sysItem.getType()));
		}
		//新手馈赠，直接使用
	...
	public void msgDLB(int serverId, String msg, String name) {
		try {
			soClient.messageDLB(serverId, msg, name);
		} catch (Exception e) {
			logger.warn("error happend during DLB, exception is" + e);
		}
	}

	public void msgDLBChannel(int serverId, int channelId, String msg,
			String name) {
		try {
			soClient.messageXLB(serverId, channelId, msg, name);
		} catch (Exception e) {
			logger.warn("error happend during DLB, exception is" + e);
		}
	}
	...
	public String useItemById(Player player, int type, PlayerItem playerItem, String message, int channelId, int serverId) throws Exception {
		...
		if (si.getIId() == 1) {
			...
			if (serverId != 0 && si.getIId() == 3) {// 大喇叭
				...
				msgDLB(serverId, message, player.getName());
				...
				message = KeywordFilterUtil.filter(StringUtil.escapeIndex(message));
				msgDLBChannel(serverId, channelId, message, player.getName());
				updateItemQuantity(playerItem);
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					ServiceLocator.nosqlService.addXunleiLog("1.3"
							+ Constants.XUNLEI_LOG_DELIMITER
...
//			updateItemQuantity(playerItem);
			...
			updateItemQuantity(playerItem);
		}else if(si.getIId() == 23){//等级礼包
			//成长任务2： 打开新手礼包
			returnStr=useLevelPackage(player, si);
			updateItemQuantity(playerItem);
...
//					}
	...
	public void choicePlayerItem(int playerId,int playerItemId,int itemId) throws Exception{
		...
		if(pi.getSysItem().getType()==Constants.DEFAULT_OPEN_TYPE&&pi.getSysItem().getIId()==26){
			...
			if(value>=14&& value<=16){
				...
				switch (value) {
				case 14:
					int[] rareArray=StringUtil.convertToInt(ServiceLocator.getService.getSysConfig().get("compete.rarekBox").split(","));
	...
	private String useNewPackage(Player player,PlayerItem playerItem) throws Exception{
		...
		if(new Date().getTime()<calendar.getTimeInMillis()){
			...
			for(int i[]:Constants.NEW_PACKAGE){
...
//				}
	...
	private String useLevelPackage(Player player,SysItem si) throws Exception{
		List<SysItem> returnList=new ArrayList<SysItem>();
		if(player.getRank()<si.getLevel()){
			throw new BaseException(ExceptionMessage.NOT_MATCH_LEVEL_USE);
		}
		int cal=0;
		for(int i=0;i<Constants.GREEN_GR_AWARD.length;i++){
			int[] levelGR=Constants.GREEN_GR_AWARD[i];
			if(levelGR[0]==si.getId()){
				awardCALToPlayer(player, levelGR[1]);
				cal=levelGR[1];
				int[][] awardList=Constants.GREEN_ITEM_AWARD[i];
...
//					SysItem sysItem = getService.getSysItemByItemId(si.getId());
				...
				if (action == 1) {// 接受礼物
					...
					soClient.puchCMDtoClient(receiver.getName(), CommonUtil.messageFormat(CommonMsg.HIGHLIGHT_STORAGE,item.getSysItem().getType()));
				}
				messageDao.deleteMessageItem(message.getId());
...
//		System.out.println("dateStr="+dateStr+" && playerId="+playerId+" && type="+type+" && yearMonth="+yearMonth+" && todayStr="+todayStr);		
		if (!nosqlService.isPlayerCheck(playerId, dateStr)) {//判断是否签过
			...
			int vipCheckTimes = 0; // vip会员允许免费补签和预签的次数
			switch (vipLevel) {
				case 1:
					vipCheckTimes = 3;
					break;
...
//			System.out.println("all checkTimes="+checkedTimes);
			...
			if (type==1) {//补签
				...
				cost = vipCheckTimes>vipCheckedTimes ? 0 : Constants.DAILY_CHECK_AGO_COST;
...
//			log.info("Player:" + playerId + "already checked on " + dateStr);
	 ...
	 * @throws Exception 
	 */
	public void createBoundsDaily(int playerId,BOUNDS bound) throws Exception{
		//个人黑原石奖励
		createResBoundsDaily(playerId,bound);
	}
	 ...
	 * @throws Exception
	 */
	private void createResBoundsDaily(int playerId,BOUNDS bound) throws Exception{
		...
		for(SysChest sc : sysChests){
			...
			if (si.getType() == 4 && si.getIId() == 8) {// CAL类道具
				...
				if(si.getIBuffId()<61||si.getIBuffId()>74){
					...
					if((flag&f)==0){
						flag=flag|f;
						soClient.puchCMDtoClient(player.getName(), CommonUtil.messageFormat(CommonMsg.HIGHLIGHT_STORAGE,si.getType()));
					}
				}
			}
			if (null != ids) {
...
//		}
	...
	public boolean createChallengeBattle(Player player,TOPTeam topTeam) throws Exception{
		...
		PlayerInfo playerInfo = getService.getPlayerInfoById(player.getId());
		int cr = playerInfo.getXunleiPoint();
		int leftMoney=cr-topTeam.getChallengeCost();
	...
	public void createPlayerBuyTeamItemLog(String s){
		infoLogger.log(LogServerMessage.buyPlayerItem.name(), Level.INFO_INT, 
				LogUtils.JoinerByTab.join("1.5",CommonUtil.simpleDateFormat.format(new Date()),s));
	}
	
	public void createIntensifyTeamItemLog(String s){
		infoLogger.log(LogServerMessage.buyPlayerItem.name(), Level.INFO_INT, 
				LogUtils.JoinerByTab.join("8.8",s));
	}
