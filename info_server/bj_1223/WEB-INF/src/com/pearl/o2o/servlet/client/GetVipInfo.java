		...
		try{
			...
			if(vipItem!=null){
				...
				if(vipLevel==Constants.VIP_LEVEL_EXP.length){
					return Converter.GetVipInfo(vipLevel,player.getVipExp(),player.getLeftMinites(), -1,vipItem);
				}	
				return Converter.GetVipInfo(vipLevel,player.getVipExp(), player.getLeftMinites(),Constants.VIP_LEVEL_EXP[vipLevel],vipItem);
			}
			return Converter.error(ExceptionMessage.ERROR_MESSAGE_ALL);
		}catch(Exception e){
