...
//						createService.sendItem(si, getService.getPlayerById(id),new Payment(num,1), "N", "Y", "N");
			...
			if(type==Constants.GAMETYPE.TEAMZYZDZ.getValue()) {
				 updateService.updatePlayerStageClearZYZDZ(stageClearInfo);
				
			}else{
				 updateService.updatePlayerStageClear(stageClearInfo);
				
			}
			String result = Converter.stageClear(stageClearInfo,mvpPlayer,mvpWeapon);		
