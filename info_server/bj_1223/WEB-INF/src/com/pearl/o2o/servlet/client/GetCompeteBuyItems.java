...
//	private int[] payMethods={Constants.CMPT_ITEM_ID_NUM_COST_ID_NUMS[0][1],Constants.CMPT_ITEM_ID_NUM_COST_ID_NUMS[1][1],Constants.CMPT_ITEM_ID_NUM_COST_ID_NUMS[2][1]};
	...
	protected String innerService(String... args) {
		try {
			...
			String timeType = "0";
			String retTimeStr = new SimpleDateFormat("yyyyMMdd").format(startTime);
			if(DateUtils.isSameDay(now, startTime)){
				if(now.before(startTime)){
					int needSeconds = (int)((startTime.getTime() - now.getTime())/1000);
					retTimeStr = String.valueOf(needSeconds);
					timeType ="1";
				}else if(now.before(endTime)){
					int leftSeconds = (int)((endTime.getTime() - now.getTime())/1000);
					retTimeStr = String.valueOf(leftSeconds);
					timeType ="2";
				}else{
					//FIXME:QP just set nextStartTime as next day for test
				//	Date nextStartTime = DateUtils.addDays(startTime, 1);
					Date nextStartTime = DateUtils.addWeeks(startTime, 1);
					retTimeStr = new SimpleDateFormat("yyyyMMdd").format(nextStartTime);
				}
			}else{
...
//			List<SysItem> items = new ArrayList<SysItem>();
