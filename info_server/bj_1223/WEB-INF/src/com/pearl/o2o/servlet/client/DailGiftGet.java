...
public class DailGiftGet extends BaseClientServlet{
	...
	protected String innerService(String... args) {
		try {
			...
			if(isGift==1){
				return Converter.dailyGift(ExceptionMessage.DAILY_GIFT_ALREADY_MSG, dailyAwards,isGift);
			}
			if(dailyAwards==null||dailyAwards.size()==0){
				return Converter.dailyGift(ExceptionMessage.DAILY_GIFT_FAILED_MSG, dailyAwards,isGift);
			}
			...
			for(OnlineAward oa : dailyAwards){
				...
				if(si!=null){
					oa.setSysItem(si);
					//根据vip等级 获得额外奖励
					pm.setUnit(oa.getUnit()+(int)Math.ceil(player.getIsVip()/3.0));
			...
			//发放不进行提示的奖励
			createBoundsDailyQuietly(playerId);
			
			return Converter.dailyGift(msg, dailyAwards,isGift);
	 ...
	 * @throws Exception
	 */
	private void createBoundsDailyQuietly(int playerId) throws Exception{
		//发放每日奖励
		createService.createBoundsDaily(playerId,Constants.BOUNDS.DAILY_CHECK_DIS_RES_BOUND);
	}
}
