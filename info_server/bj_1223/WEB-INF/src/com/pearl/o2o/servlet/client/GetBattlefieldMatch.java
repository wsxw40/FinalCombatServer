...
public class GetBattlefieldMatch extends BaseClientServlet {
	...
	protected String innerService(String... args) {
		...
		try {
			...
			if (matchCount > Constants.TeamSpaceConstants.MAX_MATCH_TIMES) {
				return Converter.GetMatchedTeam(null, null, true, false,canRobUnuseRes);
			}
			...
			if (matchCount == 1) {
				if (playerRes.get(Player.ORG_RES) < Constants.TeamSpaceConstants.MATCH_COST_STONES) {
					return Converter.GetMatchedTeam(null, null, false, true,canRobUnuseRes);
				} else {// 扣除玩家黑原石
			...
			if (defanceTeam != null) {
				...
				if (teamLevelInfo != null) {
					...
					result = Converter.GetMatchedTeam(defanceTeam,
							teamLevelInfo, false, false,canRobUnuseRes);
				} else {
					// result = Converter.GetMatchedTeam(defanceTeam,
	 ...
	 * @throws Exception
	 */
	private Team doMatch(Team attackTeam, int matchCount) throws Exception {
		Team result = null;
