...
public class GetFontPlayerInfo extends BaseClientServlet {
	...
	private static Logger log = LoggerFactory.getLogger(GetFontPlayerInfo.class.getName());	
	...
	protected String innerService(String... args) {
		try{		
			...
			ServiceLocator.missionLog.debug("GetFontPlayerInfo playerId="+playerId);
			PlayerInfo playerInfo=getService.getPlayerInfoById(playerId);
			ServiceLocator.missionLog.debug("GetFontPlayerInfo servlet point="+playerInfo.getXunleiPoint()
					+" playerId="+playerInfo.getPlayerId()+" "+(playerInfo.getXunleiPoint()==0));
			...
			if(player.getIsVip()<=0){
				deleteService.deleteVipBuff(playerId);
			}
...
//						getService.getSysItemByItemId(pi.getItemId());
					...
					if (Constants.DEFAULT_ITEM_TYPE == si.getType() && si.getIId() == 1&&si.getIBuffId()!=6 && si.getIId() == 1&&si.getIBuffId()!=32
...
//			}
			player.setBuffList(buffList);//just for smarty display in GUI,must't update player
...
//			int  rankNumInTop =StringUtil.toInt( nosqlService.getNosql().get(rankKey));
			...
			if(player.getGWin()!=0 || player.getGLose()!=0){
				winRate=CommonUtil.toTwoFloat(player.getGWin()/(player.getGWin()+player.getGLose()));
			}
			String team=player.getTeam()==null?"":player.getTeam();
			String result=Converter.fontPlayerInfo(player,emailNum,teamRequestNum,null,card,playerInfo,rankNumInTop,winRate,
					team,getService.getMaxStrengthLevel(playerId),player.getLeftMinites());
			return result;			
		}
		catch (Throwable e) {
			log.warn("Throwable in GetPlayer: "+args[1], e);
			return Converter.commonFeedback(ExceptionMessage.ERROR_MESSAGE_ALL);
//			return Converter.fontPlayerInfo(new Player(),0,0, e.getMessage(),0,new PlayerInfo());
		}	
	}
