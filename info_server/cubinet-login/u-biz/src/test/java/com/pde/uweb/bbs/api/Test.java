package com.pde.uweb.bbs.api;

import java.util.LinkedList;

import com.pde.uweb.utils.UCXMLHelper;

/**
 * ================================================
 * Discuz! Ucenter API for JAVA
 * ================================================
 * 测试类
 * 示例：本类实现在如何实现在登入/登出，以及注册。
 * 
 */
public class Test {

	/**
	 * http://192.168.2.30:8080/u-web/bbs/api
	 * 123456789qwertyuiop
	 * @param args
	 */
	public static void main(String[] args) {

//		testRegister();
		testLogin();
//		testLogout();
//		Client uc = new Client();
//		String s = uc.uc_authcode("8485m7QEfsvnOg9tKGvAsxlwXpAzZU6LhFA04pD6N0XIYG1cJVDh2Th83Qcci130UPHbXp+UNG0","DECODE");
//		System.out.println(s);
	
	}
	
	public static void testLogin(){
		
		BBSUCenterClient e = new BBSUCenterClient();
		String result = e.userLogin("hgflybird", "a1234567");
		
		LinkedList<String> rs = UCXMLHelper.unserialize(result);
		if(rs.size()>0){
			int $uid = Integer.parseInt(rs.get(0));
			String $username = rs.get(1);
			String $password = rs.get(2);
			String $email = rs.get(3);
			if($uid > 0) {
				System.out.println("登录成功");
				System.out.println($username);
				System.out.println($password);
				System.out.println($email);
				
				String $ucsynlogin = e.userSynLogin($uid);
				System.out.println("登录成功"+$ucsynlogin);

				//本地登陆代码
				//TODO ... ....
			} else if($uid == -1) {
				System.out.println("用户不存在,或者被删除");
			} else if($uid == -2) {
				System.out.println("密码错");
			} else {
				System.out.println("未定义");
			}
		}else{
			System.out.println("Login failed");
			System.out.println(result);
		}
	}
	
	public static void testLogout(){
		
		BBSUCenterClient uc = new BBSUCenterClient();

		//setcookie('Example_auth', '', -86400);
//		生成同步退出的代码
		String $ucsynlogout = uc.userSynLogout(1L);
		System.out.println("退出成功"+$ucsynlogout);
		

	}
	
	public static void testRegister(){
		
		BBSUCenterClient uc = new BBSUCenterClient();

		//setcookie('Example_auth', '', -86400);
//		生成同步退出的代码
		String $returns = uc.userRegister("test19", "111111a" ,"aa11@bb.com" );
		int $uid = Integer.parseInt($returns);
		if($uid <= 0) {
			if($uid == -1) {
				System.out.print("用户名不合法");
			} else if($uid == -2) {
				System.out.print("包含要允许注册的词语");
			} else if($uid == -3) {
				System.out.print("用户名已经存在");
			} else if($uid == -4) {
				System.out.print("Email 格式有误");
			} else if($uid == -5) {
				System.out.print("Email 不允许注册");
			} else if($uid == -6) {
				System.out.print("该 Email 已经被注册");
			} else {
				System.out.print("未定义");
			}
		} else {
			System.out.println("OK:"+$returns);
		}
		

	}

}
