...
for fileName in `ls *Week.dat1`;do
        ...
        commandFile1=${resultDir}w$shortFileName`date +%Y_%m_%d`.cm1
		commandFile2=${resultDir}wWeekRowNum`date +%Y_%m_%d`.cm2
		commandFile3=${resultDir}wWeekRank`date +%Y_%m_%d`.cm3
		...
		redis_value=`date +%Y-%U`:$first_value_num:$last_value_num
