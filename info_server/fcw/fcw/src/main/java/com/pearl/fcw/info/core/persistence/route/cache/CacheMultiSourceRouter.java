...
public class CacheMultiSourceRouter extends CacheSourceRouter implements MultiSourceRouter {
    ...
    @Override
    public <T extends BaseShardingEntity> T queryById(long shardingKey, Integer id, Class<T> clazz) {
    ...
    @Override
    public <T extends BaseShardingEntity> List<T> queryByIds(long shardingKey, Collection<Integer> ids, Class<T> clazz) {

        final List<T> result = new ArrayList<>(ids.size());

        final Collection<Integer> idsNeedQueryFromDB = new HashSet<>();
        ...
        for (Entry<Integer, T> entry : resultFromDB.entrySet()) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <T extends BaseShardingEntity> Map<Integer, T> queryByIdsForMap(long shardingKey, Collection<Integer> ids, Class<T> clazz) {

        final Map<Integer, T> result = new HashMap<>(ids.size());
        ...
        /* 从数据库查找在cache中不存在的对象，并更新进cache */
        Map<Integer, T> resultFromDB = multiSourceRouter.queryByIdsForMap(shardingKey, idsNeedQueryFromDB, clazz);
    ...
    public <T extends BaseShardingEntity> List<T> queryByFK(long shardingKey, long fk, Class<T> clazz, String fieldName) {
        List<Integer> ids = queryIdsByFK(shardingKey, fk, clazz, fieldName);
    ...
    @Override
    public <T extends BaseShardingEntity> List<Integer> queryIdsByFK(long shardingKey, long fk, Class<T> clazz, String fieldName) {
        String key = CacheKeyUtil.getCacheKeyByForeignId(fk, clazz, fieldName);
        List<Integer> ids = cacheClient.safeGet(key);
    ...
    @Override
    public <T extends BaseShardingEntity> List<Integer> queryAllIds(Class<T> clazz) {
