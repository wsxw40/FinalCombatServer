...
public class RedisClient {
	...
	public void set(String key, Object value) {
		consume(jedis -> jedis.set(SafeEncoder.encode(key),
	...
	public void setex(String key, int expire, Object value) {
		consume(jedis -> jedis.setex(SafeEncoder.encode(key), expire,
				serializer.serialize(value)));
	}
	...
	public <T> T safeGet(String key) {
		try {
			...
			logger.warn(
					"error during get of cacheClient, exception is "
							+ e.getMessage() + " key is " + key, e);
			return null;
		}
	}

	public boolean setIfAbsentMillisecond(String key, Object value,
			long expireMillis) {
	...
	public <T> Set<T> zrevrange(String key, long start, long end) {
		...
		return r.stream().<T> map(serializer::deserialize)
	...
	public double zScore(String key, String number) {
		return execute(jedis -> {
			Double score = jedis.zscore(key, number);
			if (score == null) {
				score = 0.0;
			}
			return score;
