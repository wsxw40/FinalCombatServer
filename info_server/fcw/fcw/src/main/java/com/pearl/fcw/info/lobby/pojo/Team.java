...
public class Team extends BasePojo {
	...
	public void setCreateTime(Date createTime) {
		...
		this.createTimeStr = new SimpleDateFormat("yyyy-MM-dd HH:mm")
				.format(createTime);
	}
	...
	public String getLeaderName() {
		...
		for (PlayerTeam pt : this.memberList) {
			if (pt.getJob() == Constants.TEAMJOB.TEAM_CAPTAIN.getValue()) {
	...
	private void caculateTeamRes() {
		Constants.TEAMSPACELEVELCONSTANTS baseTeamSLC = Constants.TEAMSPACELEVELCONSTANTS
				.getTeamSpaceLevelConstants(0);
		Constants.TEAMSPACELEVELCONSTANTS teamSLC = Constants.TEAMSPACELEVELCONSTANTS
		...
		int conRemainSeconds = (conDurSeconds)
				% Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL;
		int orgRemainSeconds = (orgDurSeconds)
				% Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL;

		if (orgDurSeconds >= Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL) {
			increaseOrgNum = (orgDurSeconds)
					/ Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL
					* teamSLC.gettOutPutOrgRes();
		}

		if (conDurSeconds >= Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL) {
			increaseConvertNum = (conDurSeconds / Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL)
					* (teamSLC.gettOrgResConvertNum() + baseTeamSLC
							.gettOrgResConvertNum());
			increaseGiftNum = (conDurSeconds)
					/ Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL
					* (teamSLC.gettGiftRes());

			int canConvertNum = (this.getUnusableResource() + increaseOrgNum)
					/ Constants.TeamSpaceConstants.TEAM_ORG_TO_RES_SCALE;
		...
		newUnusableRes = this.getUnusableResource() + increaseOrgNum
				- increaseConvertNum
				* Constants.TeamSpaceConstants.TEAM_ORG_TO_RES_SCALE;
	...
	public long getLevelupCD(long fullcd) throws ParseException {
		...
		if (lastTeamPlaceLevelUpTime != 0) {
			long past = System.currentTimeMillis() - lastTeamPlaceLevelUpTime
					* 1000;// 过去的毫秒数
			past = past < 0 ? 0 : past;
			result = fullcd - past;
		}
