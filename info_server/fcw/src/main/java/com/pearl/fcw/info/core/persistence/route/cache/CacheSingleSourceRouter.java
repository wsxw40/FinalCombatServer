...
public class CacheSingleSourceRouter extends CacheSourceRouter implements SingleSourceRouter {

    private SingleSourceRouter singleSourceRouter = null;

    public CacheSingleSourceRouter(SingleSourceRouter singleSourceRouter, RedisClient cacheClient, InfoServerLock infoServerLock) {
    ...
    public <T extends BaseEntity> T queryById(long id, Class<T> clazz) {
        ...
        try {
            ...
            log.warn("exception appear when try to get pojoInCache in " + getSchema() + " cacheClient.get(key), key = " + key);
    ...
    public <T extends BaseEntity> List<T> queryByIds(Collection<Long> ids, Class<T> clazz) {
        ...
        final Collection<Long> idsNeedQueryFromDB = new HashSet<Long>();
        ...
        try {
            ...
            log.warn("exception appear when try to get pojoInCaches in " + getSchema() + ", key = " + cacheKeys);
    ...
    public <T extends BaseEntity> Map<Long, T> queryByIdsForMap(Collection<Long> ids, Class<T> clazz) {
        ...
        final Map<Long, T> result = new HashMap<Long, T>(ids.size());
        ...
        /* 从数据库查找在cache中不存在的对象，并更新进cache */
        Map<Long, T> resultFromDB = singleSourceRouter.queryByIdsForMap(idsNeedQueryFromDB, clazz);
        for (Map.Entry<Long, T> entry : resultFromDB.entrySet()) {
    ...
    public <T extends BaseEntity> List<T> queryByFK(long fk, Class<T> clazz, String fieldName) {
        List<Long> ids = queryIdsByFK(fk, clazz, fieldName);
    ...
    public <T extends BaseEntity> List<T> queryAll(Class<T> clazz) {
        List<Long> ids = queryAllIds(clazz);
        return queryByIds(ids, clazz);
    }
