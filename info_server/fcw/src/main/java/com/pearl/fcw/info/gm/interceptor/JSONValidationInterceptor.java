...
/**
 * <p>Serializes validation and action errors into JSON. This interceptor does
 * not
 * perform any validation, so it must follow the 'validation' interceptor on the
 * stack.
 * </p>
 *
 * <p>This stack (defined in struts-default.xml) shows how to use this
 ...
 *
 * <p>If the request has a parameter 'struts.validateOnly' execution will return
 * after
 * validation (action won't be executed).</p>
 *
 * <p>A request parameter named 'struts.enableJSONValidation' must be set to
 * 'true' to
 * use this interceptor</p>
 *
 * <p>If the request has a parameter 'struts.JSONValidation.set.encoding' set to
 * true
 * the character encoding will NOT be set on the response - is needed in portlet
 * environment
