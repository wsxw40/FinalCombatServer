...
import com.pearl.fcw.gm.dao.WMessageDao;
import com.pearl.fcw.gm.pojo.WMessage;
import com.pearl.fcw.gm.pojo.enums.ItemType;
import com.pearl.fcw.gm.pojo.enums.UnitType;
...
public class WMessageService extends DmServiceImpl<WMessage, Integer> {
    ...
    public void addAndSend(WMessage wMessage, String receiverIds, GmUser gmUser) throws Exception {
        ...
        if (UnitType.FOREVER.getValue() != unitType && unit < 1) {
            return;
        }
        //排除物理和逻辑上不存在的系统物品
        sysItemIdList.removeIf(id -> {
            SysItem sysItem = sysItemDao.getSystemItemById(id);//验证发送的系统物品存在
                if (null == sysItem || "Y".equalsIgnoreCase(sysItem.getIsDeleted())) {
                    return true;
                }
                return false;
            });
        ...
        for (Entry<SysItem, Payment> kv : attachmentMap.entrySet()) {
            ...
            if (sysItem.getType() == ItemType.PACKAGE.getValue() && !StringUtil.isEmpty(sysItem.getItems())) {//大礼包
        ...
        wMessage.setItemUnits(sysItemIdList.stream().map(p -> wMessage.getItemUnits()).collect(Collectors.joining(",")));
        ...
        if (!StringUtil.isEmpty(content)) {
            content = CommonUtil.messageFormatI18N(CommonMsg.PRESENT_TO_PLAYER, String.join(",", attachmentMap.keySet().stream().map(SysItem::getDisplayName).collect(Collectors.toList())),
                    DateUtil.formatToFlag(now, "yyyy-MM-dd HH"));
        }
