 ...
 * 生成Smrty4j代码文件
 */
public class CodeGenerateSmarty4jUtil {
    private static Logger logger = LoggerFactory.getLogger(CodeGenerateSmarty4jUtil.class);
    //模板文件夹路径
    private static final String TEMPLATE_DIR_PATH = ConfigurationUtil.TEMPLATE_PATH;
    //生成Smarty4j.java文件路径
    private static final String TARGET_FILE_PATH = ConfigurationUtil.APP_ROOT_PATH + "\\src\\com\\pearl\\fcw\\utils\\Smarty4jUtil.java";
    //生成模板对应的java文件所属文件夹路径
    private static final String TARGET_DIR_PATH = ConfigurationUtil.APP_ROOT_PATH + "\\src\\com\\pearl\\fcw\\utils\\smarty4j\\";
    ...
    public static void generateSmarty4j() throws Exception {
        ...
        for (File f : file.listFiles()) {
            generateSmarty4j(f, new StringBuilder(), "GetQqBuyGoods.st");
        }
        logger.info("Smarty4j generate end.");
    }
    ...
    private static void generateSmarty4j(File file, final StringBuilder sb, String fileStarts) throws Exception {
        if (sb.length() < 1 && null == file) {
            ...
            sb.append(LINE_FEED);
            //类名语句块开始
            sb.append("public class Smarty4jUtil {").append(LINE_FEED);
            //固定类成员logger
            sb.append("private static Logger logger = LoggerFactory.getLogger(Smarty4jUtil.class);").append(LINE_FEED).append(LINE_FEED);
            //固定方法get
            sb.append("public static String get(String template, Context context) throws Exception{").append(LINE_FEED);
            ...
            sb.append("Class<?> clazz = Class.forName(\"com.pearl.fcw.utils.smarty4j.\" + className);").append(LINE_FEED);
            ...
            sb.append("String get(Context context) throws Exception;").append(LINE_FEED);
            ...
            try {
                ...
                logger.info("Smarty4j.java creating is end.");
            }
            ...
            //文件头部导包
            sb.append("package com.pearl.fcw.utils.smarty4j;").append(LINE_FEED).append(LINE_FEED);
            ...
            sb.append("import com.pearl.fcw.utils.Smarty4jUtil.Ctx;").append(LINE_FEED);
            sb.append(LINE_FEED);
            //类名语句块开始
    ...
    private static void generateSmarty4jMethod(File file, final StringBuilder sb) throws Exception {
        ...
        sb.append("public String get").append("(Context context) throws Exception {").append(LINE_FEED);
        //方法体内容
        try {
    ...
    private static void generateSmarty4jSingleLine(final File file, String str, final StringBuilder sb, final List<String> varList, final Map<String, String> includeVarMap) throws Exception {
        ...
        if (m.find()) {
            ...
            sb.append("Context ").append(includeContextVar).append("=new Context();").append(LINE_FEED);
            while (m1.find()) {
                if (m1.group(1).equals("file")) {
                    ...
                    sb.append(includeContextVar).append(".set(\"").append(m1.group(1)).append("\",").append(var).append(");").append(LINE_FEED);
                } else {
                    sb.append(includeContextVar).append(".set(\"").append(m1.group(1)).append("\",").append(m1.group(2)).append(");").append(LINE_FEED);
                }
            }
            //引用其他模板类生成字符串
            if (null != className) {
        ...
        if (PATTERN1.matcher(str).find()) {//for循环语句块开始
            ...
            if (m.find()) {
                ...
                sb.append("else if (").append(var).append(" instanceof String[]) ").append(tmpVar).append("=Stream.of((String[])").append(var).append(").collect(Collectors.toList());")
                        .append(LINE_FEED);
                sb.append("else if (").append(var).append(".getClass().isArray()) ").append(tmpVar).append("=Stream.of(").append(var).append(").collect(Collectors.toList());").append(LINE_FEED);
                sb.append("}").append(LINE_FEED);
