...
public class DailyDiscountBuy implements PlayerBuyWay {
	...
	public int buy() throws Exception {
		...
		//判断是否有资格购买
		int cost=(int)(onePrice.getCost()*getPlayerDiscountByItemId(player.getId(), sysItem.getId(),index));
		payment.setCost(cost - (cost%10)); 
	...
	private float getPlayerDiscountByItemId(int playerId,int sysItemId,int index) throws Exception{
		...
		//判断是否有资格购买
		if((flagNum & (int)Math.pow(2,index)) !=0||Constants.DAILY_DISCOUNTS[0].length<=index){
			throw new BaseException(ExceptionMessage.ERROR_MESSAGE_ALL);
		}
		return Constants.DAILY_DISCOUNTS[findItemIdIndex(sysItemId)][index];
	}
	
	private int findItemIdIndex(int itemId){
		for(int i=0;i<Constants.DAILY_DISCOUNT_SYSITEM_IDS.length;i++){
			if(Constants.DAILY_DISCOUNT_SYSITEM_IDS[i]==itemId){
				return i;
			}
		}
		return 0;
