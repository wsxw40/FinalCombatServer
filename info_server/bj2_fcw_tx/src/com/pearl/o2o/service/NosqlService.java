...
//		}
	 ...
	 * @throws Exception 
	 */
	public void updateQuser(Quser quser) throws Exception{
		String pidkey=NosqlKeyUtil.tx_Quser_key(quser.getId()+"");
		String oidkey=NosqlKeyUtil.tx_Quser_key(quser.getOpenid());
		nosql.set(pidkey, quser.toString());
		nosql.set(oidkey, quser.toString());
	}
	/**
	 * 获得pid对应的Quser
	 * @param playerId
	 ...
	 * @throws Exception
	 */
	public Quser getPidToQuser(int playerId) throws Exception{
		return getPidOrOidToQuser(playerId+"");
	}
	/**
	 * 获得oid对应的Quser
	 * @param playerId
	 ...
	 * @throws Exception
	 */
	public Quser getOidToQuser(String openid) throws Exception{
		return getPidOrOidToQuser(openid);
	}
	private Quser getPidOrOidToQuser(String id) throws Exception{
		String key=NosqlKeyUtil.tx_Quser_key(id);
		String quserStr = nosql.get(key);
		...
		quser.setOpenkey("0");
		quser.setPfkey("0");
		if (quserStr==null) return quser;
		
		String[] split = quserStr.split("\\|");
		if (split.length==4) {
			quser.setId(Integer.valueOf(split[0]));
			quser.setOpenid(split[1]);
			quser.setOpenkey(split[2]);
			quser.setPfkey(split[3]);
			return quser;
		} else {
			//数据有问题应该直接查询
		}
	 ...
	 * @throws Exception 
	 */
	public void updateToken(int playerId, Token token) throws Exception{
		String key=NosqlKeyUtil.tx_Token_Key(playerId);
		nosql.hashSet(key,token.getId(),token.getTime()+"|"+token.getPayitem());
	}
	
	/**
	 * 根据pid跟token_id获得对应的Payitem
	 * @param playerId
	 * @param token
	 * @return
	 * @throws Exception
	 */
	public String getToken(int playerId,String token) throws Exception{
		String key=NosqlKeyUtil.tx_Token_Key(playerId);
		String hashGet = nosql.hashGet(key, token);
		String[] split = hashGet.split("\\|");
		if (split.length==2) {
			return split[1];
		}
		return null;
	...
	/**
	 * 根据pid跟token_id删除对应的token_id
	 * @param playerId
	 * @param token
	 * @return
	 * @throws Exception
	 */
	public void delToken(int playerId,String token_id) throws Exception{
		String key=NosqlKeyUtil.tx_Token_Key(playerId);
		nosql.hashdel(key, token_id);
	}
	 ...
	 * @throws Exception
	 */
	public void neatenToken(int playerId) throws Exception{
		String key=NosqlKeyUtil.tx_Token_Key(playerId);
		Map<String, String> hgetAll = nosql.hgetAll(key);
		if (hgetAll == null) return;
		for (Entry<String, String> kv : hgetAll.entrySet()) {
			String[] split = kv.getValue().split("\\|");
			if (split.length==2) {
				int ts = StringUtil.toInt(split[0]);
				int ts1 = (int) (System.currentTimeMillis()/1000);
				if(ts+3*24*60*30<ts1)
					nosql.hashdel(key, kv.getKey());
			}else {
				nosql.hashdel(key, kv.getKey());
			}
		}
