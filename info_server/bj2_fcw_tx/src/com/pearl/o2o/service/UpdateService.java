...
import com.pearl.o2o.servlet.client.GetQwInfo;
...
//						+ Constants.XUNLEI_LOG_DELIMITER + playerInfo.getXunleiPoint()
	...
	public Integer qw_info_account(int playerId, StageClearPlayerInfo playerInfo,int s_f) throws Exception{
		...
		if(pSum.getWin() + pSum.getFall()>=10){
			...
			pSum.setRankNum((int)rank+1);//最新排名
			//算段位
	...
	private void updatePlayerItemDurablewhileStage(StageClearPlayerInfo playerInfo, int useTotal, double punish) throws Exception {
		...
		for (SingleCharacterInfo ci : characterInfoList) {
			if (ci.getUsedCount() > 0 &&ci.getCharacterId()<10) {
				int cost = 0;
				cost = (int) (punish * (Math.sqrt(Math.abs((playerInfo.getScore()))) * 0.1 * (ci.getUsedCount() * 1.0 / useTotal)));
...
//				int offset=updatePlayerActivity(Constants.ACTION_ACTIVITY.ONLINE_ACTIVITY.getValue(), player.getId(), null, 0, 0, null, null);
				...
				if (isMatch==6&&gameTpye==Constants.GAMETYPE.TEAM.getValue()) {
					...
					Integer qw_score=0;
					if (isMatch == 6 && stageClearInfo.isClear() && 1 == playerInfo.getIsWiner()){ // 匹配模式，房间正常关闭，战斗胜利
						qw_score = qw_info_account(player.getId(),playerInfo,1);
						player.setMatchWin(player.getMatchWin().substring(1) + "1");
					}
					if (isMatch == 6 && stageClearInfo.isClear() && 1 != playerInfo.getIsWiner()){ // 匹配模式，房间正常关闭，战斗失败
						qw_score = qw_info_account(player.getId(),playerInfo,0);
						player.setMatchWin(player.getMatchWin().substring(1) + "0");
					}
					if (isMatch == 6 && !stageClearInfo.isClear()){ // 匹配模式，中途退出VIP的
						qw_score = qw_info_account(player.getId(),playerInfo,-1);
						player.setMatchWin(player.getMatchWin().substring(1) + "0");
					}
					if(qw_score!=null)
				...
				if(isMatch==6&&gameTpye==Constants.GAMETYPE.TEAM.getValue()){//判断是否是匹配团竞模式
					...
					boolean flag_1 = (chipsNum[0] == chipsNum_count[0]
					                   && chipsNum[1] == chipsNum_count[1] 
					                   && chipsNum[2] == chipsNum_count[2]);//已掉落数目等于上限制数
					...
					mcc.set(mate_chip_count_key, day_surplus_time, chipsNum[0]
							+ Constants.DELIMITER + chipsNum[1]
							+ Constants.DELIMITER + chipsNum[2]
							+ Constants.DELIMITER + chipsNum[3]);
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						nosqlService.addXunleiLog("20.1"
					...
					netBarGetChance = -(1000+(chipsNum[0]+chipsNum[3] - aChipNum-aChipNum_lose) * 100
							+ (chipsNum[1] - bChipNum) * 10 + (chipsNum[2] - cChipNum));
					activityGetChance = (1000+(chipsNum[0]+chipsNum[3] - aChipNum-aChipNum_lose) * 100
							+ (chipsNum[1] - bChipNum) * 10 + (chipsNum[2] - cChipNum));//活动  箱子颜色  箱子个数
				}
				//zlm11-27------------------------------------------------------------------------------------------------
				...
				if(gameTpye==Constants.GAMETYPE.HITBOSS2.getValue()&&levelInfo.getIsGm()==10){//判断是否是BOSS末日进化
					...
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						nosqlService.addXunleiLog("19.2"
							+ Constants.XUNLEI_LOG_DELIMITER + playerInfo.getId()//玩家帐号ID
							+ Constants.XUNLEI_LOG_DELIMITER + isVip//玩家VIP等级
							+ Constants.XUNLEI_LOG_DELIMITER + getScore//玩家本局积分
							+ Constants.XUNLEI_LOG_DELIMITER + scms_box_count//算上本局后开启的箱子次数
...
//=================================================================================================================================================================
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			...
			if(isMatch==6&&gameTpye==Constants.GAMETYPE.TEAM.getValue()){//判断是否是匹配团竞模式
				ServiceLocator.nosqlService.addXunleiLog("4.4"
						+ Constants.XUNLEI_LOG_DELIMITER
						+ host.getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER
						+ levelInfo.getDisplayNameCN()
						+ Constants.XUNLEI_LOG_DELIMITER
						+ Constants.GAMETYPE.TEAM_MATCH.getValue()
						+ Constants.XUNLEI_LOG_DELIMITER
	...
	public int everydayGiftWelfare_6275(final Player player)throws Exception{
		...
		if(pItem!=null){
			...
			if(!pItem.getExpireDate().after(new Date())){
				...
				//日志记录，天天礼到期
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("17.5"
						+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
			...
			if(flag){
					...
					if (sysItem != null) {
						...
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
							nosqlService.addXunleiLog("17.4"
								+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
