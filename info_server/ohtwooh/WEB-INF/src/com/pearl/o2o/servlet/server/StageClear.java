...
public class StageClear extends BaseServerServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse res)
		...
		try{
			logger.debug("receive stage clear data rid :" + rid + "  " + stageClearInfo);
			//update player
			//meanwhile, some information of player will be set to playerInfo
			updateService.updatePlayerStageClear(stageClearInfo.getStageClearPlayerInfo(), type, winner);
			String result = Converter.stageClear(stageClearInfo);
			String key = CacheUtil.sStageClear(stageClearInfo.getRid());
			logger.debug(result);
