...
public class StageClearOpenTimeOut extends BaseClientServlet{
	...
	protected String innerService(String... args) {
		...
		try{
			...
			randomBrandIdList = mcc.get(brdListKey);//获得翻牌的东西list
			if(randomBrandIdList==null||(randomBrandIdList.size()<9&&randomBrandIdList.size()<3)){
				logger.warn("StageClearOpenTimeOut/GetBrdIdsFail:\t" + playerId + "\t" + randomBrandIdList);
			}
			mcc.delete(brdListKey);
			totalOpenChance = mcc.get(brandOpenChancekey);//可以打开的次数
			...
			bidList = mcc.get(brandOpenkey);//返回翻牌打开的位子
			if(bidList==null){
				logger.warn("StageClearOpenTimeOut/GetOpenListFail:\t" + playerId);
			}
			mcc.delete(brandOpenkey);		
			Date today = new Date();
			int length = bidList.size();//打开次数
			int randomNum = totalOpenChance - length;//剩余次数
			...
			if(randomNum>0){
				...
				for(int k=0;k<randomBrandIdList.size();k++){
					tmpList.add(k);
				}
			...
			for(int bidx : bidList){
				...
				nosqlService.updatePlayerOpenBrandsNum(playerId,today);//发送？？？
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					if (brd.getItemId()==GrowthMissionConstants.MEDAL_ID) {
					...
					//6、结算界面
					if(brd.getItemId()==GrowthMissionConstants.UPGARDMOD_ID){
						int upgradedNumByPId = getService.getUpgradedNumByPId(playerId);
						XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId,brd.getUnit(),upgradedNumByPId+brd.getUnit(), 6);
					}
				}
