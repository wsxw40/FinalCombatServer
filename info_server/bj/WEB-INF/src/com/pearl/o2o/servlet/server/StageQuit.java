...
public class StageQuit extends BaseServerServlet{
	...
	protected  byte[] innerService(BinaryReader r) throws Exception{
		...
		//System.out.println("stageQuit:"+isMatch);
		//zlm2015-10-9-匹配-结束
...
//		}
		//匹配模式中途退出或直接关闭游戏时的结算
		matchShutAccount(isMatch, playerId);
		if(stageClearInfo.getStageClearPlayerInfoAll().size()!=0){
			if(type!=Constants.GAMETYPE.TEAMZYZDZ.getValue()) {
				updateService.updatePlayerStageQuit(playerId,stageClearInfo,isMatch);
			}
	 ...
	 * @throws Exception
	 */
	private void matchShutAccount(int isMatch, int playerId) throws Exception {
		StageClearPlayerInfo playerInfo = null;
		Player player = null;
		switch (isMatch) {
		case 6:
			playerInfo = new StageClearPlayerInfo();
			playerInfo.setScore(-20);
			updateService.qw_info_account(playerId, playerInfo, -1);
			player = getService.getPlayerById(playerId);
			player.setPvalue(player.getPvalue()+10);
		...
		case 7:
			...
			player.setMvalue(player.getMvalue()+(int)(mkda>=1? 36:-36));
			ServiceLocator.updateService.updatePlayerInfo(player);
			break;
		case 8:
			break;
