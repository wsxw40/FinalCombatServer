	...
	protected byte[] innerService(BinaryReader r) throws Exception {
		try {
			...
			if (type == Constants.GAMETYPE.TEAMZYZDZ.getValue()) {
				...
				int terroristScore = r.readInt();//恐怖分子胜利
				stageClearInfo.setTerroristScore(terroristScore);
				int policeScore = r.readInt();//警察胜利
			...
			for (int i = 0; i < playerArrayLength; i++) {
				...
				if (costReliveCoinNum > 0) {
					...
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						...
						nosqlService.addXunleiLog("18.1" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
								+ 0 + Constants.XUNLEI_LOG_DELIMITER + costReliveCoinNumLog + Constants.XUNLEI_LOG_DELIMITER + (totalReliveCoinNum - costReliveCoinNumLog)
								+ Constants.XUNLEI_LOG_DELIMITER + 6 + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
					}
				}
				/*复活币扣除计算 end***********/
				...
				if (type == GAMETYPE.AGRAVITY2.getValue()) {
					...
					if (Constants.MATCH_TYPE.LEAGUE.getValue() == getService.getServer(serverId).getServerType())
						leagueFlag = true;
					System.out.println("leagueFlag:" + leagueFlag);
					for (int itemIndex = 0; itemIndex < itemsNum.length; itemIndex++) {
						if (mcc.get(mccKeys[itemIndex]) == null || leagueFlag) {
							Integer itemNum = 0;
