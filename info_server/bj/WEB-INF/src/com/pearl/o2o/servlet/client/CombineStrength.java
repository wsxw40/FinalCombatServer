	...
	protected String innerService(String... args) {
		try {
			...
			if (tempPlayerItem.getLevel() >= getService.getMaxStrengthLevel(playerId)) {
				...
				String warn = Converter.warn("1");
				int maxBuffLevel = getService.getMaxBuffLevel(playerId);
				if (tempPlayerItem.getLevel() == (16-1)
						||(tempPlayerItem.getLevel()==(17-1)&&maxBuffLevel==17)
						||(tempPlayerItem.getLevel()==(18-1)&&maxBuffLevel==18)	)
					//玩家等级未达到34级且VIP等级未达到金钻，无法继续强化
					warn = Converter.warn("16");
				else if (tempPlayerItem.getLevel() == (15-1)) 	
					//玩家等级未达到32级且VIP等级未达到银钻，无法继续强化
					warn = Converter.warn("15");
				else if (tempPlayerItem.getLevel() == (14-1)) 	
					//玩家等级未达到30级且VIP等级未达到VIP6，无法继续强化
					warn = Converter.warn("14");
				else if (tempPlayerItem.getLevel() == (13-1)) 	
					//玩家等级未达到28级且VIP等级未达到VIP5，无法继续强化
					warn = Converter.warn("13");
				else if (tempPlayerItem.getLevel() == (12-1)) 	
					//玩家等级未达到25级，无法继续强化
					warn = Converter.warn("12");
				else if (tempPlayerItem.getLevel() == (11-1)) 	
					//玩家等级未达到21级，无法继续强化
					warn = Converter.warn("11");
				else if (tempPlayerItem.getLevel() == (10-1)) 	
					//玩家等级未达到15级，无法继续强化
					warn = Converter.warn("10");
				return warn;
			}
