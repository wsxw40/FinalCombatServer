...
import com.pearl.o2o.schedule.CleanOverdue;
...
import com.pearl.o2o.schedule.LeagueApplyRecordClean;
import com.pearl.o2o.schedule.LeagueWinRateAcculateTask;
import com.pearl.o2o.schedule.PlayerOnlineCountPushTask;
import com.pearl.o2o.schedule.QWZBTopTask;
...
public class InitServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			//每次系统启动的时候，清除过期的数据
			if (Constants.CLEAN_OVERDUE == 1) {//仅仅是一个参数，决定该服务器是否执行
				new CleanOverdue().run();
			}

			//每周二2点刷新team的前一周的抢资源值
			if (Constants.TEAM_FIGHT_RES_PREWEEK == 1) {
				new InitTeamFightResPreWeekTask().run();
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new InitTeamFightResPreWeekTask(), CommonUtil.minutesToNextTuesday(), Constants.TOP_PLAYER_ACTIVITY_MINUTES,
						TimeUnit.MINUTES);
			}
			...
			if (Constants.SWITCH_SEND_TEAM_RES == 1) { //挑战赛结束后发送剩余资源
				...
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new CacZYZDZChallengeWarTask(), CommonUtil.minutesToDate(ServiceLocator.getService.getZYZDZChallengeTime(2).getTime()),
						7 * 24 * 60, TimeUnit.MINUTES);
			}

			//保证只有一台服务器执行该任务
			if (Constants.SWITCH_COMPETEBUYSENDTASK == 1) {//抢拍结束后,定时开始发奖励
				Date t = ServiceLocator.getService.getCompeteBuySendGoodsTime();
				System.out.println("~限时抢购奖励发送时间:" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(t));
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new CompeteBuyOverTask(), CommonUtil.minutesToDate(t), 7 * 24 * 60, TimeUnit.MINUTES);
			...
			if (ConfigurationUtil.SWITCH_ONLINE.getIsOn()) {
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new PlayerOnlineCountPushTask(), 60 * 1000l, 60 * 1000l, TimeUnit.MILLISECONDS);
			}
			...
			//每天2点刷新对枪王的积分，进行控制
			if (Constants.QW_SCOCE_CLEAN == 1) {
				new QWZBTopTask().run();
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new QWZBTopTask(), CommonUtil.millisecondToNextday2(), 24 * 60 * 60 * 1000l, TimeUnit.MILLISECONDS);
				//ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new QWZBTopTask(),1,24*60*60*1000l, TimeUnit.MILLISECONDS);
			}
			//清理出战名单 ,考虑到联赛参加人员名单积分要参考。而出战名单在非报名阶段无法查看。
			//所以改成在报名开始前清理
			//启动是清空 
			//每天2点刷新对枪王的积分，进行控制
			if ("1".equals(ConfigurationUtil.LEAGUE_SWITCH)) {
				new LeagueApplyRecordClean().run();
				ServiceLocator.scheduledExecutorService.scheduleAtFixedRate(new LeagueApplyRecordClean(), CommonUtil.millisecondToNextday2(), 24 * 60 * 60 * 1000l, TimeUnit.MILLISECONDS);
			}
			...
			if (Constants.ADD_GPOINT == 1) {
				...
				// 考虑每天零点统计前一天的数据
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(ServiceLocator.playerGPointPushTask, Constants.CACHE_TIMEOUT_DAY, 24 * 60 * 60 * 1000l, TimeUnit.MILLISECONDS);
			}
			//	每天0点执行计算战队联赛胜负
			if ("1".equals(ConfigurationUtil.LEAGUE_SWITCH)) {
				ServiceLocator.scheduledExecutorService.scheduleAtFixedRate(new LeagueWinRateAcculateTask(), DateUtil.getTimeMillis("23:59:00"), 24 * 60 * 60 * 1000l, TimeUnit.MILLISECONDS);
			}
		} catch (Exception e) {
