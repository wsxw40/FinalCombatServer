...
public class Converter {
	 ...
	 */
	public static String dailyCheckList(String date, int day, List<String> checkList, List<SysItem> items, int today, String myNum, String tomorrow, int isGift, int isShowAward, int freeCheckTimes,
			int checkNum) {
		String value = "";
		try {
			Context ctx = new Context();
	...
	public static String playerInfo(Player player, List news, int top, int mateRank) {
		String value = "";
	...
	public static String fontPlayerInfo(Player player, int emailNum, int teamRequestNum, String erroMsg, int card, PlayerInfo playerInfo, int top, float winRate, String team, int maxLevel,
			int vipLeftMins, int currentyEXPItem) {
		String value = "";
	 ...
	 */
	public static String exchangeSysItemList(int page, int pages, List<SysItem> list, int rank, int chipNum, int reviceCoinNum, int aChipNum, int bChipNum, int cChipNum) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("page", page);
			...
			ctx.set("aChipNum", aChipNum);
			ctx.set("bChipNum", bChipNum);
			ctx.set("cChipNum", cChipNum);
	...
	public static String playerCharacterList(List<Character> list, String selectIds, int size, int openSize, List<Character> bioListAndState, List<SysItem> bioItemList) {
		String value = "";
	...
	public static String playerPack(Character character, String name, Integer matchRank) {
		String value = "";
	...
	public static String stageClear(GeneralStageClearInfo stageClear, Player mvpPlayer, PlayerItem mvpWeapon, int level_close_type) {
		String value = "";
	 ...
	 * @return
	 */
	public static String getBattleFiledDefenseInvitation(Team attackTeam, Player receiver, Team defenseTeam, int serverid, int channelid, int roomid, String roompwd, FightType fightType,
			String config, int canRobRes, int teamSpaceLevel, int atknum) {
		String value = "";
	...
	public static String shootingJoin(int boutNum, int dartleNum, int ammoNum, int needDartleNum, SysItem shootingLook, OnlineAward goldGift, OnlineAward silverGift, List<OnlineAward> commonGifts,
			OnlineAward dartleGift, List<String> dartleTopList, List<TmpPlayerItem> dartleAwardsList) {
		String value = "";
		try {
			Context ctx = new Context();
	...
	public static String shootingLook(int boutNum, int dartleNum, int ammoNum, int lookNum, int needDartleNum, List<OnlineAward> gifts, OnlineAward dartleGift, List<String> dartleTopList,
			List<TmpPlayerItem> dartleAwardsList) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("boutNum", boutNum);
	...
	public static String getModuleStatus(ModuleStatus mission, ModuleStatus dailyMission, ModuleStatus compose, ModuleStatus online, ModuleStatus present, ModuleStatus dailyCheck,
			ModuleStatus freeChange) {
	...
	public static String getMeltingInfo(PlayerMelting playerMelting, int recycle_ratio) {
		String value = "";
		Context ctx = new Context();
		...
		ctx.set("price", 0);
		ctx.set("result", 0);
		ctx.set("recycle_ratio", recycle_ratio);
		try {
	...
	public static String getMeltingResult(MeltingReslut meltingReslut, int recycle_ratio) {
		String value = "";
		Context ctx = new Context();
	...
	public static String getCompeteBuyItems(String timeType, String retTimeStr, String itemNums, String leastNums, String validNums, String mostNums, String myNums, String leftNums) {
		String value = "";
		try {
			Context ctx = new Context();
	...
	public static String GetMatchedTeam(Team defanceTeam, Team attackTeam, TeamLevelInfo teamLevelInfo, boolean outOfCount, boolean outOfStones, int canRobUnuseRes, Player player) {
		...
		try {
			...
			if (defanceTeam != null && !outOfCount) {
				...
				HashMap<String, Integer> playerRes = player.getLatestPlayerRes(attackTeam.getTeamSpaceLevel());
				Context ctx = new Context();
	...
	public static String getPersonalPlace(int fc, int storageType, int curPage, int pages, int resoucesNum, List<PlayerItem> items) {
		String value = "";
	...
	//zlm 2016-01-15 qw s
	public static String getQwInfo(int type, int[] qwInfo, List<String[]> days, Integer page, Integer pageNum, List<String[]> sums, Integer rate) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("type", type);
	...
	//zlm 2016-01-15 qw e
	public static String playerItems(List<PlayerItem> list, int type, int meltingLevel) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("stName", "PlayerItems");
			ctx.set("type", type);
			ctx.set("list", list);
			ctx.set("meltingLevel", meltingLevel);
			return sm.getEncodedBody("PlayerItems.st", ctx);
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
		return value;
	...
	public static String getServerRemark(List<Server> serverList) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("serverList", serverList);
			return sm.getEncodedBody("ServerRemark.st", ctx);
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
		return value;
	 ...
	 * @return
	 */
	public static String LeagueApply(int apply) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("apply", apply);
			String encodedBody = sm.getEncodedBody("LeagueApply.st", ctx);
			return encodedBody;
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
	...
	//      int start = 1;//0报名时间段，1空闲，2比赛时间段
	public static String League(String apply_start, String apply_end, String game_start, String game_end, String remark_1, String[] remark_arr, String teamName, String teambName, int haveTeam,
			int job, int apply, int start, int timeDiff, int timeDiffType, int gameType, int ltmb_v, int isOfficial) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("apply_start", apply_start);
			ctx.set("apply_end", apply_end);
			ctx.set("game_start", game_start);
			ctx.set("game_end", game_end);
			ctx.set("remark_1", remark_1);
			ctx.set("remark_arr", remark_arr);
			ctx.set("teamName", teamName);
			ctx.set("teambName", teambName);
			ctx.set("haveTeam", haveTeam);
			ctx.set("job", job);
			ctx.set("apply", apply);
			ctx.set("start", start);
			ctx.set("timeDiff", timeDiff);
			ctx.set("timeDiffType", timeDiffType);
			ctx.set("gameType", gameType);
			ctx.set("ltmb_v", ltmb_v);
			ctx.set("isOfficial", isOfficial);
			String encodedBody = sm.getEncodedBody("LeagueInfo.st", ctx);
	...
	/**
	 * @param attendMemberAllList
	 * @return
	 */
	public static String LeagueAttendMemberInfo(Map<Integer, ArrayList<LeagueMember>> membersMap) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("pList0", membersMap.get(0));
			ctx.set("pList1", membersMap.get(1));
			ctx.set("pList2", membersMap.get(2));
			ctx.set("pList3", membersMap.get(3));
			ctx.set("pList4", membersMap.get(4));
			ctx.set("pList5", membersMap.get(5));
			String encodedBody = sm.getEncodedBody("LeagueAttendMemberInfo.st", ctx);
			return encodedBody;
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
	 ...
	 * @param entryList 数据
	 * @param mytid 当前站队id
	 * @param myTeamIndex 当前站队排行
	 * @param myTeamScoce 当前站队积分
	 * @return
	 */
	public static String LeagueTeamTop(int page, long pages, List<String> entryList, int mytid, int myTeamIndex, int myTeamScoce) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("page", page);
			...
			ctx.set("list", entryList);
			ctx.set("mytid", mytid);
			ctx.set("myTeamIndex", myTeamIndex);
			ctx.set("myTeamScoce", myTeamScoce);
			String encodedBody = sm.getEncodedBody("LeagueTeanTop.st", ctx);
			return encodedBody;
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
	 ...
	 * @return
	 */
	public static String LeagueMemberTop(int page, int pages, List<String> entryList) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("page", page);
			ctx.set("pages", pages);
			ctx.set("list", entryList);
			String encodedBody = sm.getEncodedBody("LeagueMemberTop.st", ctx);
			return encodedBody;
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
	...
	public static String LeagueGameRecordTop(List<LeagueGameRecord> gameRecords, int myTeamIndex) {
		String value = "";
		try {
			Context ctx = new Context();
			ctx.set("myTeamIndex", myTeamIndex);
			ctx.set("gameRecords", gameRecords);
			String encodedBody = sm.getEncodedBody("LeagueGameRecordTop.st", ctx);
			return encodedBody;
		} catch (Exception e) {
			log.warn("Error in Convertor: ", e);
		}
