...
public class Character extends BaseMappingBean<Character> {
	...
	public boolean checkHasSuitNow() {
		...
		Map<Integer, Integer> map = new HashMap<Integer, Integer>();//套装ID和件数
		//仅仅为了显示套装属性
		for (PlayerItem pi : costumeList) {
			...
			if (si.getNeedTeamPlaceLevel() > 99) {
				...
				if (!map.containsKey(si.getNeedTeamPlaceLevel())) {
					map.put(si.getNeedTeamPlaceLevel(), 0);
				}
				map.put(si.getNeedTeamPlaceLevel(), map.get(si.getNeedTeamPlaceLevel()) + 1);
			}
		}
		for (PlayerItem pi : weaponList) {
			...
			if (wsi.getNeedTeamPlaceLevel() > 99) {
				...
				if (!map.containsKey(wsi.getNeedTeamPlaceLevel())) {
					map.put(wsi.getNeedTeamPlaceLevel(), 0);
				}
				map.put(wsi.getNeedTeamPlaceLevel(), map.get(wsi.getNeedTeamPlaceLevel()) + 1);
			}
		}
		suitId = -1;
		//套装不再限制至少3件服装
		for (int tmpSuitId : map.keySet()) {
			if (map.get(tmpSuitId) >= 4) {
				suitId = tmpSuitId;
				suitNum = map.get(tmpSuitId) - 3;
				return true;
			}
		}
