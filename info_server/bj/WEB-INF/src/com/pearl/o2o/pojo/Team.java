...
public class Team extends BaseMappingBean<Team> {
	...
	private Integer leagueWin = 0;
	private Integer leagueFail = 0;
	...
	private int preweekResAmount;//上周抢夺的资源

	private int predayResAmount;//用于记录本周资源抢夺数量
	...
	private Integer tLastAddResTime; //上次系统赠送战队原石时间

	private long lastTeamPlaceLevelUpTime;

	private int curWeekRobCount;
	 ...
	 * @return
	 */
	public Team addExp(int add, Team team) {
		...
		while (exp >= getTotalExp()) {
			...
			TeamService teamService = (TeamService) ConfigurationUtil.beanFactory.getBean("teamService");
			teamService.giveSuperPower(team);	//战队升级增送能量块
		}
		return this;
	}
	...
	private void caculateTeamRes() {
		...
		final int preConvertSeconds = this.gettResourceBeginTime();
		final int preOrgSeconds = this.gettLastAddResTime();

		final int conDurSeconds = nowSeconds - preConvertSeconds;
		final int orgDurSeconds = nowSeconds - preOrgSeconds;

		int newUsableRes = this.getUsableResource();
		int newUnusableRes = this.getUnusableResource();
		int increaseConvertNum = 0;
		int increaseOrgNum = 0;
		int increaseGiftNum = 0;

		int conRemainSeconds = (conDurSeconds) % Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL;
		int orgRemainSeconds = (orgDurSeconds) % Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL;

		if (orgDurSeconds >= Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL) {
			increaseOrgNum = (orgDurSeconds) / Constants.TeamSpaceConstants.TEAM_ORG_RES_PRODUCT_INTERVAL * teamSLC.gettOutPutOrgRes();
		}

		if (conDurSeconds >= Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL) {
			increaseConvertNum = (conDurSeconds / Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL) * (teamSLC.gettOrgResConvertNum() + baseTeamSLC.gettOrgResConvertNum());
			increaseGiftNum = (conDurSeconds) / Constants.TeamSpaceConstants.TEAM_ORG_RES_CONVERT_INTERVAL * (teamSLC.gettGiftRes());

			int canConvertNum = (this.getUnusableResource() + increaseOrgNum) / Constants.TeamSpaceConstants.TEAM_ORG_TO_RES_SCALE;
			if (canConvertNum < increaseConvertNum) {
				increaseConvertNum = canConvertNum;
			}
		}

		newUsableRes += increaseGiftNum + increaseConvertNum;

		newUnusableRes = this.getUnusableResource() + increaseOrgNum - increaseConvertNum * Constants.TeamSpaceConstants.TEAM_ORG_TO_RES_SCALE;

		if (newUnusableRes < 0) {
			newUnusableRes = 0;
		} else if (newUnusableRes > teamSLC.gettMaxOrgRes()) {
			newUnusableRes = teamSLC.gettMaxOrgRes();
		}
		if (newUsableRes < 0) {
			newUsableRes = 0;
		} else if (newUsableRes > teamSLC.gettMaxRes()) {
			newUsableRes = teamSLC.gettMaxRes();
		}

		this.setUsableResource(newUsableRes);
		this.setUnusableResource(newUnusableRes);
		this.settResourceBeginTime(nowSeconds - conRemainSeconds);
		this.settLastAddResTime(nowSeconds - orgRemainSeconds);
		//		}
	}
	...
	public int getCurWeekRobCount() {
		return curWeekRobCount;
	}

	public void setCurWeekRobCount(int curWeekRobCount) {
	...
	public Integer gettLastAddResTime() {
		return tLastAddResTime;
	}

	public void settLastAddResTime(Integer tLastAddResTime) {
		this.tLastAddResTime = tLastAddResTime;
	}
	...
	public Integer getLeagueScore() {
		return leagueScore;
	}

	public void setLeagueScore(Integer leagueScore) {
	...
	public Integer getLeagueFail() {
		return leagueFail;
	}

	public void setLeagueFail(Integer leagueFail) {
		this.leagueFail = leagueFail;
	}
