...
public class SysActivity extends BaseMappingBean<SysActivity> {
	...
	public void initNeedAward(){
		if(action == 1 || action == 2 || action == 3 || action==4  || action == 9 || action == 11 
				|| action == 12 || action == 13 || action == 14 || action == 15|| action == 16|| action == 17
				|| action == 22 || action == 23 || action == 25 || action == 26 || action == 31) {
			needAward = 0;
		}
	}
	...
	private  String messageFormatI18N(String regx,String pref,int num,Object... args){//code style |
		regx = pref +regx + "^" + num+"^";
		for(Object obj :args){
			regx+="\\\""+obj+"\\\"^";
		}
	...
	public void initActivity() throws Exception{
		...
		case 6:
			name=messageFormatI18N(""+this.action,"<AC",3,startEndTargetNum);
			break;
		case 7:
			name=messageFormatI18N(""+this.action,"<AC",3,startEndTargetNum);
			break;
		...
		case 11:
			if(value==0&&targetNum==24){
				name=messageFormatI18N(""+this.action*10, "<AC", 2, startEndTime);
			}else{
				Object[] objs = new Object[]{CommonUtil.dateFormat.format(startTime), CommonUtil.dateFormat.format(endTime), 
						value ,  targetNum };
			...
			break;
		case 12:
			name=messageFormatI18N(""+this.action,"<AC",4,startEndValueTargetNum);
			break;
		case 13:
			name=messageFormatI18N(""+this.action,"<AC",2,startEndTime);
			break;
		...
		case 18:
			...
			if(this.getBackup().equals("Y")){
				name=messageFormatI18N(""+this.action*10,"<AC",3,startEndTargetNum);
			}else{
				name=messageFormatI18N(""+this.action,"<AC",3,startEndTargetNum);
			}
		...
		case 23:
			...
			break;	
		case 24:
			name=messageFormatI18N(""+this.action,"<AC",4,startEndTargetNum);
			break;
		case 25:
			if(value==0&&targetNum==24){
				name=messageFormatI18N(""+this.action*10, "<AC", 2, startEndTime);
		...
		case 27: //28 需要占用，任务内 每日与非每日需要有两段文字
			Object[] objs = new Object[]{CommonUtil.dateFormat.format(startTime), CommonUtil.dateFormat.format(endTime), value,"<SN"+this.items+"^0>"};
			name=messageFormatI18N(""+(this.action+(this.backup!=null&&this.backup.toLowerCase().equals("y")?1:0)),"<AC",4,objs);
			break;	 
		case 31: //31
			Object[] objs_28 = new Object[] {
				CommonUtil.dateFormat.format(startTime),
				CommonUtil.dateFormat.format(endTime), value, targetNum };
			name = messageFormatI18N("" + this.action, "<AC", 4, objs_28);
			break;	 
		default:
			break;
		}
	...
	public void setSysItem(SysItem sysItem) {
		...
		this.itemsStr = sysItem.getDisplayNameCN();
	}
