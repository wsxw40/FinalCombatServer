...
public class CreateService extends BaseService {
	...
	static Logger logger = LoggerFactory.getLogger(CreateService.class.getName());
	...
	public void createTeamByItem(Integer playerId, String name, String description, String logo, String province, String city, String rank, int playerItemId) throws Exception {
		...
		if (playerItemId > 0) {
			playerItem = getService.getPlayerItemByItemId(playerId, Constants.DEFAULT_ITEM_TYPE, playerItemId);
	...
	public int buyPlayerItemList(StringBuffer check, int playerId, List<String[]> playeritems, int packId, int characterId) throws Exception {
		...
		for (int i = 0; i < sysItemList.size(); i++) {
			...
			if (!CommonUtil.isZYZDZBuff(sysItem)// 不对资源争夺战的物品进行判断
					&& (Constants.BOOLEAN_YES.equals(sysItem.getIsDeleted()) || (payment != null && payment.getIsShow() == 0)))
	...
	public int buyPlayerItem(StringBuffer check, String userName, Player player, SysItem sysItem, Payment payment, String isGift, int useIt, final Player... p) throws Exception {
		...
		if (Constants.GP_PAY == payTpye) {
			...
			if (cr < cost) {
				...
				ServiceLocator.updateService.addPlayerTrack(playerId, "", 0, 0, cost, DateUtil.getDf().format(new Date()), "2" + Constants.XUNLEI_LOG_DELIMITER + sysItem.getId()
			...
			if (chipNum < cost) {
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER + 0
							+ Constants.XUNLEI_LOG_DELIMITER + cost + Constants.XUNLEI_LOG_DELIMITER + leftMoney + Constants.XUNLEI_LOG_DELIMITER + 999 + Constants.XUNLEI_LOG_DELIMITER
					...
					if (sysItem.getId() == GrowthMissionConstants.UPGARDMOD_ID) {
						...
						XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId, payment.getUnit(), upgradedNumByPId + payment.getUnit(), 999);
					}
				}
		...
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			nosqlService.addXunleiLog("1.2" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
					+ sysItem.getId() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + (payment.getUnitType() == 1 ? payment.getUnit() : 1)
					+ Constants.XUNLEI_LOG_DELIMITER + payment.getPayType() + Constants.XUNLEI_LOG_DELIMITER + payment.getCost() + Constants.XUNLEI_LOG_DELIMITER
					+ (payment.getUnitType() == 2 ? payment.getUnit() : "") + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER
					+ frontPay + Constants.XUNLEI_LOG_DELIMITER + leftMoney + Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + "" + Constants.XUNLEI_LOG_DELIMITER + ""
			...
			if (sysItem.getId() == GrowthMissionConstants.RELIVE_COIN) {
				...
				nosqlService.addXunleiLog("18.1" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER + 1
						+ Constants.XUNLEI_LOG_DELIMITER + payment.getUnit() + Constants.XUNLEI_LOG_DELIMITER + reliveCoinNum + Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER
						+ CommonUtil.simpleDateFormat.format(new Date()));
			}
			//17、商店购买 
			if (sysItem.getId() == GrowthMissionConstants.UPGARDMOD_ID) {
				int upgradedNumByPId = getService.getUpgradedNumByPId(playerId);
				XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId - payment.getUnit(), payment.getUnit(), upgradedNumByPId, 17);
			}
		}
		return playerItemId;
	}
	...
	public int buyTeamItem(StringBuffer check, String userName, Player player, SysItem sysItem, Payment payment, String isGift, int useIt, final Player... p) throws Exception {
		...
		if (Constants.CR_PAY == payTpye) {
			if (cr < cost) {
				...
				ServiceLocator.updateService.addPlayerTrack(player.getId(), "", 0, 0, payment.getCost(), DateUtil.getDf().format(new Date()), "2" + Constants.XUNLEI_LOG_DELIMITER + sysItem.getId()
						+ Constants.XUNLEI_LOG_DELIMITER + payment.getUnitType() + Constants.XUNLEI_LOG_DELIMITER + payment.getUnit());
		...
		if (!Constants.BOOLEAN_YES.equals(isGift)) {
			nosqlService.publishEvent(new BuyItemEvent(System.currentTimeMillis() / 1000, player.getId(), player.getName(), sysItem));
			if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
				nosqlService.addXunleiLog("1.2" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
						+ sysItem.getId() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + (payment.getUnitType() == 1 ? payment.getUnit() : 1)
						+ Constants.XUNLEI_LOG_DELIMITER + payment.getPayType() + Constants.XUNLEI_LOG_DELIMITER + cost + Constants.XUNLEI_LOG_DELIMITER
						+ (payment.getUnitType() == 2 ? payment.getUnit() : "") + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER
						+ frontPay + Constants.XUNLEI_LOG_DELIMITER + leftMoney + Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + "" + Constants.XUNLEI_LOG_DELIMITER + ""
						+ Constants.XUNLEI_LOG_DELIMITER + player.getRank());
			...
			if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
				nosqlService.addXunleiLog("1.2" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
						...
						+ Constants.XUNLEI_LOG_DELIMITER + payment.getPayType() + Constants.XUNLEI_LOG_DELIMITER + cost + Constants.XUNLEI_LOG_DELIMITER
						+ (payment.getUnitType() == 2 ? payment.getUnit() : "") + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER
						+ frontPay + Constants.XUNLEI_LOG_DELIMITER + leftMoney + Constants.XUNLEI_LOG_DELIMITER + 3 + Constants.XUNLEI_LOG_DELIMITER + p[0].getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER + p[0].getName() + Constants.XUNLEI_LOG_DELIMITER + player.getRank());

			}
			payLog.setPayUse(3);
		}
		{
			infoLogger.log(LogServerMessage.secondLog.name(), Level.INFO_INT, LogUtils.JoinerByTab.join("5.1", player.getUserName(), CommonUtil.simpleDateFormat.format(new Date()), cost,
					payment.getPayType(), sysItem.getDisplayNameCN(), player.getName(), sysItem.getId(), payment.getUnitType(), payment.getUnit(), leftMoney));
		...
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			...
			nosqlService.addXunleiLog("1.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
					+ sysItem.getId() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + (payment.getUnitType() == 2 ? payment.getUnit() : "")
					+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER);
		}
		...
		final Runnable writePayLog = new Runnable() {
			...
			public void run() {
				...
				String message = String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", payLog.getPlayerId(), CommonUtil.simpleDateFormat.format(new Date()), payLog.getPayAmount(),
	...
	public int sendItem(SysItem sysItem, Player player, Payment payment, String isGift, String isBind, String isDefault) throws Exception {
		...
		//zlm2016-09-07-C币购买武器装备，由原先的“可赠送”改为“不可赠送”-开始-------------------------------------
		if (payment != null && payment.getPayType() == Constants.GP_PAY) {
		...
		isBind = sysItem.getProvisional_item_flag() ? Constants.BOOLEAN_YES : isBind;
		//zlm2016-09-07-C币购买武器装备，由原先的“可赠送”改为“不可赠送”-结束-------------------------------------
		if (isDefault == null) {
		...
		//vip专属名片 直接使用
		if (sysItem.getType() == Constants.DEFAULT_ITEM_TYPE && sysItem.getSubType() == 2 && sysItem.getIId() == 1 && sysItem.getIBuffId() == Constants.DEFAULT_CARD_BUFF_ID && sysItem.getIsVip() > 0) {
			useItemById(player, Constants.DEFAULT_ITEM_TYPE, playerItemId, null, 0, 0);
		}
		...
		if (sysItem.getProvisional_item_flag()) {
			...
			nosqlService.getNosql().expire(key, 40 * 24 * 60 * 60);
		}
		//zlm2015-5-7-限时装备-结束-------------------------------------
		return playerItemId;
	}
	...
	private boolean needToPlusPersonalItem(SysItem sysItem, String isGift) {
		...
		if (isGift.equals("N")
				&& (sysItem.getType() == Constants.DEFAULT_MATERIAL_TYPE || sysItem.getType() == Constants.DEFAULT_OPEN_TYPE || sysItem.getSubType() == Constants.DEFAULT_ITEM_SUBTYPE.COST.getValue())) {
	...
	public int sendItemWithoutPushCmd(SysItem sysItem, Player player, Payment payment, String isGift, String isBind, String isDefault) throws Exception {
		...
		List<PlayerItem> piList = null;
		if (isGift.equals("N")
				&& (sysItem.getType() == Constants.DEFAULT_MATERIAL_TYPE || sysItem.getType() == Constants.DEFAULT_OPEN_TYPE || sysItem.getSubType() == Constants.DEFAULT_ITEM_SUBTYPE.COST.getValue())) {//materials & open type must overlap
			piList = getService.getPlayerItemListByItemId(player.getId(), sysItem.getId(), sysItem.getType());
			if (piList.isEmpty()) {
				playerItemId = playerItemDao.createPlayItem(player.getId(), sysItem, unit, unitType, isGift, isBind, isDefault);
			} else {
	...
	public void msgXLB(int serverId, String msg, String name) {
		try {
	...
	public String useItemById(Player player, int type, PlayerItem playerItem, String message, int channelId, int serverId) throws Exception {
		...
		if (si.getIId() == 1) {
			...
			if (serverId != 0 && si.getIId() == 3) {// 大喇叭
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					ServiceLocator.nosqlService.addXunleiLog("1.3" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + serverId + Constants.XUNLEI_LOG_DELIMITER + message + Constants.XUNLEI_LOG_DELIMITER + si.getId() + Constants.XUNLEI_LOG_DELIMITER
							+ "大喇叭" + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
				}
				//成长任务22：使用大喇叭
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					ServiceLocator.nosqlService.addXunleiLog("1.3" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + serverId + Constants.XUNLEI_LOG_DELIMITER + message + Constants.XUNLEI_LOG_DELIMITER + si.getId() + Constants.XUNLEI_LOG_DELIMITER
							+ "小喇叭" + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
				}
			} else {
		...
		} else if (si.getIId() == 5 || si.getIId() == 11 || si.getIId() == 12 || si.getIId() == 13) {// 各种清零卡
			int iid = si.getIId();
			if (5 == iid) {// 战绩清零卡
				player.setGWin(0);
				player.setWinRate(0);
			...
			if (!KeywordFilterUtil.isLegalInput(message) || StringUtil.filter(message)) {
				isError = true;
					...
					public void run() {
						try {
							...
							for (PlayerTeam pt : ptList) {
								...
								messageService.sendSystemMail(getService.getPlayerById(pt.getPlayerId()), CommonMsg.TEAM_RENAME_MAIL_SUBJECT,
										CommonUtil.messageFormatI18N(CommonMsg.TEAM_RENAME_MAIL_CONTENT, new Object[] { oldTeamName, newTeamName }));
							}
						} catch (Exception e) {
			...
			for (OnlineAward oa : randomAwards) {
				...
				if (sysItem != null) {
					...
					int reliveCoinNum = getService.getReliveCoinNumByPlayerId(playerId);
					if (si.getIId() == Constants.SPECIAL_ITEM_IIDS.MYSTIC_BAG.getValue()) {
						logger.info("UseBoss2Package/Award:\t" + playerId + "\t" + si.getId() + "\t" + si.getDisplayNameCN() + "\t" + sysItem.getDisplayNameCN() + "\t" + oa.getUnit() + "\t"
								+ oa.getUnitType());
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn() && oa.getItemId() == GrowthMissionConstants.MEDAL_ID) {
							nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
									+ Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + oa.getUnit() + Constants.XUNLEI_LOG_DELIMITER + chipNum + Constants.XUNLEI_LOG_DELIMITER
									+ 9 + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						}
						...
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn() && oa.getItemId() == GrowthMissionConstants.MEDAL_ID) {
							nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
									+ Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + oa.getUnit() + Constants.XUNLEI_LOG_DELIMITER + chipNum + Constants.XUNLEI_LOG_DELIMITER
									+ 10 + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						}
						//						int totalRare = sysItem.getRareLevel();
						//						if(oa.getUnitType() == Constants.NUM_ONE){
						//							totalRare *= oa.getUnit();
					...
					if (item.getIId() == Constants.SPECIAL_ITEM_IIDS.RELIVE_COIN.getValue()) {
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn() && oa.getItemId() == GrowthMissionConstants.RELIVE_COIN) {
							nosqlService.addXunleiLog("18.1" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
									+ Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + oa.getUnit() + Constants.XUNLEI_LOG_DELIMITER + reliveCoinNum
									+ Constants.XUNLEI_LOG_DELIMITER + 4 + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						}
					}
				} else {
		...
		//			updateItemQuantity(playerItem);
			...
			for (int i = 0; i < awardList.length; i++) {
				...
				if (item == null) {
					...
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						if (sysItem.getId() == GrowthMissionConstants.MEDAL_ID) {
							int chipNum = getService.getMedolNumByPlayerId(playerId);
							nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
									+ Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + unit + Constants.XUNLEI_LOG_DELIMITER + chipNum + Constants.XUNLEI_LOG_DELIMITER + 3
									+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						}
						//3、进阶礼包
						if (sysItem.getId() == GrowthMissionConstants.UPGARDMOD_ID) {
							int upgradedNumByPId = getService.getUpgradedNumByPId(playerId);
							XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId, unit, upgradedNumByPId + unit, 3);
						}
					}
				}
			}
		...
		} else if ((si.getIId() >= Constants.SPECIAL_ITEM_IIDS.SUPER_POWER1.getValue() && si.getIId() <= Constants.SPECIAL_ITEM_IIDS.SUPER_POWER9.getValue())
				|| si.getIId() == Constants.SPECIAL_ITEM_IIDS.SUPER_POWER10.getValue()) {	//超级能量块

			Team team = getService.getTeamByPlayer(player);
			int pre = team.getExp();
			TeamUtils.updateTeamExp(team.getId(), Integer.parseInt(si.getIValue()));	//增加相对应的战队经验
			updateItemQuantity(playerItem);
			logger.info("UseSuperPower/Award:\t" + playerId + "\t" + si.getIId() + "\t" + si.getUnit() + "\t" + si.getUnitType() + "\t team:" + team.getId() + "\t " + team.getName() + "\t " + pre
					+ "\t" + team.getExp());
			...
			for (OnlineAward oa : randomAwards) {
				...
				if (sysItem != null) {
					...
					item2.setUnit(oa.getUnit());
					returnList.add(item2);
					logger.info("UseAGRAVITYBOXITEM/Award:\t" + playerId + "\t" + si.getId() + "\t" + si.getDisplayNameCN() + "\t" + sysItem.getDisplayNameCN() + "\t" + oa.getUnit() + "\t"
							+ oa.getUnitType());
				} else {
					logger.warn("OnlineAward id:" + oa.getId() + " relating SysItem id:" + oa.getItemId() + " is null");
				}
	...
	public void choicePlayerItem(int playerId, int playerItemId, int itemId) throws Exception {
		...
		//zlm2015-5-7-限时装备-开始   里面物品有时间限制的多选一礼包送东西 ，默认有3天 
			...
			//			int[] array=Constants.SXZB_CHOICE_BOX[-1];
			List<OnlineAward> onlineAwards = getService.getOnlineAwardByTypeAndLevel(Constants.SPECIAL_ITEM_IIDS.XSZB_CHOICE_BOX.getValue(), value);
			//hard code,ugly
			//仅仅为了抢购宝箱可配置
			//			for(int i:array){
			//			if(i==itemId){
			...
			//			}
			//		}
			for (OnlineAward onlineAward : onlineAwards) {
				if (onlineAward.getItemId() == itemId) {
					Payment payment = new Payment();
					payment.setUnit(onlineAward.getUnit());//3天 
					payment.setUnitType(onlineAward.getUnitType());//天数
					awardToPlayer(getService.getPlayerById(playerId), getService.getSysItemByItemId(onlineAward.getItemId()), payment, null, Constants.BOOLEAN_YES);
					updateItemQuantity(pi);
					isContain = true;
					break;
				}
		...
		for (int i = 0; i < Constants.GREEN_GR_AWARD[giftType].length; i++) {
			...
			if (levelGR[0] == si.getId()) {
				...
				for (int j = 0; j < awardList.length; j++) {
					...
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						if (itemId == GrowthMissionConstants.MEDAL_ID) {
							int chipNum = getService.getMedolNumByPlayerId(player.getId());
							nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
									+ Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER + unit + Constants.XUNLEI_LOG_DELIMITER + chipNum + Constants.XUNLEI_LOG_DELIMITER + 2
									+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						}
						//2、新手成长礼包
						if (itemId == GrowthMissionConstants.UPGARDMOD_ID) {
							int upgradedNumByPId = getService.getUpgradedNumByPId(player.getId());
							XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId, item2.getUnit(), upgradedNumByPId + item2.getUnit(), 2);
						}
					}
	...
	public void useBuffItem(PlayerItem playerItem, Player player) throws Exception {
		...
		PlayerBuff buff = playerBuffDao.fuzzyGetPlayerBuffById(playerItem.getPlayerId(), playerBuff.getBuffId());
		if (buff != null) {
	...
	public void firstUse(PlayerBuff playerBuff, Player player, PlayerItem pItem, PlayerItem iCurrenty) {
		...
		if (playerBuff.getBuffId() == Constants.DEFAULT_ON_CARD_BUFF || playerBuff.getBuffId() == Constants.DEFAULT_EVERY_DAY_GIFT_BUFF
			...
			try {
				...
				if (pItem != null) {	//不为空则证明，该BUFF正在使用，二次叠加时间，但不赠送FC点
					...
					case Constants.DEFAULT_EVERY_DAY_GIFT_BUFF:	//天天礼 首次使用赠送VIP
						...
						nosqlService.addXunleiLog("17.3" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getId() + Constants.XUNLEI_LOG_DELIMITER
								...
								+ sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + expireDay + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						break;
					case Constants.NWE_YEAR_EVERY_DAY_GIFT_BUFF:	//新春天天礼 首次使用赠送VIP
						...
						nosqlService.addXunleiLog("17.6" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getId() + Constants.XUNLEI_LOG_DELIMITER
								+ player.getName() + Constants.XUNLEI_LOG_DELIMITER + player.getRank() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getId() + Constants.XUNLEI_LOG_DELIMITER
								+ sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + expireDay + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
						break;
					default:
	 ...
	 * @throws Exception
	 */
	public void useTeamItem(PlayerItem playerItem, int playerId) throws Exception {
		updateItemQuantity(playerItem);
		// playerItemDao.updatePlayerItem(userId, playerId, playerItem.getId(),
		// "QUANTITY", playerItem.getQuantity()-1);
	}

	public void useUpgradeTeamItem(PlayerItem playerItem, int playerId, int teamId) throws Exception {
	...
	public void detachMessageItem(int playerId, int messageId, int action) throws Exception {
		...
		if (Constants.BOOLEAN_YES.equals(message.getHaveAttached())) {
			...
			if (message.getSenderId() != 0) {
				item = getService.getPlayerItemById(sender.getId(), message.getPlayerItemId());
			} else {
				item = getService.getPlayerItemById(playerId, message.getPlayerItemId());
			}
			...
			logger.debug("get gift use : " + (System.currentTimeMillis() - start) + "ms");
			start = System.currentTimeMillis();
			if (si.getType() == Constants.DEFAULT_PACKAGE_TYPE) {
				...
				//					SysItem sysItem = getService.getSysItemByItemId(si.getId());
					...
					logger.debug("get big gift package use : " + (System.currentTimeMillis() - start) + "ms");
					start = System.currentTimeMillis();
					messageDao.updateMessageContent(CommonUtil.messageFormatI18N(CommonMsg.GIFT_EAMIL_ACCEPT, message.getContent()), sysId, unit, unitType, messageId);
					logger.debug("update mail content use : " + (System.currentTimeMillis() - start) + "ms");
					start = System.currentTimeMillis();
				}
				...
				messageDao.deleteMessageItem(message.getId());
				logger.debug("delete detache item use : " + (System.currentTimeMillis() - start) + "ms");
				start = System.currentTimeMillis();
			}
			//附件物品数量大于仓库限制后，整理仓库
	...
	// ===============================================================================
	public Player createAddXunleiOrderLog(XunleiOrderLog xunleiPayLog, int playerId, StringBuffer checkSuccess) throws Exception {
	...
	public void createReduceXunleiOrderLog(XunleiOrderLog xunleiPayLog, int playerId, PlayerInfo playerInfo) throws Exception {
		xunleiOrderLogDao.createXunleiOrderLog(xunleiPayLog);
	...
	//	}	
	public int createPlayerMission(PlayerMission playerMission) throws Exception {
	...
	public int buyPlayerItemByNewPayment(StringBuffer check, SysItem sysItem, Player player, Payment payment) throws Exception {
		...
		if (tempPayment.getPayType() == 1) {//C币
			...
			ServiceLocator.updateService.updatePlayerGrowthMission(player, GrowthMissionType.FIRST_PURCHASE);
			ServiceLocator.updateService.addPlayerTrack(player.getId(), "", 0, 0, payment.getCost() * unit, DateUtil.getDf().format(new Date()), "2" + Constants.XUNLEI_LOG_DELIMITER + sysItem.getId()
					+ Constants.XUNLEI_LOG_DELIMITER + payment.getUnitType() + Constants.XUNLEI_LOG_DELIMITER + payment.getUnit());
		}

		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			nosqlService.addXunleiLog("7.3" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
					+ tempPayment.getPayType() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getId() + Constants.XUNLEI_LOG_DELIMITER + sysItem.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER
					+ (payment.getUnitType() == 1 ? payment.getUnit() : 1) + Constants.XUNLEI_LOG_DELIMITER + unit * payment.getCost() + Constants.XUNLEI_LOG_DELIMITER
					+ CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER + frontPay + Constants.XUNLEI_LOG_DELIMITER + leftMoney);
		}
		{
			infoLogger.log(
					LogServerMessage.secondLog.name(),
					Level.INFO_INT,
					LogUtils.JoinerByTab.join("5.1", player.getUserName(), CommonUtil.simpleDateFormat.format(new Date()), ConfigurationUtil.XUNLEI_SERVER_IP, unit * payment.getCost(),
							payment.getPayType(), sysItem.getDisplayNameCN(), player.getName(), sysItem.getId(), payment.getUnitType(), payment.getUnit(), leftMoney));
		}
	...
	public String dailyCheckPay(int playerId, int type, Calendar checkDate, Calendar today) throws Exception {
		...
		if (!nosqlService.isPlayerCheck(playerId, dateStr)) {//判断是否签过
...

			{
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("7.2" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER + 2
							+ Constants.XUNLEI_LOG_DELIMITER + type + Constants.XUNLEI_LOG_DELIMITER + cost + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date())
							+ Constants.XUNLEI_LOG_DELIMITER + cr + Constants.XUNLEI_LOG_DELIMITER + leftMoney);
				}
			}
	...
	public void sendSystemMail(final Player player, final String title, final String content, String itemsId) {
		...
		Runnable sendMailToPlayer = new Runnable() {
			...
			public void run() {
				try {
					messageService.sendSystemMailWithSysItemInContent(player, title, content, temp);
				} catch (Exception e) {
					logger.warn("error happened in sending mail to player");
				}
			};
	...
	void awardExpToPlayer(Player player, int exp) throws Exception {
		...
		if (newLevel > player.getRank()) {
			...
			if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
				ServiceLocator.nosqlService.addXunleiLog("4.5" + Constants.XUNLEI_LOG_DELIMITER + "任务获得" + Constants.XUNLEI_LOG_DELIMITER + 999 + Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()) + Constants.XUNLEI_LOG_DELIMITER
						+ oldLevel + Constants.XUNLEI_LOG_DELIMITER + newLevel);
			}
			soClient.updateCharacterInfo(player, player.getTeam(), player.getRank());
	...
	public void awardGrowthMission(Player player, int cal, int exp, List<AwardItemVo> awardItemVos, String missionName) throws Exception {
		...
		if (null != awardItemVos) {
			for (AwardItemVo entry : awardItemVos) {
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					if (gift.getId() == GrowthMissionConstants.MEDAL_ID) {
						int chipNum = getService.getMedolNumByPlayerId(player.getId());
						nosqlService.addXunleiLog("7.4" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
								+ 1 + Constants.XUNLEI_LOG_DELIMITER + num + Constants.XUNLEI_LOG_DELIMITER + chipNum + Constants.XUNLEI_LOG_DELIMITER + 1 + Constants.XUNLEI_LOG_DELIMITER
								+ CommonUtil.simpleDateFormat.format(new Date()));
					}
					//1、新手成长任务
					if (gift.getId() == GrowthMissionConstants.UPGARDMOD_ID) {
						int upgradedNumByPId = getService.getUpgradedNumByPId(player.getId());
						XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId - num, num, upgradedNumByPId, 1);
					}
				}
			}
		}
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			nosqlService.addXunleiLog("9.2" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName() + Constants.XUNLEI_LOG_DELIMITER
					+ player.getRank() + Constants.XUNLEI_LOG_DELIMITER + 2 + Constants.XUNLEI_LOG_DELIMITER + missionName + Constants.XUNLEI_LOG_DELIMITER + medalNum + Constants.XUNLEI_LOG_DELIMITER
					+ exp + Constants.XUNLEI_LOG_DELIMITER + cal + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
		}
		if (exp > 0) {
	...
	public OnlineAward completePinTu(int playerId) throws Exception {
		...
		if (si != null) {
			...
			soClient.proxyBroadCast(
					...
					CommonUtil.messageFormatI18N(CommonMsg.PINTU_CMT_SYS,
							new Object[] { GunProperty.RED + "@!" + getService.getPlayerById(playerId).getName() + "@!" + GunProperty.RED + "@!" + player.getId(), color + "@!" + si.getDisplayName(),
									misticOa.getUnit() }));
	...
	public void createItemForVipLevelUp(Player player, int vipRank) throws Exception {
		...
		if (vipCard != null) {
			...
			if (vipCardBuff == null || vipCardBuff.getItemId() != vipCard.getId()) {
				sendItem(vipCard, player, new Payment(30, 2), Constants.BOOLEAN_NO, Constants.BOOLEAN_YES, Constants.BOOLEAN_NO);
			}
		}
		...
		if (vipStrengthItem != null) {
			...
			if (null == playerBuff || playerBuff.getItemId() != vipStrengthItem.getId()) {
				int playerItemId = sendItem(vipStrengthItem, player, new Payment(0, 0), Constants.BOOLEAN_NO, Constants.BOOLEAN_YES, Constants.BOOLEAN_NO);
				PlayerItem playerItem = getService.getPlayerItemByItemId(player.getId(), vipStrengthItem.getType(), playerItemId);
	...
	public boolean createChallengeBattle(Player player, TOPTeam topTeam) throws Exception {
		...
		if (leftMoney >= 0) {//有资格(钱)进行挑战	
			//尝试进行挑战
			//key,时间1，开始时间，时间2
			boolean startSuc = mcc.add(challengeKey, Constants.TeamSpaceConstants.BATTLEFIELD_CACHE_CHALLENGE_LIFE_TIMEOUT, System.currentTimeMillis(),
					Constants.TeamSpaceConstants.BATTLEFIELD_CACHE_CHALLENGE_TIMEOUT);
			if (startSuc) {
				playerInfo.setXunleiPoint(leftMoney);
	...
	public TOPTeam createTopTeam(Team defenceTeam, List<Team> preZYZDZRank, boolean isChallengeZYZDZOn, Team attackTeam) throws Exception {
		if (defenceTeam != null && preZYZDZRank.contains(defenceTeam)) {
			...
			if (isChallengeZYZDZOn) {
				leftTime = getService.timeLeftForChallengeOnWar(topTeam.getTeam());
			...
			/** set操作 */
			if (isChallengeZYZDZOn && getService.beOkForChallenge(leftTime) && getService.isLevelOkForChallenge(attackTeam, defenceTeam)) {
	...
	public void createPlayerBuyTeamItemLog(String s) {
		infoLogger.log(LogServerMessage.buyPlayerItem.name(), Level.INFO_INT, LogUtils.JoinerByTab.join("1.5", DateUtil.getDf3().format(new Date()), s));
	}

	public void createIntensifyTeamItemLog(String s) {
		infoLogger.log(LogServerMessage.buyPlayerItem.name(), Level.INFO_INT, LogUtils.JoinerByTab.join("8.8", DateUtil.getDf3().format(new Date()), s));
	}
	...
	public void createPlayerBuyFunctionLog(int playerId, CostType costType, int cost, CostFunction costFunction, String... sOtherMessage) {
		...
		infoLogger.log(LogServerMessage.buyPlayerItem.name(), Level.INFO_INT, LogUtils.JoinerByTab.join("13.1", CommonUtil.simpleDateFormat.format(new Date()), sbBuffer.toString()));
	}
	...
	public String use_SXZB_Choice_Box(SysItem si) throws Exception {
		List<SysItem> returnList = new ArrayList<SysItem>();
		int level = StringUtil.toInt(si.getIValue());
		List<OnlineAward> onlineAwards = getService.getOnlineAwardByTypeAndLevel(Constants.SPECIAL_ITEM_IIDS.XSZB_CHOICE_BOX.getValue(), level);
		for (OnlineAward onlineAward : onlineAwards) {
			SysItem sysItem = getService.getSysItemByItemId(onlineAward.getItemId());
			sysItem.setUnit(onlineAward.getUnit());//3天
			sysItem.setUnitType(onlineAward.getUnitType());//天数
			int strengthLevel = sysItem.getStrengthLevel();
			if (sysItem.getStrengthLevel() > 0) {
				sysItem.setCResistanceFire_add((float) ((ServiceLocator.getService.getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem.getCResistanceFire())));
				sysItem.setCResistanceBlast_add((float) ((ServiceLocator.getService.getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem.getCResistanceBlast())));
				sysItem.setCResistanceBullet_add((float) ((ServiceLocator.getService.getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem.getCResistanceBullet())));
				sysItem.setCResistanceKnife_add((float) ((ServiceLocator.getService.getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem.getCResistanceKnife())));
				sysItem.setCBloodAdd_add((float) ((ServiceLocator.getService.getStrengthenProperty(sysItem, strengthLevel - 1, 1)) * 100 * (1 + 0.01 * sysItem.getCBloodAdd())));
			}
			if (strengthLevel >= 16) {
				sysItem.getGunProperty().setColor(6);
			} else if (strengthLevel >= 13) {
				sysItem.getGunProperty().setColor(5);
			} else if (strengthLevel >= 10) {
				sysItem.getGunProperty().setColor(2);
			} else if (strengthLevel >= 7) {
				sysItem.getGunProperty().setColor(3);
			} else if (strengthLevel >= 4) {
				sysItem.getGunProperty().setColor(4);
			}
			if (sysItem != null) {
				returnList.add(sysItem);
			}
