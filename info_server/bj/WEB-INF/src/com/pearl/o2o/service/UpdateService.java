...
public class UpdateService extends BaseService {
	...
	private void updatePlayerItemDurablewhileStage(StageClearPlayerInfo playerInfo, int useTotal, double punish) throws Exception {
		...
		for (SingleCharacterInfo ci : characterInfoList) {
			if (ci.getUsedCount() > 0 && ci.getCharacterId() < 10) {
				...
				ServiceLocator.stageClearLog.debug(playerInfo.getName() + " decrease durable " + cost + " getScore " + playerInfo.getScore() + " UsedCount " + ci.getUsedCount() + " UseTotal "
						+ playerInfo.getUseTotal());
			}
		}
			...
			if (characterData != null) {
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					ServiceLocator.nosqlService.addXunleiLog("4.2" + Constants.XUNLEI_LOG_DELIMITER + levelInfo.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + levelInfo.getType()
							...
							+ Constants.XUNLEI_LOG_DELIMITER + playerInfo.getKill() + Constants.XUNLEI_LOG_DELIMITER + playerInfo.getDead() + Constants.XUNLEI_LOG_DELIMITER
	...
	public void updatePlayerStageClear(final GeneralStageClearInfo stageClearInfo) throws Exception {
		...
		if (isMatch == 7 && gameTpye == Constants.GAMETYPE.TEAM.getValue()) {
			for (StageClearPlayerInfo playerInfo : stateClearPlayerInfo[0]) {
				int playerId = playerInfo.getId();
				Player player = getService.getPlayerById(playerId);
				meritADV += player.getMvalue();
			}
			for (StageClearPlayerInfo playerInfo : stateClearPlayerInfo[1]) {
				int playerId = playerInfo.getId();
				Player player = getService.getPlayerById(playerId);
				meritADV += player.getMvalue();
			}
		...
		if (isMatch == 8 /*&& stageClearInfo.isClear()*/) {
			...
			if (lGameType == 5) {//跳跳乐因为积分机制不同所以分开
				...
				for (int i = 0; i < 2; i++) {
					...
					for (int j = 0; j < list.size(); j++) {
						...
						lMember.setScore(3 + lMember.getScore());
						LeagueMemberImpl.updataMember(teamaId, lMember);
						Integer value = maps.get(teamaId);
						value = value == null ? 0 : value;
						maps.put(teamaId, value + 1);
					}
				}
				//记录战队id 然后最后一起把 排行榜 战斗记录 跟 (LATI)(temaid) (胜|负|积分) 一起更新
				for (Map.Entry<Integer, Integer> entry : maps.entrySet()) {
					//更新战队排行榜积分
					int teamaId = entry.getKey();
					int teambId = LeagueTeamMateImpl.get(teamaId);//根据myTeamId获得本轮联赛战队对战方的id
					int leagueGameScoreA = entry.getValue() * 3;
					int scoce_a = LeagueTeamTopImpl.getScoce(teamaId) + leagueGameScoreA;
					//更新战队排行榜积分
					LeagueTeamTopImpl.updataLeagueTeamTop(teamaId, scoce_a);
					//更新联赛战队战斗记录
					int[] scoces = LeagueGameRecordImpl.getMyTeamScoce(teamaId, teambId, 5);
					LeagueGameRecordImpl.updataGameRecords(teamaId, teambId, lGameType, scoces[0] + leagueGameScoreA, scoces[1]);
					LeagueGameRecordImpl.updataGameRecords(teambId, teamaId, lGameType, scoces[1], scoces[0] + leagueGameScoreA);
					Team a = getService.getTeam(teamaId);
					a.setLeagueScore(a.getLeagueScore() + leagueGameScoreA);
					updateTeamInfo(a);
					//获得联赛参加战队胜负积分
					LeagueTeam teama = LeagueTeamImpl.getTeamById(teamaId, 1);
					teama.setScoce(scoce_a);//积分
					log.error("#####tiaotiaole start teamId : " + teamaId + " , win : " + teama.getWin() + " , fall : " + teama.getFall() + " , score : " + teama.getScoce());
					LeagueTeamImpl.updataTeam(teamaId, teama);
					log.error("#####tiaotiaole end teamId : " + teamaId + " , win : " + teama.getWin() + " , fall : " + teama.getFall() + " , score : " + teama.getScoce());
				}
			}
			if (lGameType >= 0 && lGameType <= 4) {
				...
				int teamaId = 0;//红方战队id
				int teambId = 0;//蓝方战队id
				int terrorist = stageClearInfo.getTerroristScore();     //红方胜利盘数
				int police = stageClearInfo.getPoliceScore();           //蓝方胜利盘数
				//红方胜利盘数-蓝方胜利盘数 TerroristScore红方0   PoliceScore蓝方1
				int bout = terrorist - police;
				int leagueGameScoreA = 0;//红方本轮积分
				int leagueGameScoreB = 0;//蓝方本轮积分
				//正常结束的比赛
				if (playerInfos_0.size() != 0 && playerInfos_1.size() != 0) {
					//获得红蓝两方的战队ID,因为同一边都为同一战对队，所以获得第一个玩家的战队id就可以了
				...
				//一边跑光,导致一边数据为空无法获得战场类型
				if (playerInfos_0.size() != 0 && playerInfos_1.size() == 0) {//红方胜利,蓝方逃跑
					teamaId = getService.getPlayerById(playerInfos_0.get(0).getId()).getTeamId();
					teambId = LeagueTeamMateImpl.get(teamaId);//根据myTeamId获得本轮联赛战队对战方的id
					if (lGameType == 0) { //胜利导致盘数不对这里进行处理,高级战场进行三盘
						terrorist = terrorist < 3 ? 3 : terrorist;
						bout = terrorist - police;
					} else if (lGameType == 1) {//胜利导致盘数不对这里进行处理,中级战场进行三盘
						terrorist = terrorist < 2 ? 2 : terrorist;
						bout = terrorist - police;
					} else {//胜利导致盘数不对这里进行处理,低级战场只分胜负
						bout = 1;
					}
				}
				//一边跑光,导致一边数据为空无法获得战场类型
				if (playerInfos_1.size() != 0 && playerInfos_0.size() == 0) {
					teambId = getService.getPlayerById(playerInfos_1.get(0).getId()).getTeamId();
					teamaId = LeagueTeamMateImpl.get(teambId);//根据myTeamId获得本轮联赛战队对战方的id
					if (lGameType == 0) {//胜利导致盘数不对这里进行处理,高级战场进行三盘
						police = police < 3 ? 3 : police;
						bout = terrorist - police;
					} else if (lGameType == 1) {//胜利导致盘数不对这里进行处理,中级战场进行三盘
						police = police < 2 ? 2 : police;
						bout = terrorist - police;
					} else {//胜利导致盘数不对这里进行处理,低级战场只分胜负
						bout = -1;
					}
				...
				//获得本轮联赛积分
				leagueGameScoreA = LeagueUtil.leagueGameScore(bout, lGameType);
				leagueGameScoreB = LeagueUtil.leagueGameScore(-bout, lGameType);
				if (playerInfos_0.size() == 0) {//任何一队无玩家完赛，得零分
					leagueGameScoreA = 0;
				}
				if (playerInfos_1.size() == 0) {
					leagueGameScoreB = 0;
				}
				//获得排行榜战队积分+本轮获得积分
				int scoce_a = LeagueTeamTopImpl.getScoce(teamaId) + leagueGameScoreA;
				int scoce_b = LeagueTeamTopImpl.getScoce(teambId) + leagueGameScoreB;
				//获得联赛参加战队胜负积分
				LeagueTeam teama = LeagueTeamImpl.getTeamById(teamaId, 1);
				LeagueTeam teamb = LeagueTeamImpl.getTeamById(teambId, 1);
				log.debug("#####start teamAId : " + teamaId + " , win : " + teama.getWin() + " , fall : " + teama.getFall() + " , score : " + teama.getScoce());
				log.debug("#####start teamBId : " + teambId + " , win : " + teamb.getWin() + " , fall : " + teamb.getFall() + " , score : " + teamb.getScoce());
				teama.setScoce(scoce_a);//积分
				teamb.setScoce(scoce_b);//积分
				if (bout > 0) {
					teama.setWin(teama.getWin() + 1);
					teamb.setFall(teamb.getFall() + 1);
				} else if (bout < 0) {
					teamb.setWin(teamb.getWin() + 1);
					teama.setFall(teama.getFall() + 1);
				}
				//更新战队排行榜积分
				LeagueTeamTopImpl.updataLeagueTeamTop(teamaId, scoce_a);
				LeagueTeamTopImpl.updataLeagueTeamTop(teambId, scoce_b);
				// 更新到联赛战队胜负积分表
				log.debug("#####end teamAId : " + teamaId + " , win : " + teama.getWin() + " , fall : " + teama.getFall() + " , score : " + teama.getScoce());
				log.debug("#####end teamBId : " + teambId + " , win : " + teamb.getWin() + " , fall : " + teamb.getFall() + " , score : " + teamb.getScoce());
				LeagueTeamImpl.updataTeam(teamaId, teama);
				...
				a.setLeagueScore(a.getLeagueScore() + leagueGameScoreA);
				b.setLeagueScore(b.getLeagueScore() + leagueGameScoreB);
				updateTeamInfo(a);
				updateTeamInfo(b);
				//更新战队成员积分排行榜 (lami:teamid) (pid) (战场type|积分|"名字")
				for (int i = 0; i < 2; i++) {//0为红方即为A,1为蓝方即为B
					...
					for (StageClearPlayerInfo playerInfo : stateClearPlayerInfo[i]) {
						...
						if (lMember == null)
							continue;
						//加上积分
						lMember.setScore(leagueGameScore + lMember.getScore());
						//存入参赛玩家的数据
	...
	//
	// }
	public void updatePlayerAchievement(PlayerAchievement pa, int numberThisRound, Player p, List<PlayerAchievement> finishedList, GeneralStageClearInfo stageClearInfo, StageClearPlayerInfo player,
			SysAchievement sa) throws Exception {
		...
		for (PlayerMission pm : playerMissions) {
			if (0 == pm.getStatus()) {// 已完成任务不做处理
				...
				if (Constants.ACTION_M.MAP1_MODEL1.getValue() == action) {
					...
					completeNum = player.getMaxDamageNum();
				} else if (Constants.ACTION_M.MAP44_MODEL0.getValue() == action && stageClearInfo.getIsMatch() == 7) {//拾取补给（匹配）
					completeNum = player.getBottleHpNum() + player.getBulletNum();
				} else if (Constants.ACTION_M.MAP45_MODEL0.getValue() == action && stageClearInfo.getIsMatch() == 7) {//累积积分（匹配）
					if (player.getScore() != 0 && player.getScore() > 0) {
						completeNum = player.getScore();
					}
				} else if (Constants.ACTION_M.MAP112_MODEL0.getValue() == action && stageClearInfo.getIsMatch() == 7) {//杀戮指令（匹配）在自动匹配中击杀{0}人
					completeNum = player.getKill();
				} else if (Constants.ACTION_M.MAP111_MODEL0.getValue() == action && stageClearInfo.getIsMatch() == 7) {//冲锋行动（匹配）在自动匹配中赢得{0}场胜利(3人或以上有效)
					if (player.getIsWiner() == 1)
						completeNum = 1;
				} else if (Constants.ACTION_M.MAP118_MODEL0.getValue() == action && stageClearInfo.getIsMatch() == 7) {//援护使命（匹配）在自动匹配中完成{0}次助攻(3人或以上有效)
					completeNum = player.getAssistNum();
				}
		...
		if (finishedMissionList.size() > 0) {// 有完成的任务
			...
			for (PlayerMission pmis : finishedMissionList) {
				...
				if (ServiceLocator.missionCompleteLog.isDebugEnabled()) {
					ServiceLocator.missionCompleteLog.debug(String.format("%s\t%s\t%s\t%s", player.getId(), player.getRank(), type, pmis.getId()));
				}
			}
