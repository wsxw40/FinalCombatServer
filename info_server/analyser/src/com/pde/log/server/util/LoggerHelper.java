...
public class LoggerHelper {
	// public static final LoadingCache<String, Logger> loggerCache =
	// CacheBuilder.newBuilder().build(new CacheLoader<String, Logger>() {
	// @Override
	...
	public static void log(Logger logger, LogMsg logMsg) {
		try {
			...
			FilterReply decision = logger.getLoggerContext()
					.getTurboFilterList().size() == 0 ? FilterReply.NEUTRAL
			...
			Constants.logServerLogger.error("LogServerHandler.log", e);
		}
	}
	...
	public static void logs(List<LogMsg> logs) {
		...
		// HashMultimap<String, LogMsg> multimap = HashMultimap.<String, LogMsg>
		// create();
