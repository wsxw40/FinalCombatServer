 ...
 * 玩家卸载道具
 */
@Service("fcw_c_storage_req_unpack")
public class CStorageReqUnpack extends BaseClientServlet implements Servletable {

    private static final long serialVersionUID = -6124047153822936179L;
    private Logger logger = LoggerFactory.getLogger(getClass());
    @Resource
    private CStorageReqUnpack fcw_c_storage_req_unpack;
    @Resource
    private StorageService storageService;

    @Override
    protected String[] paramNames() {
        return new String[] { "uid", "pid", "cid", "t", "seq" };
    }

    @Override
    protected String innerService(String... strings) {
        try {
            return fcw_c_storage_req_unpack.rpc(strings);
        } catch (BaseException e) {
            return Smarty4jConverter.error(e.getMessage());
        } catch (Exception e) {
            logger.error("c_storage_req_unpack has error : " + getLockedKey(strings), e);
        }
    ...
    public String rpc(String... args) throws Exception {
        ...
        int seq = Integer.parseInt(args[4]);
        String packType = type == 1 ? "W" : "C";
        storageService.upEquip(playerId, sysCharacterId, packType, seq);
