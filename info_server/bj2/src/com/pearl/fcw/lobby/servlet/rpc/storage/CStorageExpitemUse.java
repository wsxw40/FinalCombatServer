 ...
 * 玩家使用增长经验的物品
 */
@Service("fcw_c_storage_expitem_use")
public class CStorageExpitemUse extends BaseClientServlet implements Servletable {

    private static final long serialVersionUID = 1960333693203028583L;
    private Logger logger = LoggerFactory.getLogger(getClass());
    @Resource
    private CStorageExpitemUse fcw_c_storage_expitem_use;
    @Resource
    private StorageService storageService;

    @Override
    protected String[] paramNames() {
        return new String[] { "uid", "pid", "piid" };
    }

    @Override
    protected String innerService(String... strings) {
        try {
            return fcw_c_storage_expitem_use.rpc(strings);
        } catch (Exception e) {
            logger.error("c_storage_expitem_use has error : " + getLockedKey(strings), e);
        }
