 ...
 * 熔炼奖励预览
 */
@Service("fcw_c_melting_output_lookover")
public class CMeltingOutputLookover extends BaseClientServlet implements Servletable {

	private static final long serialVersionUID = 966139074392600411L;
	private Logger logger = LoggerFactory.getLogger(getClass());
    @Resource
	private CMeltingOutputLookover fcw_c_melting_output_lookover;
    @Resource
    ...
    protected String innerService(String... strings) {
        try {
			return fcw_c_melting_output_lookover.rpc(strings);
        } catch (BaseException e) {
            return Smarty4jConverter.error(e.getMessage());
		} catch (ProxyInterruptException e) {
			return e.getMessage();
		} catch (Exception e) {
			logger.error("c_melting_output_lookover has error : " + getLockedKey(strings), e);
        }
