...
public class WGmLogService extends DmServiceImpl<WGmLog, Integer> {
    ...
    public DataTablesPage<Map<String, Object>> getTransactionPageMap(DataTablesParameter param) throws Exception {
        DataTablesPage<WGmTransaction> pageWGmTransaction = wGmTransactionDao.findPage(param);
        List<Map<String, Object>> list = new ArrayList<>();
        List<WGmTransaction> wGmTransactionList = pageWGmTransaction.getData();
        list = MapUtil.toListMapExcept(wGmTransactionList);

        list.forEach(p -> {
            try {
                WGmUser wGmUser = wGmUserService.findEntity(Integer.parseInt(p.get("gmUserId").toString()));
                p.put("userName", wGmUser.getUserName());
            } catch (Exception e) {
                p.put("userName", "");
            }
        });
        DataTablesPage<Map<String, Object>> page = new DataTablesPage<Map<String, Object>>(list);
        page.setDraw(pageWGmTransaction.getDraw());
        page.setError(pageWGmTransaction.getError());
        page.setRecordsFiltered(pageWGmTransaction.getRecordsFiltered());
        page.setRecordsTotal(pageWGmTransaction.getRecordsTotal());
        return page;
    }
