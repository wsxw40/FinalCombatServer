...
//		numberFormat.setMaximumFractionDigits(4);
    ...
    /**
     * 获取稀有度 1-25 普通 26-50 精良 51-75 卓越 76-100 稀有
     * @param map
     * @return
     */
...
//		randomArray[random.nextInt(totalWeight)];
	 ...
	 * @param playerId
     * @param type 5
     * @param mType SysItem.mType
     * @param mValue SysItem.mValue
	 * @return
	 * @throws Exception
	 */
	...
	public static String generateDartleTopRetStr(String key,Player player) throws Exception{
        ...
        String pref = new StringBuilder().append("{").append("\"").append(player.getName()).append("\",").append(player.getRank()).append(",").toString();
	...
	public static boolean isResMagicBox(SysItem sysItem) {
        if (sysItem.getType() == Constants.DEFAULT_OPEN_TYPE && "48".equals(sysItem.getIId())) {
			return true;
		} else {
			return false;
		}
	...
	public static boolean isZYZDZBuff(SysItem sysItem) {
        if (sysItem.getType() != null && sysItem.getType() == Constants.DEFAULT_ZYZDZ_TYPE && sysItem.getSubType() != null && sysItem.getSubType() == 6) {
			return true;
		} else {
			return false;
		}
	...
	public static double Team_Res_ratio(int attTeam_today_Res, int level) {
        double result = attTeam_today_Res > (Constants.ZYZDZ_TEAM_RES_RATIO[level - 1][0] + Constants.ZYZDZ_TEAM_RES_RATIO[level - 1][1]) ? 0
                : attTeam_today_Res > Constants.ZYZDZ_TEAM_RES_RATIO[level - 1][0] ? 0.5 : 1;
		return result;
	}
	...
	public static double player_Res_ratio(int player_today_Res, int level) {
        double result = player_today_Res > (Constants.ZYZDZ_PLAYER_RES_RATIO[level - 1][0] + Constants.ZYZDZ_PLAYER_RES_RATIO[level - 1][1]) ? 0
                : player_today_Res > Constants.ZYZDZ_PLAYER_RES_RATIO[level - 1][0] ? 0.5 : 1;
		return result;
	}
	...
	public static boolean provisional_item_scope_flag(int sysItemId) {
		if(sysItemId>=Constants.PROVISIONAL_ITEM_SCOPE_BEGIN&&sysItemId<=Constants.PROVISIONAL_ITEM_SCOPE_END){
			return true;
        } else if (sysItemId >= Constants.PROVISIONAL_ITEM_SCOPE_BEGIN_2 && sysItemId <= Constants.PROVISIONAL_ITEM_SCOPE_END_2) {
            return true;
		}else {
			return false;
		}
    ...
    //zlm2015-5-7-限时装备-结束-------------------------------------
    //TODO zlm_7_15匹配新算法start========
    /**
     * P_单 kill 杀人数 die 死亡 assist 助攻
     */
    public static double get_P_kda(int kill, int dead, int assist) throws Exception {
        //算出kda
        double kda = (kill + assist * 0.5 + 1) / (dead + 1);
        double p_kad = 0;
        if (kda > 10) {
            p_kad = 15;
        } else if (kda > 1) {
            p_kad = 5 * Math.sqrt(kda) - 5;
        } else if (kda > 0.1) {
            p_kad = -(5 * Math.sqrt(11 - 10 * kda) - 5);
        } else if (kda >= 0) {
            p_kad = -15;
        }
        return p_kad;
    }
    //TODO zlm_7_15匹配新算法end==========
    public static double get_M_kda_Win(Integer meritPlayer, double meritADV, int indexPlayer, int fPlayer) {
        return 18 * (1 + ((fPlayer - 2 * indexPlayer) / fPlayer) * 0.5) * (1 + ((meritADV - meritPlayer) / 500) * 0.1);
    }
	
    public static double get_M_kda_Fall(Integer meritPlayer, double meritADV, int indexPlayer, int fPlayer) {
        return 15 * (1 + ((2 * indexPlayer - fPlayer) / fPlayer) * 0.5) * (1 + ((meritPlayer - meritADV) / 500) * 0.1);
    }
    /**
     * 获得可续费时间
     * @param nosql
     ...
     * @throws Exception
     */
    public static int provisional_time(NoSql nosql,PlayerItem playerItem,int playerId) throws Exception {
        String key = Constants.PROVISIONAL_ITEM_FLAG_KEY + playerId + Constants.DELIMITER + playerItem.getId();
        //装备的可续费天数只保留40天,而可续费天数只有30天
        //所以创建天数距现在大于40天的物品,一定没有可续费天数
        if ((40 * 24 * 60 * 60) > ((System.currentTimeMillis() - playerItem.getValidDate().getTime())/1000)) {
            String flag = nosql.get(key);
            if (flag == null || "".equals(flag)) {//没值
                long ttl = nosql.ttl(key);
                nosql.set(key, "30");
                nosql.expire(key, (int) ttl);
                return 30;//默认可以续费天数为30天
            } else {
                return Integer.valueOf(flag);
		}
		}
        return 0;//默认可以续费天数为30天
	}
}