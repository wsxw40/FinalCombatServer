...
public class KeywordFilterUtil {
	...
	public static Set<String> getKeywordsList() {
		return keywords;
	}
	...
	public static void loadKeyWords() throws Exception{
		...
		keywords = ServiceLocator.getService.getBannedWordsStrList().stream().collect(Collectors.toSet());
		keywords.removeIf(p -> StringUtil.isEmpty(p));
		initRegEx();
...
//
	...
	private static void initRegEx() throws Exception{
		regex = "(" + keywords.stream().map(p -> {
			try {
				return com.pearl.o2o.utils.StringUtil.escape(p);
			} catch (Exception e) {
			}
			return p;
