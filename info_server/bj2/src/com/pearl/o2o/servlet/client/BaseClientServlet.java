...
public class BaseClientServlet extends HttpServlet {
	...
	private static final LuaScriptEngine luaScriptEngine = new LuaScriptEngine();
	...
	//FCW jmeter测试接口用
	public String getResponseStr(HttpServletRequest req, HttpServletResponse res) throws Exception {
		res.setCharacterEncoding("UTF-8");
		req.setCharacterEncoding("UTF-8");
		...
		//检查lua脚本格式
		try {
			CompiledScript cs = luaScriptEngine.compile(result);
			Bindings b = luaScriptEngine.createBindings();
			cs.eval(b);
		} catch (Exception e) {
			System.err.println(e);
			System.err.println(result);
			result = Converter.error(ExceptionMessage.ERROR_MESSAGE_ALL);
		}
		return result;
	}
	...
	public List<Response> execute(Map<String, String> params) throws MemcachedException, TimeoutException, Exception {
		String[] paramValues = new String[paramNames().length + 1];
		//获取参数时，增加获取防沉迷时间的逻辑
		int playerId = -1;
		int time = -1;
		...
		paramValues[paramValues.length - 1] = params.get("uri");
		...
		try {
			...
			throw new Exception("rpc gzip error");
		}
