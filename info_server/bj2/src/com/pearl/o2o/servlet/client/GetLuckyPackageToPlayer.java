    ...
    protected String innerService(String... args) {
		try{
			...
			for(SysChest sc :fixList){
				if(ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()&&sc.getSysItemId()==GrowthMissionConstants.UPGARDMOD_ID){
					int upgradedNumByPId = getService.getUpgradedNumByPId(playerId);
					//998、博彩宝箱模块
					XunleiLogUtils.xlLog_22_1(player, 1, upgradedNumByPId,sc.getNumber()*multiple,upgradedNumByPId+sc.getNumber()*multiple, 998);
				}
			...
			int num = 1;
			if(payType == 2){
				if(level == 1) num = 6; else if(level == 2) num = 12; else if (level == 3) num = 22;
			}else if (payType==1){
				num = 1;
			}else{
				return null;
			}
			String msg	 = getService.getLuckyPackageIsPay(playerId, fixList.get(0), payType, level, multiple * num);
			if(msg!=null){
				return Converter.playerLuckyPackage(null,null, null, msg,null,null,null);
			}

			ServiceLocator.updateService.addPlayerTrack(playerId, "", payType==1?level==3?multiple:0:0,payType==2?multiple:0,0, "", "");
			...
			if(player!=null && player.getIsChest()!=null){
				if(FirstCheckUtils.getLuckyPackageFirst(player) && level== 3){
