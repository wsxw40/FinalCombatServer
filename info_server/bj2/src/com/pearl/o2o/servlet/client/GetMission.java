...
public class GetMission extends BaseClientServlet {
	...
	//		final int time = StringUtil.toInt(args[5]);
			...
			case 2:
				...
				if (playerActivityList == null || playerActivityList.size() == 0){
				...
				if(0==type){
					...
					if(null != playerActivity){
						...
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
							...
							//13、活动
							if (playerActivity.getSysActivity().getSysItem().getId() == GrowthMissionConstants.UPGARDMOD_ID) {
								int upgradedNumByPId = getService.getUpgradedNumByPId(player.getId());
								XunleiLogUtils.xlLog_22_1(player, 1,upgradedNumByPId, playerActivity.getSysActivity().getUnit(),
										upgradedNumByPId+ playerActivity.getSysActivity().getUnit(), 13);
							}
						}
