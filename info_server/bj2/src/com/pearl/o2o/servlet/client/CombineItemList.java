			...
			if (type < Constants.NUM_ONE || type > Constants.TYPE_NUM || page < Constants.NUM_ONE) {
				...
				if (type == Constants.DEFAULT_MATERIAL_TYPE) {
					...
					for (PlayerItem pi : itemArray) {
						if (combineType == EItemCombineType.ENHANCE.getNumber()) {
							if (pi != null && pi.getSysItem() != null)
								if (pi.getSysItem().getMType() == Constants.STRENGTH_PART || pi.getSysItem().getMType() == Constants.ADD_S || pi.getSysItem().getMType() == Constants.NOT_BREAK
										|| pi.getSysItem().getMType() == Constants.NOT_BACK_LEVEL || pi.getSysItem().getMType() == Constants.STRENGTH_ADD_S) {
									tempArray.add(pi);
								}
						} else if (combineType == EItemCombineType.ENHANCE.getNumber()) {
							if (pi != null && pi.getSysItem() != null)
						...
						} else if (combineType == 4 && pi != null && pi.getSysItem() != null
								&& (pi.getSysItem().getEvaluateRmb() > 0 || pi.getLevel() > 0 || pi.getSysItem().getMType() == 31 || pi.getSysItem().getMType() == 32) && pi.getPack() == 0) {
							//FCW 排除不可熔炼的物品
							if (pi.getSysItem().getMType() != EItemMType.M_NOT_MELTING_NOT_RENEWAL.getNumber()) {
								tempArray.add(pi);
							}
						}
					}
				...
				if (itemArray != null) {
					...
					if (combineType == Constants.NUM_FOUR) {//如果是熔炼
						for (PlayerItem pi : itemArray) {
							if (pi != null && pi.getSysItem() != null && "N".equals(pi.getIsDefault()) && combineType == 4
									&& (pi.getSysItem().getEvaluateRmb() > 0 || pi.getLevel() > 0 || pi.getSysItem().getMType() == 31 || pi.getSysItem().getMType() == 32) && pi.getPack() == 0
									&& pi.getBuff() == 0) {
					...
					} else if (type == EItemType.WEAPON.getNumber() || type == EItemType.COSTUME.getNumber() || type == EItemType.PART.getNumber()) {
						for (PlayerItem playerItem : itemArray) {
							if (playerItem != null && playerItem.getSysItem() != null && playerItem.getSysItem().getIsStrengthen() == 1) {//是否可以强化
								//								if(each.getSysItem().getWId()==Constants.WEAPON_JQT_GUN_ID&&(combineType==2||combineType==3)){
								//									continue;
								//								}
								//如为合成[升星]
								if (combineType == EItemCombineType.GST.getNumber()) {
									//去掉普通武器，并取消身上携带
									if (!pkayerPacks.containsKey(playerItem.getId())
											&& (playerItem.getSysItem().getType() == EItemType.WEAPON.getNumber() || playerItem.getSysItem().getType() == EItemType.COSTUME.getNumber() || playerItem
													.getSysItem()
													.getType() == EItemType.PART.getNumber())
 && playerItem.getSysItem().getRareLevel() >= Constants.RARE_LEVEL[0]) {
										tempList.add(playerItem);
					...
					tempList.sort(LobbyCompareUtil.PLAYER_ITEM_COMPARATOR);
	...
	private String meltingItemStr(PlayerItem pi) {
		Integer[] array = Iterables
				.toArray(Iterables.transform(MeltingConstants.splitterByUnderScore.split(pi.getSysItem().getResourceChangeable()), MeltingConstants.functionStrToInt), Integer.class);
