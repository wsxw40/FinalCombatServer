...
public class CreatePlayerItem extends BaseClientServlet {
	...
	protected String innerService(String... args) {
		...
		try {
			...
			if (sysItem.getShoppingType() != EItemShoppingType.NORAML_SALE.getNumber() && sysItem.getShoppingType() != EItemShoppingType.FLASH_SALE.getNumber()) {//该道具不销售
			...
			if(sysItem.getIBuffId()==Constants.DEFAULT_ON_CARD_BUFF){
				...
				//续费物品
				if (onCard != null && sysItem.getShoppingType() != EItemShoppingType.UNSALE.getNumber() && Constants.BOOLEAN_NO.equals(onCard.getIsDeleted())) {	//月卡叠加
