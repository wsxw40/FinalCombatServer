...
public class LeagueTeamTop extends BaseClientServlet {
	...
	protected String innerService(String... args) {
		try{
			...
			final int type = StringUtil.toInt(args[2]);//第几页
			Player player = getService.getPlayerById(playerId);
            Integer myTeamId = player.getTeamId();
            LeagueTeam myLeagueTeam = LeagueTeamTopImpl.getLeagueTeam(myTeamId);
          ...
          /*  if (type==1) //当前赛季
            ...
            if (type==0) //上个赛季
                key =key;*/
            Page page = new Page(8,LeagueTeamTopImpl.getNumber(),pageNow);
