...
public class CombineTwoToOne extends BaseClientServlet {
	...
	static Logger log = LoggerFactory.getLogger(CombineTwoToOne.class.getName());
	...
	protected String innerService(String... args) {
		...
		try {
			PlayerItem pItem1 = getService.getPlayerItemById(playerId, mainItemId);
			...
			for (String itemIdStr : pItems) {
				...
				// 主武器,服饰,配饰限定
				if (pItem2.getSysItem().getType() == EItemType.WEAPON.getNumber() || pItem2.getSysItem().getType() == EItemType.COSTUME.getNumber()
						|| pItem2.getSysItem().getType() == EItemType.PART.getNumber()) {
					//类型不能混合升星
					if (!pItem1.getSysItem().getType().equals(pItem2.getSysItem().getType()))
			...
			if (items != null && items.size() > 0)
				updateService.combinePlayerItem(playerId, pItem1, items, repeat.get(pItem1.getSysItem().getId()), isFree == 1 ? true : false);

			PlayerItem item = getService.getPlayerItemById(playerId, mainItemId);
