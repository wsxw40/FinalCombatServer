	...
	protected String innerService(String... args) {
		...
		try{
			...
			for(SysCharacter sc:characterList){
				...
				if (sc.getId() == 2) {
					ch.setDescription("完成“等级达到lv6”成就可解锁职业");
				}
				if (sc.getId() == 5) {
					ch.setDescription("完成“等级达到lv3”成就可解锁职业");
				}
			}
			//检测玩家所选参战名单
			...
			//FCW 玩家参战角色控制方式改变
			result = Converter.playerCharacterList(enableList, player.getSelectedCharacters(), Constants.MAX_SELECT_CHARACTER_SIZE, openSize, bioListAndState, bioBuffItemList);
			return result;
		}
		catch (Exception e) {
