		...
		try {
			...
			if (player == null || player.getId() == 0) {
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					...
					ServiceLocator.nosqlService.addXunleiLog("1.5" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
				...
				//					String loginMessage = String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s", playerId, player.getName(), player.getUserName(), player.getRank(), ip, new SimpleDateFormat(
				//							"yyyy-MM-dd-HH-mm-ss").format(player.getCreateTime()), new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date()));
