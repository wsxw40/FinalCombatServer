...
public class CharacterOnline extends BaseServerServlet {
	...
	protected byte[] innerService(BinaryReader r) throws IOException, Exception {
		...
		try {
			...
			if (!versionCorrect) {
				throw new BaseException(CommonUtil.messageFormatI18N(ExceptionMessage.CLIENT_VERSION_ERROR, clientVersions));
			}
			...
			//			}else
			{
				...
				if (player.getIsCheck() == 0) {
					for (int i : Constants.defalutItems) {
						createService.getPlayerItemDao().createPlayItem(player.getId(), getService.getSysItemByItemId(i), 1, 1, Constants.BOOLEAN_NO, Constants.BOOLEAN_YES, Constants.BOOLEAN_NO);
					}
					for (int i = 0; i < Constants.weaponPack.length; i++) {
						if (i != 3) {
							SysItem si = getService.getSysItemByItemId(Constants.weaponPack[i][3]);
							int playerItemId = createService.getPlayerItemDao().createPlayItem(player.getId(), si, 1, 0, Constants.BOOLEAN_NO, Constants.BOOLEAN_YES, Constants.BOOLEAN_YES);
				...
				out.write(BinaryUtil.toByta(ConfigurationUtil.GM_USER_IDS.contains(String.valueOf(player.getId())) ? (byte) 1 : (byte) 0));
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					ServiceLocator.nosqlService.addXunleiLog("1.1" + Constants.XUNLEI_LOG_DELIMITER + player.getUserName() + Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getRank() + Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date()));
				}
				//				{
					//					infoLogger.log(
					//							LogServerMessage.secondLog.name(),
					//							Level.INFO_INT,
					//							LogUtils.JoinerByTab.join("2.1", playerId, player.getUserName(), CommonUtil.simpleDateFormat.format(new Date()), ConfigurationUtil.XUNLEI_SERVER_IP, ip, playerId,
					//									player.getName(), player.getRank()));

				//					String message = String.format("%s\t%s\t%s\t%s\t%s\t%s\t%s", playerId, player.getName(), player.getUserName(), player.getRank(), ip,
				//							new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(player.getCreateTime()), new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date()));
				//					transferDataToDc.addLog("bjLoginIn", message);

				//				}
				//FCW 客户端UI功能开关
				out.write(BinaryUtil.toByta(WSysConfigService.getClientSwitch()));
			}
