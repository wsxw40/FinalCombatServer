...
//			log.info("start server with port" + appPort);
			...
			if(Constants.TEAM_FIGHT_RES_PREWEEK==1){
				...
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new InitTeamFightResPreWeekTask(),CommonUtil.minutesToNextTuesday() , Constants.TOP_PLAYER_ACTIVITY_MINUTES, TimeUnit.MINUTES);
			}
...
//				new CacZYZDZChallengeWarTask().run();
				ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(
						new CacZYZDZChallengeWarTask(),  CommonUtil.minutesToDate(ServiceLocator.getService.getZYZDZChallengeTime(2).getTime()), 7*24*60, TimeUnit.MINUTES);
			}
		...
		//		ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new CompeteBuyOverTask(), CommonUtil.minutesToDate(ServiceLocator.getService.getCompeteBuyTime(2)), 60, TimeUnit.MINUTES);
            ...
            if(Constants.QW_SCOCE_CLEAN==1){
                ...
                ServiceLocator.scheduledExecutorService.scheduleWithFixedDelay(new LeagueApplyRecordClean(), 
                        CommonUtil.millisecondToNextday2(), 24 * 60 * 60 * 1000l, TimeUnit.MILLISECONDS);
            }   
