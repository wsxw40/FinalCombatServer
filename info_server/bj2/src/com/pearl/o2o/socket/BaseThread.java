...
public class BaseThread implements Runnable {
	...
	public void run() {
		...
		try {
			if (null != request.getParamMap()) {
				//FCW 可以将接口url作为参数的一部分
				request.getParamMap().put("uri", request.getUrl());
				log.info(request.getParamMap().toString());
			}
			...
			//			File file=new File(ConfigurationUtil.APP_ROOT_PATH+"response.txt");
			//			BufferedWriter bWriter=new BufferedWriter(new FileWriter(file));
			//			for(Response rp:result){
			//			//	log.error(rp.getText());
			//				bWriter.write(rp.getText());
			//			}
			//			bWriter.flush();
			//			bWriter.close();
	...
	private void sendError(ServerMessage responseType, int queueId, int expectedRequestCount) {
		BinaryChannelBuffer buffer = new BinaryChannelBuffer(BinaryUtil.getLength((byte) responseType.ordinal()) + BinaryUtil.getLength(queueId) + BinaryUtil.getLength(expectedRequestCount)
				+ BinaryUtil.getLength(request.getRpcId()) + BinaryUtil.getLength(StatusCode.FAIL.ordinal()) + BinaryUtil.getLength(request.getUserData()));
