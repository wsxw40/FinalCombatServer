...
//		nosql.zAdd(NosqlKeyUtil.commonLevelNumInRealTopByType("kKillDead"), 0.0, String.valueOf(player.getId()));
	...
	public int buyPlayerItemList(StringBuffer check,int playerId, List<String[]> playeritems, int packId, int characterId) throws Exception {
		...
		for (String[] str : playeritems) {
			...
			if (sysItem.getShoppingType() == EItemShoppingType.UNSALE.getNumber()) {//该道具不销售
				throw new BaseException(ExceptionMessage.BUY_FAIL);
			}
			Date now = new Date();
			if (sysItem.getShoppingType() == EItemShoppingType.FLASH_SALE.getNumber()) {
				if (com.pearl.fcw.utils.DateUtil.after(now, sysItem.getShoppingStartTime(), Calendar.MILLISECOND, 1)
						&& com.pearl.fcw.utils.DateUtil.before(now, sysItem.getShoppingEndTime(), Calendar.MILLISECOND, 1)) {
		...
		for (int i = 0; i < sysItemList.size(); i++) {
			...
			if (!CommonUtil.isZYZDZBuff(sysItem)// 不对资源争夺战的物品进行判断
					&& (sysItem.getShoppingType() == EItemShoppingType.UNSALE.getNumber() || (payment != null && payment
							.getIsShow() == 0))		)
...
//		int playerAction = 0;
		if (Constants.GP_PAY == payTpye) {
			...
			if (cr < cost) {
				...
				//FCW 购买C币立刻使用，该逻辑挖东墙补西墙，根本上不合理
                if (useIt > 0 && sysItem.getIId() == EItemIId.ITEM_GAME_COIN.getNumber()) {
                    awardCALToPlayer(player, Integer.parseInt(sysItem.getIValue()));
                    playerItemId = 1;
					// 发布购买事件
				} else if (!Constants.BOOLEAN_YES.equals(isGift) && !(sysItem.getIBuffId() >= 61 && sysItem.getIBuffId() <= 74)) {//战队buff:ibuffId:61-74,不会显示通知
                        nosqlService.publishEvent(new PurchaseEvent(DateUtil.getCurrentTimeStr(), player.getId(), player.getName(), sysItem));
                    }
				// 发布道具
                    if (isPresentPackage && !Constants.BOOLEAN_YES.equals(isGift)) {
...
//				}
	...
	private String useLevelPackage(Player player,SysItem si,int giftType) throws Exception{
		...
		int cal=0;
		//FCW 礼包配置从代码固定改为sysItem表items字段
		List<Payment> paymentList = Stream.of((null == si.getItems() ? "" : si.getItems()).split(";")).map(p -> {
			Payment payment = new Payment();
			payment.setPayType(EPayType.PAY_NONE.getNumber());
			try {
				List<Integer> list = Stream.of(p.split(",")).map(Integer::parseInt).collect(Collectors.toList());
				payment.setItemId(list.get(0));
				payment.setPayType(list.get(1));
				payment.setUnitType(list.get(2));
				payment.setCost(list.get(3));
				payment.setUnit(list.get(4));
			} catch (Exception e) {
			}
			return payment;
		}).collect(Collectors.toList());
		for (Payment payment : paymentList) {
			switch (EPayType.forNumber(payment.getPayType())) {
			case PAY_GP://游戏币
				awardCALToPlayer(player, payment.getCost() * payment.getUnit());
				cal += payment.getCost() * payment.getUnit();
				break;
			case PAY_ITEM://道具
				SysItem sysItem = getService.getSysItemByItemId(payment.getItemId());
				sendItem(sysItem, player, payment, Constants.BOOLEAN_NO, Constants.BOOLEAN_YES, Constants.BOOLEAN_NO);
				SysItem item2 = sysItem.clone();
				item2.setUnit(payment.getUnit());
				item2.setUnitType(sysItem.getUnitType());
				returnList.add(item2);
				break;
			default:
				break;
...
//			log.info("Player:" + playerId + "already checked on " + dateStr);
	...
	public QuietBounds createBoundsDaily(int playerId) throws Exception{
		...
		quietBounds.setDisResBounds(0);//FCW 页游不发个人原石
		//个人黑原石奖励
...
//		}
	...
	public String use_SXZB_Choice_Box(SysItem si) throws Exception{
		...
		List<OnlineAward> onlineAwards = getService
				.getOnlineAwardByTypeAndLevel(Constants.SPECIAL_ITEM_IIDS.XSZB_CHOICE_BOX.getValue(),level);
		for (OnlineAward onlineAward : onlineAwards) {
			...
			if (sysItem.getStrengthLevel()>0) {
			    sysItem.setCResistanceFire_add((float) ((ServiceLocator.getService
                        .getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem
                                .getCResistanceFire())));
			    sysItem.setCResistanceBlast_add((float) ((ServiceLocator.getService
                        .getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem
                                .getCResistanceBlast())));
			    sysItem.setCResistanceBullet_add((float) ((ServiceLocator.getService
	                    .getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem
	                    .getCResistanceBullet())));
			    sysItem.setCResistanceKnife_add((float) ((ServiceLocator.getService
	                    .getStrengthenProperty(sysItem, strengthLevel - 1, 0)) * 100 * (1 + 0.01 * sysItem
	                    .getCResistanceKnife())));
			    sysItem.setCBloodAdd_add((float) ((ServiceLocator.getService
	                    .getStrengthenProperty(sysItem, strengthLevel - 1, 1)) * 100 * (1 + 0.01 * sysItem
	                    .getCBloodAdd())));
			}
			if (strengthLevel>=16) {
