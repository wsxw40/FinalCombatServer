	...
	public void createMessage(Player sender,Player receiver ,String subject, String content ,int itemId,int costId,String itemIds,int giftId)throws Exception{
		...
		if (itemId !=0 && costId !=0){
			...
			if ((sysItem.getType() == Constants.DEFAULT_WEAPON_TYPE
					...
					&& payment.getPayType() == Constants.GP_PAY)
				throw new NotBuyEquipmentException(ExceptionMessage.NOT_GIFT_BIND);
			//zlm2016-09-07-C币购买武器装备，由原先的“可赠送”改为“不可赠送”-结束-------------------------------------
			if (sysItem.getShoppingType() == EItemShoppingType.UNSALE.getNumber() || (payment != null && payment.getIsShow() == 0)) //payment.getIsShow()==0 该支付价格不能使用
