...
public class SysItem extends BaseItemPojo<SysItem> {
	...
	private void writeWeaponBaseInfo(OutputStream out) throws Exception {
		...
		if (this.resource != null) {
			...
			List<String> resourceList = getResouceToServer(resChangeable, resStable);
	...
	public void writeWeapon(OutputStream out) throws Exception {
		...
		if (this.wId != null && this.wId > 0) {//base info
			...
			out.write(BinaryUtil.toByta(this.wCritAvailable));//爆头伤害倍数
			...
			if (this.wId < Constants.WEAPON_KNIFE_ID) {
				...
				if (this.wId == Constants.WEAPON_PISTOL_ID || this.wId == Constants.WEAPON_DOUBLE_PISTOL_ID) {// pistol
					...
					if (wId == Constants.WEAPON_MACHINE_GUN_ID) {//机枪
						...
						//float fly_speed[3];
						out.write(BinaryUtil.toByta(wFlySpeed)); // fly_speed[0]
						out.write(BinaryUtil.toByta(wUpModifier)); // fly_speed[1]
						out.write(BinaryUtil.toByta(wAccuracyTime)); // fly_speed[2]
						//float fly_speed_multiple[3];
						out.write(BinaryUtil.toByta(wAccuracyTimeModefier)); // fly_speed_multiple[0]
						out.write(BinaryUtil.toByta(wMaxAccuracy)); // fly_speed_multiple[1]
						out.write(BinaryUtil.toByta(wMinAccuracy)); // fly_speed_multiple[2]
						...
						out.write(BinaryUtil.toByta(range));
				...
				} else if (wId == Constants.WEAPON_MEDITNEEDLE_GUN_ID || wId == Constants.WEAPON_ROCKET_LUNCHER_ID || wId == Constants.WEAPON_BOW_ID || wId == Constants.WEAPON_ARROW_ID
						|| wId == Constants.WEAPON_BOSSPVE_ID) {//投射类
					out.write(BinaryUtil.toByta(wAmmoType));
					...
					out.write(BinaryUtil.toByta(wStabTime));
					out.write(BinaryUtil.toByta(wStabLightTime));
					...
					out.write(BinaryUtil.toByta(wStabHurt));
					out.write(BinaryUtil.toByta(wStabLightHurt));
	...
	public void initGunProperty() {
		...
		if (Constants.NUM_ONE == this.isStrengthen) {
			...
			//			} else if (strengthLevel >= 10 && strengthLevel < 13) {
			//				color = GunProperty.PURPLE;
			//			} else if (strengthLevel >= 13 && strengthLevel <= 15) {
			//				color = GunProperty.ORANGE;
			//			} else if (strengthLevel > 15){
			//				color = GunProperty.COLORFUL;
			//			}
			color = WSysConfigService.getPlayerItemLevelColor().entrySet().stream().filter(p -> p.getKey() <= strengthLevel).max((p1, p2) -> p1.getKey() - p2.getKey()).map(p -> p.getValue())
	...
	//仅仅为了 系统装满有默认强化等级时能显示正确
	private Float showDamage;
	private Float showShootSpeed;
	//zlm_7_15装备属性start================================================================================
	private float cResistanceFire_add_game;
	private float cResistanceBlast_add_game;
	private float cResistanceBullet_add_game;
	private float cResistanceKnife_add_game;
	private float cBloodAdd_add_game;
	//
	private float showDamage_game;
	private float showShootSpeed_game;
	//
	private float damangeAdd_game;
	private float shootSpeedAdd_game;

	public float getCResistanceFire_add_game() {
		return cResistanceFire_add_game;
	}

	public void setCResistanceFire_add_game(float resistanceFire_add_game) {
		cResistanceFire_add_game = resistanceFire_add_game;
	}

	public float getCResistanceBlast_add_game() {
		return cResistanceBlast_add_game;
	}

	public void setCResistanceBlast_add_game(float resistanceBlast_add_game) {
		cResistanceBlast_add_game = resistanceBlast_add_game;
	}

	public float getCResistanceBullet_add_game() {
		return cResistanceBullet_add_game;
	}

	public void setCResistanceBullet_add_game(float resistanceBullet_add_game) {
		cResistanceBullet_add_game = resistanceBullet_add_game;
	}

	public float getCResistanceKnife_add_game() {
		return cResistanceKnife_add_game;
	}

	public void setCResistanceKnife_add_game(float resistanceKnife_add_game) {
		cResistanceKnife_add_game = resistanceKnife_add_game;
	}

	public float getCBloodAdd_add_game() {
		return cBloodAdd_add_game;
	}

	public void setCBloodAdd_add_game(float bloodAdd_add_game) {
		cBloodAdd_add_game = bloodAdd_add_game;
	}

	public float getShowDamage_game() {
		return showDamage_game;
	}

	public void setShowDamage_game(float showDamage_game) {
		this.showDamage_game = showDamage_game;
	}

	public float getShowShootSpeed_game() {
		return showShootSpeed_game;
	}

	public void setShowShootSpeed_game(float showShootSpeed_game) {
		this.showShootSpeed_game = showShootSpeed_game;
	}

	public float getDamangeAdd_game() {
		return damangeAdd_game;
	}

	public void setDamangeAdd_game(float damangeAdd_game) {
		this.damangeAdd_game = damangeAdd_game;
	}

	public float getShootSpeedAdd_game() {
		return shootSpeedAdd_game;
	}

	public void setShootSpeedAdd_game(float shootSpeedAdd_game) {
		this.shootSpeedAdd_game = shootSpeedAdd_game;
	}

	//zlm_7_15装备属性end==========
	public Float getShowDamage() {
		return showDamage;
	}

	public void setShowDamage(Float showDamage) {
		this.showDamage = showDamage;
	}

	public Float getShowShootSpeed() {
		return showShootSpeed;
	}

	public void setShowShootSpeed(Float showShootSpeed) {
		this.showShootSpeed = showShootSpeed;
	}
	...
	private void calculateAppendStr4Des() {
		showDamage = getDamange();
		showShootSpeed = getShootSpeed();
		if (getIsStrengthen() != 0 && this.getStrengthLevel() > 0) {
			this.cResistanceFire_add = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0)) * 100 * (1 + 0.01 * this.getCResistanceFire()));
			this.cResistanceBlast_add = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0)) * 100 * (1 + 0.01 * this.getCResistanceBlast()));
			this.cResistanceBullet_add = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0)) * 100 * (1 + 0.01 * this.getCResistanceBullet()));
			this.cResistanceKnife_add = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0)) * 100 * (1 + 0.01 * this.getCResistanceKnife()));
			this.cBloodAdd_add = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 1)) * 100 * (1 + 0.01 * this.getCBloodAdd()));
			//zlm_7_15装备属性start================================================================================
			this.cResistanceFire_add_game = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 10)) * 100 * (1 + 0.01 * this.getCResistanceFire()));
			this.cResistanceBlast_add_game = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 10)) * 100 * (1 + 0.01 * this.getCResistanceBlast()));
			this.cResistanceBullet_add_game = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 10)) * 100 * (1 + 0.01 * this.getCResistanceBullet()));
			this.cResistanceKnife_add_game = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 10)) * 100 * (1 + 0.01 * this.getCResistanceKnife()));
			this.cBloodAdd_add_game = (float) ((ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 11)) * 100 * (1 + 0.01 * this.getCBloodAdd()));
			//zlm_7_15装备属性end==========

			double damageOffset = 1;
			double shootSpeedOffset = 1;
			damageOffset = 1 + ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0);
			shootSpeedOffset = 1 + ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 1);
			this.showDamage = (float) (((int) (getWDamage() * damageOffset) + 50) / 2);
			this.showShootSpeed = (float) ((1 / getWFireTime() * shootSpeedOffset) * 10);

			//zlm_7_15装备属性start================================================================================
			double damageOffset_game = 1;
			double shootSpeedOffset_game = 1;
			damageOffset_game = 1 + ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 0);
			shootSpeedOffset_game = 1 + ServiceLocator.getService.getStrengthenProperty(this, getStrengthLevel() - 1, 1);
			this.showDamage_game = ((int) (getWDamage() * damageOffset_game) + 50) / 2;
			this.showShootSpeed_game = (float) ((1 / getWFireTime() * shootSpeedOffset_game) * 10);
			//zlm_7_15装备属性end==========
		}
	}
	...
	private void calculateParam() {
		...
		if (this.subType != 3) {
			if (wDamage == 0) {
				...
				this.damange = (wDamage + 50) / 2;
			}
		} else {
			if (wStabHurt == 0) {
				this.damange = 0;//weapon
			} else {
				this.damange = (wStabHurt + 50) / 2;
			}
		}
		this.damange = CommonUtil.toTwoFloat(damange);
		//shoot (1/firetime)*10
		if (this.subType != 3) {
			if (wFireTime == 0) {
				this.shootSpeed = 0;
			} else {
				this.shootSpeed = (1 / wFireTime) * 10;
			}
		} else {
			if (wStabTime == 0) {
				this.shootSpeed = 0;
			} else {
				this.shootSpeed = (1 / wStabTime) * 10;
			}
		}
		float k = CommonUtil.toTwoFloat(shootSpeed);
		this.shootSpeed = k;
		this.weight = 0;
		this.range = 0;
		this.stopPpower = 0;
		if (this.strengthLevel > 0) {
			this.damangeAdd = (float) (this.damange * ServiceLocator.getService.getStrengthenProperty(this, level - 1, 0));
			this.shootSpeedAdd = (float) (this.damange * ServiceLocator.getService.getStrengthenProperty(this, level - 1, 1));
		}
		//zlm_7_15装备属性start================================================================================
		if (this.strengthLevel > 0) {
			this.damangeAdd_game = (float) (this.damange * ServiceLocator.getService.getStrengthenProperty(this, level - 1, 10));
			this.shootSpeedAdd_game = (float) (this.damange * ServiceLocator.getService.getStrengthenProperty(this, level - 1, 11));
		}
		//zlm_7_15装备属性end==========
	}
	...
	private Float cResistanceFire_add = 0F;
	private Float cResistanceBlast_add = 0F;
	private Float cResistanceBullet_add = 0F;
	private Float cResistanceKnife_add = 0F;
	private Float cBloodAdd_add = 0F;
	...
	private List<SysItem> packages = new ArrayList<>();
	...
	private List<Payment> resPricesList = new ArrayList<Payment>();
	private List<Payment> resDisPricesList = new ArrayList<Payment>();
	private List<Payment> resTeamPricesList = new ArrayList<Payment>();
	private List<Payment> resDisTeamPricesList = new ArrayList<Payment>();
	private List<Payment> reviveCoinList = new ArrayList<Payment>();
	//20160401zlm start
	private List<Payment> achipPricesList = new ArrayList<Payment>();
	private List<Payment> bchipPricesList = new ArrayList<Payment>();
	private List<Payment> cchipPricesList = new ArrayList<Payment>();

	public List<Payment> getAchipPricesList() {
		return achipPricesList;
	}

	public void setAchipPricesList(List<Payment> achipPricesList) {
		this.achipPricesList = achipPricesList;
	}

	public List<Payment> getBchipPricesList() {
		return bchipPricesList;
	}

	public void setBchipPricesList(List<Payment> bchipPricesList) {
		this.bchipPricesList = bchipPricesList;
	}

	public List<Payment> getCchipPricesList() {
		return cchipPricesList;
	}

	public void setCchipPricesList(List<Payment> cchipPricesList) {
		this.cchipPricesList = cchipPricesList;
	}
	...
	private List<Payment> allResPricesList = new ArrayList<Payment>();
	private List<Payment> allResDisPricesList = new ArrayList<Payment>();
	private List<Payment> allResTeamPricesList = new ArrayList<Payment>();
	private List<Payment> allResDisTeamPricesList = new ArrayList<Payment>();
	//20160401zlm start
	private List<Payment> allAchipPricesList = new ArrayList<Payment>();
	private List<Payment> allBchipPricesList = new ArrayList<Payment>();
	private List<Payment> allCchipPricesList = new ArrayList<Payment>();

	public List<Payment> getAllAchipPricesList() {
		return allAchipPricesList;
	}

	public void setAllAchipPricesList(List<Payment> allAchipPricesList) {
		this.allAchipPricesList = allAchipPricesList;
	}

	public List<Payment> getAllBchipPricesList() {
		return allBchipPricesList;
	}

	public void setAllBchipPricesList(List<Payment> allBchipPricesList) {
		this.allBchipPricesList = allBchipPricesList;
	}

	public List<Payment> getAllCchipPricesList() {
		return allCchipPricesList;
	}

	public void setAllCchipPricesList(List<Payment> allCchipPricesList) {
		this.allCchipPricesList = allCchipPricesList;
	}
	...
	private float teamOccupyLength;
	private float teamOccupyWidth;
	private int needTeamPlaceLevel;
	private float teamItemUpgradePrice;

	private int timeForBuildingUp;//建筑物升级所需的时间
	//	private int timeForBuild;//建筑物建造所需的时间

	private int timeForCreate;//建筑物生产所需的时间，分钟

	private int buyRecordTimes; //购买次数 ，仅限资源争夺战兑换使用，不存库

	private SysSuit belongSuit; //所属套装，不存库

	/**
	 * 分钟
	 * @return
	 */
	public int getTimeForCreate() {
		return timeForCreate;
	}

	public long getTimeForCreateMsec() {
		return getTimeForCreate() * 60 * 1000l;
	}

	public void setTimeForCreate(int timeForCreate) {
		this.timeForCreate = timeForCreate;
	}

	public int getTimeForBuildingUp() {
		return timeForBuildingUp;
	}

	public void setTimeForBuildingUp(int timeForBuildingUp) {
		this.timeForBuildingUp = timeForBuildingUp;
	}

	public float getTeamOccupyLength() {
		return teamOccupyLength;
	}

	public void setTeamOccupyLength(float teamOccupyLength) {
		this.teamOccupyLength = teamOccupyLength;
	}

	public float getTeamOccupyWidth() {
		return teamOccupyWidth;
	}

	public void setTeamOccupyWidth(float teamOccupyWidth) {
		this.teamOccupyWidth = teamOccupyWidth;
	}
	...
	public void setNeedTeamPlaceLevel(int needTeamPlaceLevel) {
		...
		if (this.getType() == Constants.DEFAULT_WEAPON_TYPE) {
			if (this.getSubType() == Constants.NUM_ONE) {// 主武器
				...
				figthNumOutput = (int) (Constants.FIGHT_PARAM[0][0]
						* Math.pow(Constants.FIGHT_PARAM[0][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[0][2]) - Constants.FIGHT_PARAM[0][3]) * multiplier3);
				...
				figthNumOutput = (int) (Constants.FIGHT_PARAM[5][0]
						* Math.pow(Constants.FIGHT_PARAM[5][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[5][2]) - Constants.FIGHT_PARAM[5][3]) * multiplier3);
				...
				figthNumOutput = (int) (Constants.FIGHT_PARAM[1][0]
						* Math.pow(Constants.FIGHT_PARAM[1][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[1][2]) - Constants.FIGHT_PARAM[1][3]) * multiplier3);
			}
			...
			figthNumOutput = (int) (Constants.FIGHT_PARAM[2][0] * ((Math.pow(Constants.FIGHT_PARAM[2][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[2][2])
			...
			if (this.getSubType() == Constants.NUM_ONE) {// 帽子
				...
				figthNumOutput = (int) (Constants.FIGHT_PARAM[3][0] * ((Math.pow(Constants.FIGHT_PARAM[3][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[3][2])
				...
				figthNumOutput = (int) (Constants.FIGHT_PARAM[4][0] * ((Math.pow(Constants.FIGHT_PARAM[4][1], this.strengthLevel + ((this.rareLevel - 1) / Constants.FIGHT_PARAM[4][2])
	...
	public int getDefaultPropertySize() {
		return (this.gunPropertyList2.size() / 4 + this.gunPropertyList3.size() / 4 + this.gunPropertyList4.size() / 4 + this.gunPropertyList5.size() / 4 + this.gunPropertyList6.size() / 4 + this.gunPropertyList7
				.size() / 4);
	}
	...
	public SysItem clone(boolean deep) throws CloneNotSupportedException {
		SysItem sysitem = this.clone();
		if (deep) {

			//深度克隆其中一个集合对象，如果以后有需要则在clone其他对象
			if (allResPricesList.size() > 0) {
				sysitem.allResPricesList = new ArrayList<Payment>();
				for (Payment payment : allResPricesList) {
					sysitem.allResPricesList.add(payment.clone());
				}
			}

			//深度克隆其中第二个一个集合对象，如果以后有需要则在clone其他对象
			if (resPricesList.size() > 0) {
				sysitem.resPricesList = new ArrayList<Payment>();
				for (Payment payment : resPricesList) {
					sysitem.resPricesList.add(payment.clone());
				}
			}

			//深度克隆其中第三个一个集合对象，如果以后有需要则在clone其他对象
			if (crPricesList.size() > 0) {
				sysitem.crPricesList = new ArrayList<Payment>();
				for (Payment payment : crPricesList) {
					sysitem.crPricesList.add(payment.clone());
				}
			}
		}
		return sysitem;
	...
	public Payment getDefaultPayment() {
		if (null == defaultPayment) {
			...
			if (null != this.getAllGpPricesList() && this.getAllGpPricesList().size() > 0) {
				...
				//20160401zlm start
			} else if (null != this.getAllAchipPricesList() && this.getAllAchipPricesList().size() > 0) {
				unitType = this.getAllAchipPricesList().get(0).getUnitType();
				unit = this.getAllAchipPricesList().get(0).getUnit();
			} else if (null != this.getAllBchipPricesList() && this.getAllBchipPricesList().size() > 0) {
				unitType = this.getAllBchipPricesList().get(0).getUnitType();
				unit = this.getAllBchipPricesList().get(0).getUnit();
			} else if (null != this.getAllCchipPricesList() && this.getAllCchipPricesList().size() > 0) {
				unitType = this.getAllCchipPricesList().get(0).getUnitType();
				unit = this.getAllCchipPricesList().get(0).getUnit();
				//20160401zlm end
			}
	...
	public int getIsBuff() {
		return (this.type == 4 && this.iId == 1) ? 1 : 0;
	}
	...
	public Float getCResistanceFire_add() {
		return cResistanceFire_add;
	}

	public void setCResistanceFire_add(Float cResistanceFire_add) {
		this.cResistanceFire_add = cResistanceFire_add;
	}

	public Float getCResistanceBlast_add() {
		return cResistanceBlast_add;
	}

	public void setCResistanceBlast_add(Float cResistanceBlast_add) {
		this.cResistanceBlast_add = cResistanceBlast_add;
	}

	public Float getCResistanceBullet_add() {
		return cResistanceBullet_add;
	}

	public void setCResistanceBullet_add(Float cResistanceBullet_add) {
		this.cResistanceBullet_add = cResistanceBullet_add;
	}

	public Float getCResistanceKnife_add() {
		return cResistanceKnife_add;
	}

	public void setCResistanceKnife_add(Float cResistanceKnife_add) {
		this.cResistanceKnife_add = cResistanceKnife_add;
	}

	public Float getCBloodAdd_add() {
		return cBloodAdd_add;
	}

	public void setCBloodAdd_add(Float cBloodAdd_add) {
		this.cBloodAdd_add = cBloodAdd_add;
	}

	private List<String> getResouceToServer(String[] resChangeable, String[] resStable) {
		List<String> tmpList = new ArrayList<String>();
	...
	public int getBuyRecordTimes() {
		return buyRecordTimes;
	}

	public void setBuyRecordTimes(int buyRecordTimes) {
		this.buyRecordTimes = buyRecordTimes;
	}

	public SysSuit getBelongSuit() {
		return belongSuit;
	}

	public void setBelongSuit(SysSuit belongSuit) {
		this.belongSuit = belongSuit;
	}
	 ...
	 * @return long
	 */
	public long getGst_level_exp() {
		return Constants.getGst_Level_Exp(getRareLevel());
	}

	//zlm2015-5-7-限时装备-开始
	private Boolean provisional_item_flag;//是否是限时装备 flase不是限时装备 true是限时装备

	public boolean getProvisional_item_flag() {
		if (this.provisional_item_flag == null) {
			provisional_item_flag = CommonUtil.provisional_item_scope_flag(this.getId());
		}
	...
	public String toString() {
		return this.getClass().getSimpleName() + " : " + this.displayNameCN + " : id=" + this.id;
	}
	...
	public int getIsFlashSale() {
		Date now = new Date();
		boolean b = shoppingType == EItemShoppingType.FLASH_SALE.getNumber() && DateUtil.after(now, shoppingStartTime, Calendar.MILLISECOND, 1)
				&& DateUtil.before(now, shoppingEndTime, Calendar.MILLISECOND, 1);
		return b ? 1 : 0;
	}
}
