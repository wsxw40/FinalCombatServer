...
//				si.init();
				if(i==0){
					if(hasSuitNow){
						//FCW 3改6 if(suitNum==3){
						if (4 < suitNum && suitNum <= 6) {
							pi.writeCostume(out,si,sysCharacter,new int[]{suitId,4});
							//FCW 1改4}else if(suitNum>0&& suitNum<3){
						} else if (suitNum == 4) {
...
//					si.init();
	...
	public boolean checkHasSuitNow(){
		...
		//FCW 页游套装只依赖NeedTeamPlaceLevel start
		Map<Integer, Integer> map = new HashMap<>();
		weaponList.forEach(p -> {
			int tmpSuitId = p.getSysItem().getNeedTeamPlaceLevel();
			if (tmpSuitId > 99) {
				map.put(tmpSuitId, map.getOrDefault(tmpSuitId, 0) + 1);
			}
		});
		costumeList.forEach(p -> {
			int tmpSuitId = p.getSysItem().getNeedTeamPlaceLevel();
			if (tmpSuitId > 99) {
				map.put(tmpSuitId, map.getOrDefault(tmpSuitId, 0) + 1);
			}
		});
		suitId = map.entrySet().stream().max((p1, p2) -> p1.getValue() - p2.getValue()).map(p -> p.getKey()).orElse(0);//正在装备的最多的套装ID
		suitNum = map.getOrDefault(suitId, 0);//正在装备的最多的套装的数量
		if (suitNum >= 4) {
			return true;
		}
		//FCW 页游套装只依赖NeedTeamPlaceLevel end
