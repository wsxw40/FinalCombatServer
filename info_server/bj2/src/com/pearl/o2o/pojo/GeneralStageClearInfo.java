...
//	}
	...
	public int getPlayerCount() {
		int count = 0;
		count += null == stageClearPlayerInfoTeam0 ? 0 : stageClearPlayerInfoTeam0.size();
		count += null == stageClearPlayerInfoTeam1 ? 0 : stageClearPlayerInfoTeam1.size();
		count += null == stageClearPlayerInfoTeam2 ? 0 : stageClearPlayerInfoTeam2.size();
		return count;
	}
	 ...
	 * @return
	 */
	public StageClearPlayerInfo getStageClearPlayerIfno(int playerId, boolean isRobot) {
		StageClearPlayerInfo m = stageClearPlayerInfoTeam0.stream().filter(p -> {
			return p.getId() == playerId && (isRobot ? p.getRobotId() > 0 : p.getRobotId() == 0);
		}).findFirst().orElse(null);
		if (null == m) {
			m = stageClearPlayerInfoTeam1.stream().filter(p -> {
				return p.getId() == playerId && (isRobot ? p.getRobotId() > 0 : p.getRobotId() == 0);
			}).findFirst().orElse(null);
		}
		if (null == m) {
			m = stageClearPlayerInfoTeam2.stream().filter(p -> {
				return p.getId() == playerId && (isRobot ? p.getRobotId() > 0 : p.getRobotId() == 0);
			}).findFirst().orElse(null);
		}
		return m;
