...
//	private	Integer		credit 			= 0;
	...
	private static  List<SysMatchP> sysMatchPList=null;
	private Integer pvalue; // P值进行匹配

	public Integer getPvalue(){
		//原先通过积分换算p值的,改成保存在玩家身上的
		if (pvalue != null)	return pvalue;
		int maxValue = 0;//最大战斗力值
		try {
			//FCW 无工程兵
			for (int i = 0; i < 6; i++) {//得到最大战斗力值
				Integer fightNum = ServiceLocator.getService.getCharacterByCharacterId(id, i + 1).getFightNum();
				maxValue = Math.max(fightNum, maxValue);
			}
			if (sysMatchPList == null)
				sysMatchPList = ServiceLocator.getService.getSysMatchPList();
			for (SysMatchP sysMatchP : sysMatchPList) {
				if (maxValue <= sysMatchP.getScore())
					this.pvalue=sysMatchP.getId();
					return sysMatchP.getId();
			}
		} catch (Exception e) {
			//e.printStackTrace();
			this.pvalue=1;
	...
	public String getMatchWin() {
		if (matchWin==null)
			this.matchWin = "1010101010";
		if (matchWin.length() != 10)
			this.matchWin = "1010101010";
		return matchWin;
	}
	...
	public Integer getMatchPValue() {
		Integer count=0;
		for (int i = 0; i < getMatchWin().length(); i++) {
			if(getMatchWin().charAt(i)=='1'){
				count++;
			}
		}
		return count;
	...
	public Integer getMvalue() {
		return mvalue==null? 1:mvalue;
	}

	/**
	 * 功勋快速匹配数值算法, 每个角色的P值=玩家功勋值/修正系数 每50点功勋值折算成一点P值。
	 * @return
	 */
	public Integer getMvalueToRatio() {
		return (int)((getMvalue()+1)/50);
	}

	public void setMvalue(Integer mvalue) {
		this.mvalue =mvalue>=5000? 5000:mvalue;
	}
