...
//		randomArray[random.nextInt(totalWeight)];
	 ...
	 * 返回战斗力对应的P值
	 */
	public static Integer get_Pvalue(Player player) throws Exception {
		int maxValue = 0;
		for (int i = 0; i < 7; i++) {
			Integer fightNum = ServiceLocator.getService
					.getCharacterByCharacterId(player.getId(), i + 1)
					.getFightNum();
		...
		for (SysMatchP sysMatchP : sysMatchPList) {
			if (maxValue <= sysMatchP.getScore())
				return sysMatchP.getId() + player.getMatchPValue();
		}
		return player.getMatchPValue();
	}
	//zlm2015-10-9-匹配-结束
}