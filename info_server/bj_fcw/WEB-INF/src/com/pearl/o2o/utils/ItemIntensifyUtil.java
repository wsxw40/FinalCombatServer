...
public class ItemIntensifyUtil {
	...
	public static int getIntensifyPrice(int currency, int itemLevel,
		...
		if (si.getSubType() == TEAM_STORAGE_TYPE.DEFENSE_TOWER.getValue()) {
			return getTeamItemIntensifyPriceForAttack(currency, curLevel
					+ (itemLevel - 1) * TEAM_ATTACK_MAX_UP_LEVEL,
					intensifyType, si.getWId());
		} else if (si.getSubType() == TEAM_STORAGE_TYPE.WALL.getValue()) {
			return getTeamItemIntensifyPriceForWall(currency, curLevel
					+ (itemLevel - 1) * TEAM_WALL_MAX_LEVEL, intensifyType);
		} else if (si.getSubType() == TEAM_STORAGE_TYPE.OIL_TANK.getValue()) {
			return getTeamItemIntensifyPriceForOilTank(currency, curLevel
					+ (itemLevel - 1) * TEAM_OIL_TANK_LEVEL, intensifyType);
		} else if (si.getSubType() == TEAM_STORAGE_TYPE.PERSONAL_TANK
				.getValue()) {// 个人坦克
			return getPersonalIntensifyPriceForTank(currency, curLevel
					+ (itemLevel - 1) * PERSONAL_ITEM_MAX_LEVEL, intensifyType);
		} else {
			throw new Exception();
		}
	}
	...
	public static class ConditionForPlaceUp {
		...
		public static int checkIntensifyLevel(TeamItem ti, int level)
				throws NumberFormatException, Exception {
			if (ti == null || ti.getShowQuantity() <= 0) {
				return 0;
			} else {
				boolean p1 = ti.getProperty1().get("level").intValue()
						+ (ti.getLevel() - 1)
						* getMaxLevel(ti.getSysItem().getSubType()) >= level;
				boolean p2 = ti.getProperty2().get("level").intValue()
						+ (ti.getLevel() - 1)
						* getMaxLevel(ti.getSysItem().getSubType()) >= level;
				boolean p3 = ti.getProperty3().get("level").intValue()
						+ (ti.getLevel() - 1)
						* getMaxLevel(ti.getSysItem().getSubType()) >= level;
				boolean p4 = ti.getProperty4().get("level").intValue()
						+ (ti.getLevel() - 1)
						* getMaxLevel(ti.getSysItem().getSubType()) >= level;
				boolean p5 = ti.getProperty5().get("level").intValue()
						+ (ti.getLevel() - 1)
						* getMaxLevel(ti.getSysItem().getSubType()) >= level;
				if (p1 && p2 && p3 && p4 && p5) {
					return level;
	...
	public static Map<String, Number> getPropertyBasic(String sGunProperty,
		...
		if(sysItem.getType()!=Constants.DEFAULT_ZYZDZ_TYPE){//非资源争夺战不做次处理
			return map;
		}
		
		if (sGunProperty != null && sGunProperty.contains(",")) {
			String[] p = sGunProperty.split(",");
			if(p.length>=3){
				double curExp = Double.valueOf(p[1]);
				double maxExp = ItemIntensifyUtil.getIntensifyPrice(
						Constants.RES_PAY_TEAM, level, teamItemPropertyType
								.getType(), sysItem, Integer.valueOf(p[0]));
				double value = mathOperator.cal(baseValue, Double
						.valueOf(p[2]));
				if(value<=0){
					value=baseValue;
				}
				map.put("property", teamItemPropertyType.getType());
				map.put("level", Integer.valueOf(p[0]));
				map.put("value", value);
				map.put("curExp", curExp);
