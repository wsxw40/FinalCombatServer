...
//						+ Constants.XUNLEI_LOG_DELIMITER + playerInfo.getXunleiPoint()
						...
						+ Constants.XUNLEI_LOG_DELIMITER + mName
						+ Constants.XUNLEI_LOG_DELIMITER + sysChest.getNumber()
	 ...
	 * @throws Exception
	 */
	private void qw_info_jisuan(int playerId, StageClearPlayerInfo playerInfo,int s_f) throws Exception{
		NoSql nosql = nosqlService.getNosql();
		int vip=getService.getPlayerById(playerId).getIsVip();
		...
		qw_s = qw_s==null? "0|0|0|0|0|0|-1|-1|12|-1|-1|-1":qw_s;
		
		int[] ints_d=StringUtil.strsToInts(qw_d,"\\|"); 
		int[] ints_s=StringUtil.strsToInts(qw_s,9,"\\|"); 
		
		ints_d[1]+=playerInfo.getKill();//杀人
		ints_d[2]+=playerInfo.getDead();//死亡
		ints_d[3]+=playerInfo.getAssistNum();//助攻
		ints_s[1]+=playerInfo.getKill();//杀人
		ints_s[2]+=playerInfo.getDead();//死亡
		ints_s[3]+=playerInfo.getAssistNum();//助攻
		//胜负
		if (s_f==1) {
			ints_s[4]++;
			ints_d[4]++;
		...
		//判断k值
		if (ints_s[4] + ints_s[5] <= 10)
			ints_s[8] = 12 + ints_s[4] * 2 - ints_s[5];//K值
		//满10盘判断K值给积分
		if (ints_s[4]+ints_s[5]==10) {
			for (int i = 0; i < Constants.QW_K_RATIO.length; i++) {
				if(ints_s[8]<=Constants.QW_K_RATIO[i][0]){
					ints_s[0]=Constants.QW_K_RATIO[i][1];
					break;
				}
			}
		}
		...
		if (ints_s[4]+ints_s[5]>10) {
			...
			ints_s[0]=(playerInfo.getScore()+ints_s[0]);	//积分
			if (ints_s[0] <= 0)
				ints_s[0] = 1;// 积分
			//算排名=======================================
			//获得现在的排名，如果有就直接加入
			...
			int value = ints_s[0];
			if(rank==-1){
				int total = (int)nosql.zCard(QW_KEY_TOP);
				if(total<1000){//排名总人数小于1000就加入
					nosql.zAdd(QW_KEY_TOP, -value, String.valueOf(playerId));//加入
				}
				if(total>=1000){//排名总人数大于等于1000
					//就获得1000位的评分，如果大于就加入
					double lastValue = nosql.zrangeWithScores(QW_KEY_TOP, 999, 999).iterator().next().getScore();
					if(value > lastValue){
						nosql.zAdd(QW_KEY_TOP, -value, String.valueOf(playerId));
					}
				}
				if(total>=1500){//排名总人数大于等于1500，对总表清理一次多余的人
					Set<String> range = nosql.zRange(QW_KEY_TOP, 1000, 1599);
					for (String s : range) {
						nosql.zRem(QW_KEY_TOP, s);//删除
					}
				}
				rank = (int)nosql.zRank(QW_KEY_TOP, String.valueOf(playerId));//获得元素位置 -1为不存在，从0开始
			}else {//已经存在直接加入
				nosql.zAdd(QW_KEY_TOP, -value, String.valueOf(playerId));//加入
			}
			ints_s[7]=(int)rank+1;//最新排名
		}
		//大于10盘进行正常积分结算end=====================================================
		//算段位start====
		if(ints_s[4]+ints_s[5]>=10){
			for (int i = 0; i < Constants.QW_RATIO.length; i++) {
				if(ints_s[0]<Constants.QW_RATIO[i]){
					ints_s[6]=i;
					break;
				}
			}
		...
		//算段位end====
		ints_d[0]=ints_s[0];//积分
		ints_d[6]=ints_s[6];//段位
		ints_d[7]=ints_s[7];//排名
		ints_d[8]=ints_s[8];//k值
		ints_d[9]=ints_s[1];//当天的总杀人数
		
		nosql.set(QW_KEY_DAY, StringUtil.intsJoint(ints_d, "|"));
		nosql.set(QW_KEY_SUM, StringUtil.intsJoint(ints_s, "|")+"|"+playerInfo.getName()+"|"+playerInfo.getRank()+"|"+vip);// 第一次
	}
	...
	public void updatePlayerStageClear(final GeneralStageClearInfo stageClearInfo) throws Exception {
		...
		int iterator = 0;
		//================临时后面要去掉
		SysItem s_XZ 	= getService.getSysItemByItemId(125);	//125	强化部件
		SysItem s_QHBJ 	= getService.getSysItemByItemId(4479);	//4479	勋章
		SysItem s_5k 	= getService.getSysItemByItemId(4402);	//4402  5000C币
		SysItem s_10k 	= getService.getSysItemByItemId(4002);	//4002	1.0000
		SysItem s_20k 	= getService.getSysItemByItemId(4415);	//4415	2.0000
		List<SysItem> c_items = stageClearInfo.getItems();
		c_items.add(s_XZ);
		c_items.add(s_QHBJ);
		c_items.add(s_5k);
		c_items.add(s_10k);
...
//				int offset=updatePlayerActivity(Constants.ACTION_ACTIVITY.ONLINE_ACTIVITY.getValue(), player.getId(), null, 0, 0, null, null);
				...
				if(isMatch==6&&gameTpye==Constants.GAMETYPE.TEAM.getValue()){//判断是否是匹配团竞模式
					String mate_award_count_key=Constants.MATE_AWARD_COUNT+playerId;//key
					int cB=0;//C币
					int qhbjANDxz=0;//强化部件跟勋章//2个同增，所以先写一起
					int CB_SIZE=200000;
					Object obj = mcc.get(mate_award_count_key);
					if (obj!=null) {
						String[] split = obj.toString().split(Constants.DELIMITER);
						if(split.length==2){
							cB=Integer.valueOf(split[0]);
							qhbjANDxz=Integer.valueOf(split[1]);
						}
					}
					...
					if (getScore > 0 && getScore <= 1000) {
						if (cB<CB_SIZE) {
							int r_CB=200000-cB;//还可以拿多少
							if (r_CB>=25000) {//可以拿到上限
								itemkv.put(s_5k, new Payment(1, 1));
								itemkv.put(s_10k, new Payment(2, 1));
								cB+=25000;
							}else  {//拿不到上限
								itemkv.put(s_5k, new Payment(r_CB/5000, 1));
								cB+=r_CB;
							}
						}
						if (qhbjANDxz<15) {
							int count=15-qhbjANDxz>=2? 2:15-qhbjANDxz;
							itemkv.put(s_QHBJ, new Payment(count, 1));
							itemkv.put(s_XZ, new Payment(count, 1));
							qhbjANDxz+=count;
						}
					}else if (getScore > 1000 && getScore <4000) {
						if (cB<CB_SIZE) {
							int r_CB=200000-cB;//还可以拿多少
							if (r_CB>=40000) {//可以拿到上限
								itemkv.put(s_5k, new Payment(2, 1));
								itemkv.put(s_10k, new Payment(3, 1));
								cB+=40000;
							}else  {//拿不到上限
								itemkv.put(s_5k, new Payment(r_CB/5000, 1));
								cB+=r_CB;
						...
						if (qhbjANDxz<15) {
							int count=15-qhbjANDxz>=3? 3:15-qhbjANDxz;
							itemkv.put(s_QHBJ, new Payment(count, 1));
							itemkv.put(s_XZ, new Payment(count, 1));
							qhbjANDxz+=count;
						}
						...
						if (cB<CB_SIZE) {
							int r_CB=200000-cB;//还可以拿多少
							if (r_CB>=70000) {//可以拿到上限
								itemkv.put(s_5k, new Payment(2, 1));
								itemkv.put(s_20k, new Payment(3, 1));
								cB+=70000;
							}else  {//拿不到上限
								itemkv.put(s_5k, new Payment(r_CB/5000, 1));
								cB+=r_CB;
						...
						if (qhbjANDxz<15) {
							int count=15-qhbjANDxz>=5? 5:15-qhbjANDxz;
							itemkv.put(s_QHBJ, new Payment(count, 1));
							itemkv.put(s_XZ, new Payment(count, 1));
							qhbjANDxz+=count;
						}
					...
					mcc.set(mate_award_count_key, day_surplus_time, cB
							+ Constants.DELIMITER + qhbjANDxz);
					log.error("hou"+new Date().toString()+" "+mate_award_count_key+""+cB + Constants.DELIMITER + qhbjANDxz );
				...
				if(gameTpye==Constants.GAMETYPE.RISK_ISLET.getValue()){//判断是否是生存模式
					...
					if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
						nosqlService.addXunleiLog("19.1"
							+ Constants.XUNLEI_LOG_DELIMITER + playerInfo.getId()//玩家帐号ID
		...
		if(stageClearInfo.getTotalRes()==-1){//挑战模式
			...
			if(attTeam==null){
				updateZYZDZPlayerStageClear(stageClearInfo.getStageClearPlayerInfoTeam0(),stageClearInfo.getStageClearPlayerInfoTeam1(),
						hostId,capGetRes,memGetRes,0,1,defTeam.getTeamSpaceLevel(),1);
...
//			if(attTeamId==stageClearInfo.getTeamaId()){
			...
			if(attTeam==null){
				updateZYZDZPlayerStageClear(stageClearInfo.getStageClearPlayerInfoTeam0(),stageClearInfo.getStageClearPlayerInfoTeam1(),
						hostId,attCaptainRes,attMemberRes,defMemberRes,1,defTeam.getTeamSpaceLevel(),0);
			}else{
				updateZYZDZPlayerStageClear(stageClearInfo.getStageClearPlayerInfoTeam0(),stageClearInfo.getStageClearPlayerInfoTeam1(),
...
//			}
	 ...
	 */
	private float getTeam_Res_ratio(int attTeamId,int teamSpaceLevel,
			float attTeamRealRes) throws TimeoutException,
		...
		for(StageClearPlayerInfo playerInfo: attInfos){
			...
			if(playerId==hostId){
				if(type==1){
					...
					updatePlayerRobRes(playerId,(int)(attCaptainRes*player_Res_ratio),playerInfo,attTeamSpaceLevel);
				}
			}else{
				//获得系数
				float player_Res_ratio = getPlayer_Res_ratio(attTeamSpaceLevel, playerId, attMemberRes);
				updatePlayerRobRes(playerId,(int)(attMemberRes*player_Res_ratio),playerInfo,attTeamSpaceLevel);
			}
			//战队贡献
	 ...
	 * @throws MemcachedException
	 */
	private float getPlayer_Res_ratio(int attTeamSpaceLevel,
			int playerId, int att_player_Res) throws TimeoutException,
			InterruptedException, MemcachedException {
		float player_Res_ratio=1;//收益系数 //要返回的
		int player_Res_ratio_status=1;//收益状态 1，为正常  2，为收益0.5 3.为收益零
		int player_RealRes_int=(int)att_player_Res;//本局资源
	 ...
	 * @throws Exception
	 */
	public void dealZyzdzTeamExp(StageClearPlayerInfo playerInfo,int isTeamAdd ) throws Exception{
		long fightTime = playerInfo.getEnd().getTime()-playerInfo.getStart().getTime();
		...
		if(playerTeam!=null){
			...
			if(playerInfo.getIsWiner()==1){	//该玩家赢得比赛
				this.updatePlayerTeamAndSetPlayerInfo(playerTeam, playerInfo, dayMaxForTeam, Constants.ZYZDZ_TEAMEXP_TEAM_WIN,1,isTeamAdd);
			}else if(playerInfo.getIsWiner()==0){//该玩家输掉
				this.updatePlayerTeamAndSetPlayerInfo(playerTeam,  playerInfo, dayMaxForTeam, Constants.ZYZDZ_TEAMEXP_TEAM_LOSE,1,isTeamAdd);
			}
	...
	public void updatePlayerStageQuit(int playerId, final GeneralStageClearInfo stageClearInfo,int isMatch) throws Exception {
		...
		for (final StageClearPlayerInfo playerInfo : stateClearPlayerInfo) {
			...
			if (player.getIsVip() >= 1 && gameType.getValue() != Constants.GAMETYPE.NEWTRAIN.getValue()) {
				...
				/** update battle fields for player and team */
				//中途退出,又是VIP，所以有奖励 
				setPlayerInfoStageClear(player, playerInfo, gameType, false,isMatch);
...
//					boostKillNum += ci.getBoostKill();
				...
				//zlm2015-10-9-匹配-开始 
				//中途退出而且还不是vip,所有奖励都没
				System.out.println(playerInfo.getIsWiner()+"应该是胜利的标准"+"中途退出而且还不是vip");
				System.out.println(player.getMatchWin()+"qian "+"中途退出而且还不是vip");
				//boolean isMatch=true;
				if (isMatch==6 ) //匹配模式，中途退出
					player.setMatchWin(player.getMatchWin().substring(1)+"0");
				System.out.println(player.getMatchWin()+"hou  "+"中途退出而且还不是vip");
				//zlm2015-10-9-匹配-结束
	...
	private void addXuleiLogWhileSteageClear(Player player, GeneralStageClearInfo stageClearInfo, int getExp, int getGP, String stageStr, LevelInfo levelInfo, int getScore,StageClearPlayerInfo playerInfo)
		...
		ServiceLocator.nosqlService.addXunleiLog("4.3" + Constants.XUNLEI_LOG_DELIMITER + levelInfo.getDisplayNameCN() + Constants.XUNLEI_LOG_DELIMITER + levelInfo.getType()
				...
				+ playerInfo.getAssistNum() + Constants.XUNLEI_LOG_DELIMITER+stageClearInfo.getIsTeam() + Constants.XUNLEI_LOG_DELIMITER + playerInfo.getCostReliveCoin());
	}
	...
	public boolean renewPlayerItem(int playerId, int playerItemId, int type, int costId,boolean timeLimit) throws Exception {
		...
		String provisional_key=null;
		if(playerItem.getProvisional_item_flag()){
			provisional_key = Constants.PROVISIONAL_ITEM_FLAG_KEY + playerId + Constants.DELIMITER +  playerItem.getId();
			String flag = nosqlService.getNosql().get(provisional_key);
			if(flag == null || "".equals(flag)){//没值
				nosqlService.getNosql().set(provisional_key, "30");
				provisional_flag_int=30;
			}else {
				provisional_flag_int=Integer.valueOf(flag);
			}
			//30-7 结果21天  真
			//30-30 结果0天 真
			//21-30结果-9 假
			...
			if (provisional_flag_int<=-1) {
				//System.out.println("updateService:2883天数不够： "+provisional_flag_int);
				//throw new BaseException(ExceptionMessage.NOT_RENEW);//返回：你的装备续费天数不足
				throw new BaseException(ExceptionMessage.NOT_PROVISIONAL_ITRM_RENEW);
			}
		}
		//zlm2015-5-7-限时装备-结束-------------------------------------
		...
		if (payment.getPayType() == Constants.GP_PAY) {
			...
			if (cr < cost) {
				...
				ServiceLocator.updateService.addPlayerTrack(playerId, "", 0, 0, cost, DateUtil.getDf().format(new Date()),"1"+Constants.XUNLEI_LOG_DELIMITER+ playerItem.getSysItem().getId()+Constants.XUNLEI_LOG_DELIMITER+payment.getUnitType()+Constants.XUNLEI_LOG_DELIMITER+payment.getUnit());
			}
		...
		//zlm2015-5-7-限时装备-开始-------------------------------------
		if(playerItem.getProvisional_item_flag()){
			nosqlService.getNosql().set(provisional_key, provisional_flag_int.toString());
		}
		//zlm2015-5-7-限时装备-结束-------------------------------------
			...
			public void run() {
				...
				try {
					for (PlayerTeam tempPt : getService.getPlayerTeamByTeamId(team.getId())) {
						soClient.puchCMDtoClient(tempPt.getNickName(), CommonUtil.messageFormat(CommonMsg.TEAM_NUMBER_CHANGE));
					}
				} catch (Exception e) {
...
//		TeamUtils.updateTeamMemberNum(teamId, -1);
		...
		for (PlayerTeam tempPt : ptList) {
			...
			soClient.puchCMDtoClient(player.getName(), CommonUtil.messageFormat(CommonMsg.TEAM_NUMBER_CHANGE));
		}
	}
	...
	public void sendMission(Player player, boolean isDailySend, boolean isWeekSend) throws Exception {
		...
		if (isDailySend) {
			...
			for(int i =0 ;i<randomDailyMission.size();i++){
				...
				temp.setSysMissionId(m.getId());
				if (m.getType()==9){	//固定任务，难度固定为1
					temp.setTarget(m.getSimpleTarget());
				}else if (randomDailyMission.indexOf(m) == 2) {// 设置一个中等的任务
					temp.setTarget(m.getNormalTarget());
	...
	public int strengthenPlayerItem(int playerId, int playerItemId, int strengthenItemId, int safeItemId, int stableItemId,PlayerItem[] itemList,List<SysItem> failAwards) throws Exception {
		...
		case Constants.SUCCESS:
			...
			if (pi.getLevel() >= 7 && pi.getLevel() < 10) {
				...
				soClient.updateBillBoardList(CommonUtil.messageFormatI18N(CommonMsg.STRENGTH_16_SYS, new Object[] { (GunProperty.GOLD + "@!" +  player.getName()), (pi.getGunProperty().getColor() + "@!" +  si.getDisplayName()), (pi.getGunProperty().getColor() + "@!" +  pi.getLevel()) }));
				soClient.proxyBroadCast(Constants.MSG_NBA, Constants.SYSTEM_SPEAKER,
						CommonUtil.messageFormatI18N(CommonMsg.STRENGTH_16_SYS, new Object[] { (GunProperty.COLORFUL + "@!" +  player.getName()+"@!"+GunProperty.COLORFUL + "@!" + player.getId()) , (pi.getGunProperty().getColor() + "@!" +  si.getDisplayName()) , (pi.getGunProperty().getColor() + "@!" +  pi.getLevel()) }));
			}
			// 成长任务：强化到3-15级
...
//			ServiceLocator.insertLog.debug("\t" + playerId + "\t" + property + "\t");
		...
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			nosqlService.addXunleiLog("8.4"
					...
					+ Constants.XUNLEI_LOG_DELIMITER + si.getDisplayNameCN()
					+ Constants.XUNLEI_LOG_DELIMITER + mValue
					+ Constants.XUNLEI_LOG_DELIMITER + propertyItemOldNum
					+ Constants.XUNLEI_LOG_DELIMITER + (propertyItemOldNum-1)
					+ Constants.XUNLEI_LOG_DELIMITER+ CommonUtil.simpleDateFormat.format(new Date())
					);
		}	
			 ...
			 if(qualityProp!=null){qsi=sysItemMap.get(qualityProp.getItemId());}
				...
				if(playeritems.toString().matches("勋章")){
					...
					nosqlService.addXunleiLog("7.4"
							...
							+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + 1
							+ Constants.XUNLEI_LOG_DELIMITER + (chipNum-chipNum2)
							+ Constants.XUNLEI_LOG_DELIMITER + chipNum2
				...
				if(playeritems.toString().matches("复活币")){
					...
					nosqlService.addXunleiLog("18.1"
							...
							+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + 0
							+ Constants.XUNLEI_LOG_DELIMITER + (reliveCoinNum-reliveCoinNum2)
							+ Constants.XUNLEI_LOG_DELIMITER + reliveCoinNum2
...
//			item2=playerBuff.getSysItem();
	...
	public int updateVipUpExp(int type, int playerId, int... earnExp)
		...
		if (addedExp > 0) {
			...
			ServiceLocator.playerLog.info("VIP CURRENTY ./EXPERIENCE ./EXPERIENCE: "+preVipExp+" WELFARE: "+currVipExp+"("+getService.getPlayerById(playerId).getVipExp()+") PLAYER ID:" +player.getUserName() +" ADD EXPERIENCE: " + addedExp);
	...
	public void updateTeamRobRes(int teamId,float res)throws Exception{
		...
		if(team!=null){
			...
			ServiceLocator.zyzdzStageClearLog.info(String.format("zyzdz rersult : %s\t%s\t%s\t%s", 
	...
	public void onCardsWelfare(final Player player)throws Exception{
		...
		if(null!=pItem)
		{
			...
			if(!pItem.getExpireDate().after(new Date())){
				...
				//日志记录，月卡到期
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("14.2"
						+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
						+ Constants.XUNLEI_LOG_DELIMITER + pInfo.getXunleiPoint()
						+ Constants.XUNLEI_LOG_DELIMITER +  DateUtil.getTimeDifference(new Date(), pItem.getExpireDate())
						+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date())
						);
					}
			...
			if(null != pInfo && null != item)
			{
				...
				//日志记录，福利送出
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("14.1"
							+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
							+ Constants.XUNLEI_LOG_DELIMITER + cXlpt
							+ Constants.XUNLEI_LOG_DELIMITER + wXlpt
							+ Constants.XUNLEI_LOG_DELIMITER + pInfo.getXunleiPoint()
							+ Constants.XUNLEI_LOG_DELIMITER +  DateUtil.getTimeDifference(new Date(), pItem.getExpireDate())
							+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date())
							);
	...
	public int everydayGiftWelfare(final Player player)throws Exception{
		...
		if(pItem!=null){
			...
			if(!pItem.getExpireDate().after(new Date())){
				...
				//日志记录，天天礼到期
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("17.2"
						+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
						+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
			...
			if(flag){
					...
					if (sysItem != null) {
						...
						if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
							nosqlService.addXunleiLog("17.1"
								+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
								+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
...
//						messageService.sendSystemMail(player, CommonMsg.EG_SUCCESS_MESSAGE_TITLE, content);
	...
	public void combinePlayerItem(final int playerId,final PlayerItem pItem1, final List<PlayerItem> pItem2,final int mulCount, boolean isFree) throws Exception{
		...
		if(isFree)
		{
			...
			if(flag == null || "".equals(flag) || !flag.equals(toDay))
			{
				...
				if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
					nosqlService.addXunleiLog("15.2"
							+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
							+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
							+ Constants.XUNLEI_LOG_DELIMITER + pItem1.getSysItem().getDisplayNameCN()
							+ Constants.XUNLEI_LOG_DELIMITER + pItem1.getId()
							+ Constants.XUNLEI_LOG_DELIMITER + pItem1.getItemId()
							+ Constants.XUNLEI_LOG_DELIMITER + Constants.getQualityLevel(pItem1.getSysItem().getRareLevel())
							+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date())
							);
				}	
	...
	private void combinePlayerItem(Player player,PlayerItem item1, List<PlayerItem> pItem2,final int mulCount,boolean isFree)throws Exception{
		...
		if (ConfigurationUtil.SWITCH_XUNLEI_LOG.getIsOn()) {
			nosqlService.addXunleiLog("15.1"
					+ Constants.XUNLEI_LOG_DELIMITER + player.getUserName()
					+ Constants.XUNLEI_LOG_DELIMITER + player.getId()
					+ Constants.XUNLEI_LOG_DELIMITER + player.getName()
					+ Constants.XUNLEI_LOG_DELIMITER + player.getRank()
					+ Constants.XUNLEI_LOG_DELIMITER + item1.getSysItem().getDisplayNameCN()
					+ Constants.XUNLEI_LOG_DELIMITER +  cLevel + ">>" + nLevel
					+ Constants.XUNLEI_LOG_DELIMITER + item1.getGst_level_exp()
					+ Constants.XUNLEI_LOG_DELIMITER + Constants.getGst_Level_Exp(item1.getGst_level(), item1.getSysItem().getRareLevel())
					+ Constants.XUNLEI_LOG_DELIMITER + item1.getId()
					+ Constants.XUNLEI_LOG_DELIMITER + item1.getItemId()
					+ Constants.XUNLEI_LOG_DELIMITER + itemIds
					+ Constants.XUNLEI_LOG_DELIMITER + CommonUtil.simpleDateFormat.format(new Date())
					);
		}	
	...
	public void addPlayerTrack(int playerId,String giftTime,int medalCount, int goldCount,double firstCost,String firstCostTime,String newRenewItem){
		...
		if(ConfigurationUtil.PLAYER_TRACK_TIMESTAMP==-1 || ConfigurationUtil.PLAYER_TRACK_TIMESTAMP!=(new Date().getMonth()+1))return;
					...
					if(!StringUtil.isEmptyString(pt.getNewRenewItem())){
						...
						for(String costItemList : saveCostItemList){
							...
							if(ths[0].equals(costItem[0]) && ths[1].equals(costItem[1]) && ths[2].equals(costItem[2])){
								pt.setNewRenewItem(pt.getNewRenewItem().replace(costItemList,
										costItemList + 
