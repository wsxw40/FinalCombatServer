...
#define default_max_apexreqpool_size	20000

#define default_server_buffersize		32 * 1024

#define SECTION_APPCFG				"ApexServer"

AppCfg::AppCfg()
	:max_proxy_count(default_max_proxy_count)
	,max_apexreqpool_size(default_max_apexreqpool_size)
...
void AppCfg::LoadConfigFile(const char * file)
{
	if (file)
	{
		...
		max_apexreqpool_size = read_profile_int(SECTION_APPCFG, "max_apexreqpool_size", default_max_apexreqpool_size, file);
...
void AppCfg::PrintAppCfg()
{
	...
	log_write_sys(LOG_INFO, "max_apexreqpool_size = %d", max_apexreqpool_size);
