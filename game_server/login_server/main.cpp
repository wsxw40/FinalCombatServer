...
int main(int argc, char** argv)
{
	...
	for (;;)
	{
		...
		case 'v':
			{
				...
				fprintf(stderr, "loginserver version %s\n", build_version);
				return 0;
			}
			break;
		...
		case flag_address:
			...
			{
				fprintf(stderr, "loginserver: invalid address -- %s\n", optarg);
				return 1;
			}
			break;
	...
	log_set_ident("loginserver-%d", ntohs(server.listener.addr.sin_port));
