...
链接: https://pan.baidu.com/s/1SIyQvSS57bUVCkRSurNu2A?pwd=xqxe 提取码: xqxe
...
| 认证服务器端口 | `8080`（login_server.py，返回 token/游戏地址） |
...
**2. Windows 登录脚本 `start_test01.bat` ~ `start_test05.bat`**
每个脚本开头有：
```bat
...
```
bat 脚本会自动：请求 `http://服务器IP:8080/confirm` 获取 token，然后按上述格式启动客户端。
...
### 启动客户端（Windows）
把 `start_test01.bat` ~ `start_test05.bat` 复制到任意位置，双击运行即可（也可用命令行参数指定客户端 exe 路径）：

```bat
start_test01.bat D:\FinalCombat\game\FinalCombat.exe
```
...
sudo bash start_all.sh        # 一键启动全部服务
```
...
| apexserver | 9003 |
| logserver | 11111 |
| dummy logserver (防 InfoLogger NPE) | 8085/8086/2222 |

停止：`pkill -f "proxyserver|channelserver|matchingserver|apexserver|logserver|logserver_dummy"`
